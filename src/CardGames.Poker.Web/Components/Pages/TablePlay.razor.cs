using System.ComponentModel;
using System.Net;
using System.Net.Http;
using System.Net.Http.Json;
using System.Security.Claims;
using System.Text.Json;
using CardGames.Contracts.SignalR;
using CardGames.Core.French.Cards;
using CardGames.Poker.Api.Clients;
using CardGames.Poker.Api.Contracts;
using CardGames.Poker.Evaluation;
using CardGames.Poker.Hands.DrawHands;
using CardGames.Poker.Hands.HandTypes;
using CardGames.Poker.Hands.StudHands;
using CardGames.Poker.Hands.WildCards;
using CardGames.Poker.Web.Components.Shared;
using CardGames.Poker.Web.Extensions;
using CardGames.Poker.Web.Infrastructure.Validation;
using CardGames.Poker.Web.Services;
using CardGames.Poker.Web.Services.TableActions;
using Microsoft.AspNetCore.Authorization;
using Microsoft.AspNetCore.Components;
using Microsoft.AspNetCore.Components.Authorization;
using Microsoft.AspNetCore.Components.Web;
using Microsoft.JSInterop;
using IApiResponse = Refit.IApiResponse;

namespace CardGames.Poker.Web.Components.Pages;

public partial class TablePlay
{
    [Parameter]
    public Guid GameId { get; set; }

    private bool _disposed;

    // Consolidated table-action lifecycle (guard, loading state, refresh, logging, notifications).
    private TableActionExecutor? _tableActions;

    private TableActionExecutor TableActions =>
        _tableActions ??= new TableActionExecutor(
            Logger,
            InvokeStateHasChangedAsync,
            (message, type, durationMs) => ShowToastAsync(message, type, durationMs));


    // Connection state (driven by SignalR)
    private bool _isConnected = false;
    private bool _isReconnecting;

    private bool _isLoading = true;

    private const int CompletedGameReturnToLobbyDurationSeconds = 10;
    private const double ScrewYourNeighborDealerTradeShowdownDelaySeconds = 4.5;
    private const int ScrewYourNeighborTradeAnimationDurationMs = 650;

    // Table info
    private int _ante = 5;

    // Game state
    private bool _isPaused;
    private int _pot;
    private int _dealerSeatIndex = 0;
    private int _currentActorSeatIndex = -1;

    // Player state
    private bool _isHost; // Determined from game creator
    private bool _isSeated;
    private bool _isReady;
    private bool _hasRequestedLeave;
    private int _currentPlayerSeatIndex = -1;
    // Authoritative chip count for the seated player, sourced from server-driven seat state.
    private int _playerChips =>
        _currentPlayerSeatIndex >= 0 && _currentPlayerSeatIndex < _seats.Count
            ? _seats[_currentPlayerSeatIndex].Chips
            : 0;
    private int _joinBuyInAmount = 100;
	private bool _joinBuyInInitializedFromTable;
    private bool _showJoinBuyInModal;
    private bool _isJoiningSeat;
    private int? _pendingJoinSeatIndex;
    private bool _showJoinApprovalPendingModal;
    private Guid? _pendingJoinRequestId;
    private string? _joinApprovalHostName;
    private int _pendingApprovalBuyIn;
    private int _joinBuyInBalance;
    private int? _joinBuyInBalanceCap;
    private bool _isPlayerTurn;
    private bool _isSubmittingAction;

    private string? _currentPlayerName;
    private Guid? _currentPlayerId; // Player ID for API calls (hand history, etc.)

    // Action state
    private List<BettingActionType> _legalActions = [];
    private int _currentMinBet;
    private int _currentMaxBet;
    private int _currentBetToCall;

    private AvailableActionsResponse? _availableActions;

    // Action timer state (from SignalR)
    private ActionTimerStateDto? _actionTimerState;

    // Draw phase state
    private bool _isDrawPhase;
    private bool _isPlayerDrawTurn;
    private bool _isSubmittingDraw;
    private bool _isDrawAnimating;
    private bool _isDiscardingCards;
    private bool _isDrawingNewCards;
    private bool _cardsDrawn;
    private List<int> _newCardIndices = [];
    private HashSet<int> _selectedDiscardIndices = [];
    private List<CardInfo> _currentPlayerDrawCards = [];
    private List<CardInfo>? _pendingCurrentPlayerDrawCards;
    private GetCurrentDrawPlayerResponse? _currentDrawPlayer;
    private bool _forceShowDrawPanel;
    private int _maxDiscards = 3;
    private string? _newHandDescription; // Hand description after drawing new cards

    // Kings and Lows specific state
    private bool _hasDropOrStayDecided;
    private string? _dropOrStayDecision;
    private bool _isProcessingDropOrStay;
    private BuyCardOfferPrivateDto? _buyCardOffer;
    private bool _isProcessingBuyCard;

    // Tollbooth specific state
    private TollboothOfferPrivateDto? _tollboothOffer;
    private bool _isProcessingTollboothChoice;
    private bool _hasDeckDrawn;
    private bool _isProcessingDeckDraw;
    private bool _isAcknowledgingPotMatch;
    private HashSet<int> _deckDrawSelectedIndices = new();

    // Screw Your Neighbor specific state
    private bool _isProcessingKeepOrTrade;
    private string? _synKeepOrTradeDecision;
    private HashSet<string> _synAnimatedTradeKeys = [];
    private HashSet<string> _playedTableSoundKeys = [];
    private List<int> _pendingSynTradeAnimationSeatIndices = [];
    private bool _synChipAnimationFired;
    private Dictionary<int, int> _synBonusChipsBySeat = new();

    // In-Between specific state
    private bool _isProcessingInBetween;
    private bool _ibAceChoiceRequired;
    private bool _ibShowBetControls;
    private bool _ibShowDealPrompt;
    private string? _ibLastTurnDescription;
    private bool _ibWaitingForReveal;
    private bool _ibWaitingForBoundaryCards;

    // Chip check pause state (for Kings and Lows)
    private bool _isPausedForChipCheck;
    private bool _isRebuyGracePause;
    private DateTimeOffset? _chipCheckPauseEndsAt;
    private int _chipCheckPotAmount;

    // Dealer's Choice state
    private bool _isDealersChoice;
    private int? _dealersChoiceDealerPosition;
    private IReadOnlyList<GetAvailablePokerGamesResponse> _dcAvailableGames = [];
    private bool _dcGamesLoading;

    // Deck draw animation state
    private bool _isDeckDrawAnimating;
    private bool _isDeckDiscardingCards;
    private bool _isDeckDrawingNewCards;
    private bool _deckCardsDrawn;
    private List<int> _deckNewCardIndices = [];
    private List<CardInfo> _deckDrawCards = [];
    private string? _deckHandDescription;
    private bool _delayShowdownOverlay; // Delay showdown after deck draw animation
    private DateTimeOffset? _showdownDelayUntil; // When the delay expires
    private string? _previousPhaseCategory; // Track phase category for transition detection

    private IReadOnlyList<CardInfo> CurrentPlayerHandCards =>
    _currentPlayerSeatIndex >= 0 && _currentPlayerSeatIndex < _seats.Count
    ? _seats[_currentPlayerSeatIndex].Cards
    : [];

    private string? currentPlayerHandEvaluationDescription =>
    _currentPlayerSeatIndex >= 0 && _currentPlayerSeatIndex < _seats.Count
    ? FormatHandDescriptionForDisplay(_seats[_currentPlayerSeatIndex].HandEvaluationDescription)
    : null;

    // Showdown state
    private PerformShowdownSuccessful? _showdownResult;
    private bool _showdownLoadInFlight;
    private bool _showShowdownOverlay;
    private bool _showdownWon;
    private bool _showdownLost;
    private ShowdownPublicDto? _showdownStateSnapshot;
    private IReadOnlyList<ShowdownCard> _showdownCommunityCardsSnapshot = [];
    private GetRabbitHuntSuccessful? _rabbitHuntResult;
    private bool _rabbitHuntLoadInFlight;
    private string? _rabbitHuntError;

    // All-In Runout state (for animated card dealing on table seats when all players are all-in)
    private AllInRunoutStateDto? _allInRunoutState;
    private bool _runoutAnimationInProgress;
    private Timer? _runoutDealTimer;
    private int _runoutCurrentStreetIndex; // Which street we're currently dealing (0 = 4th street, etc.)
    private int _runoutCurrentSeatIndex; // Which seat we're dealing to within the current street
    private Dictionary<int, int> _runoutVisibleCardCount = new(); // Seat index -> number of runout cards visible
    private Dictionary<int, int> _runoutInitialCardCounts = new(); // Seat index -> number of cards BEFORE runout started

    // Card Deal Animation state (for supported game variants)
    private bool _dealAnimationInProgress;
    private bool _dealAnimationPausedForBuyCardOffer;
    private int _dealPausedBuyCardOfferSeatIndex = -1;
    private Timer? _dealTimer;
    private TableStatePublicDto? _dealTargetState;
    private PrivateStateDto? _dealPendingPrivateState;
    private List<int> _dealOrderedSeatIndices = [];
    private int _dealCurrentCardIndex; // Which card index we are dealing (0, 1, 2 for initial; 3, 4...)
    private int _dealTargetEndIndex; // Stop dealing when _dealCurrentCardIndex reaches this
    private int _dealCurrentSeatIdx; // Index into _dealOrderedSeatIndices
    private Dictionary<int, List<CardInfo>> _dealTargetCards = new(); // SeatIndex -> Full Public Cards list

    // Flying card animation state (visual card flying from deck to player)
    private TableCanvas.FlyingCardInfo? _flyingCard;
    private Timer? _flyingCardTimer;


    // Continuous play state
    private int _secondsUntilNextHand;
    private bool _isResultsPhase;
    private Timer? _countdownTimer;
    private DateTimeOffset? _countdownDeadlineUtc;
    private TimeSpan _serverClockOffset;

    // Hand odds state
    private List<HandTypeOddsDto> handOdds = [];
    private IReadOnlyList<CardPrivateDto> _currentPrivateHand = [];

    // Dashboard / Hand history state
    private List<CardGames.Contracts.SignalR.HandHistoryEntryDto> _handHistoryEntries = [];
    private CardGames.Contracts.SignalR.ChipHistoryDto? _chipHistory;
    private int _handNumber = 0;

    // Toast notification state
    private List<ToastMessage> _toastMessages = [];

    private bool _isDealSoundMuted;
    private bool _isJsInteropReady;
    private int _previousFaceUpCommunityCardCount;
    private bool _showGameInfoOverlay;
    private bool _isUpdatingOddsVisibility;
    private bool _pendingTurnAlertSound;

    // Community card staggered reveal (for flop in Hold'Em-style games)
    private int _communityCardRevealCount;
    private int _communityCardRevealTarget;
    private Timer? _communityCardRevealTimer;
    private bool _communityCardRevealInProgress;
    private bool _deferCommunityCardsForDealAnimation;

    private List<SeatInfo> _seats = [];

    private int _seatedPlayerCount;
    private bool _canStartGame;

    private bool _initialized;
    private bool _shouldAutoJoinFromQuery;
    private bool _autoJoinIntentConsumed;
    private Guid? _leagueId;

    private bool HasFixedTournamentBuyIn => (_gameResponse?.TournamentBuyIn ?? 0) > 0;
    private bool IsLeagueTournament => HasFixedTournamentBuyIn;

    private string? _loggedInUserEmail;

    // Game type code for routing to correct API client
    private string? _gameTypeCode;
    private bool IsTwosJacksManWithTheAxe => string.Equals(_gameTypeCode, "TWOSJACKSMANWITHTHEAXE",
    StringComparison.OrdinalIgnoreCase);
    private bool IsFiveCardDraw => string.Equals(_gameTypeCode, "FIVECARDDRAW", StringComparison.OrdinalIgnoreCase);
    private bool IsKingsAndLows => string.Equals(_gameTypeCode, "KINGSANDLOWS", StringComparison.OrdinalIgnoreCase);
    private bool IsSevenCardStud => string.Equals(_gameTypeCode, "SEVENCARDSTUD", StringComparison.OrdinalIgnoreCase);
    private bool IsPairPressure => string.Equals(_gameTypeCode, "PAIRPRESSURE", StringComparison.OrdinalIgnoreCase);
    private bool IsRazz => string.Equals(_gameTypeCode, "RAZZ", StringComparison.OrdinalIgnoreCase);
    private bool IsGoodBadUgly => string.Equals(_gameTypeCode, "GOODBADUGLY", StringComparison.OrdinalIgnoreCase);
    private bool IsFollowTheQueen => string.Equals(_gameTypeCode, "FOLLOWTHEQUEEN", StringComparison.OrdinalIgnoreCase);
    private bool IsBaseball => string.Equals(_gameTypeCode, "BASEBALL", StringComparison.OrdinalIgnoreCase);
    private bool IsHoldEm => string.Equals(_gameTypeCode, "HOLDEM", StringComparison.OrdinalIgnoreCase);
    private bool IsRedRiver => string.Equals(_gameTypeCode, "REDRIVER", StringComparison.OrdinalIgnoreCase);
    private bool IsHoldTheBaseball => string.Equals(_gameTypeCode, "HOLDTHEBASEBALL", StringComparison.OrdinalIgnoreCase);
    private bool IsOmaha => string.Equals(_gameTypeCode, "OMAHA", StringComparison.OrdinalIgnoreCase);
    private bool IsNebraska => string.Equals(_gameTypeCode, "NEBRASKA", StringComparison.OrdinalIgnoreCase);
    private bool IsSouthDakota => string.Equals(_gameTypeCode, "SOUTHDAKOTA", StringComparison.OrdinalIgnoreCase);
    private bool IsBobBarker => string.Equals(_gameTypeCode, "BOBBARKER", StringComparison.OrdinalIgnoreCase);
    private bool IsIrishHoldEm => string.Equals(_gameTypeCode, "IRISHHOLDEM", StringComparison.OrdinalIgnoreCase);
    private bool IsPhilsMom => string.Equals(_gameTypeCode, "PHILSMOM", StringComparison.OrdinalIgnoreCase);
    private bool IsCrazyPineapple => string.Equals(_gameTypeCode, "CRAZYPINEAPPLE", StringComparison.OrdinalIgnoreCase);
    private bool IsScrewYourNeighbor => string.Equals(_gameTypeCode, "SCREWYOURNEIGHBOR", StringComparison.OrdinalIgnoreCase);
    private bool IsTollbooth => string.Equals(_gameTypeCode, "TOLLBOOTH", StringComparison.OrdinalIgnoreCase);
    private bool IsKlondike => string.Equals(_gameTypeCode, "KLONDIKE", StringComparison.OrdinalIgnoreCase);
    private bool IsInBetween => string.Equals(_gameTypeCode, "INBETWEEN", StringComparison.OrdinalIgnoreCase);
    private bool IsStartedScrewYourNeighborTable => IsScrewYourNeighbor && _gameResponse?.StartedAt is not null;
    private bool SupportsRabbitHunt => IsHoldEm || IsRedRiver || IsKlondike || IsOmaha || IsNebraska || IsSouthDakota || IsBobBarker || IsIrishHoldEm || IsPhilsMom || IsCrazyPineapple || IsHoldTheBaseball;
    private bool SupportsOdds => !IsInBetween && !IsScrewYourNeighbor;
    private bool IsIrishStyleHoldEm => IsIrishHoldEm || IsPhilsMom || IsCrazyPineapple;
    private bool IsSevenCardStudStyle => IsSevenCardStud || IsPairPressure;
    private bool UsesCardDealAnimation => IsSevenCardStudStyle || IsRazz || IsGoodBadUgly || IsBaseball || IsFollowTheQueen || IsFiveCardDraw || IsTwosJacksManWithTheAxe || IsKingsAndLows || IsHoldEm || IsRedRiver || IsKlondike || IsHoldTheBaseball || IsOmaha || IsNebraska || IsSouthDakota || IsBobBarker || IsIrishHoldEm || IsPhilsMom || IsCrazyPineapple || IsScrewYourNeighbor || IsTollbooth || IsInBetween;
    
    private int DealAnimationDelayMs => 750;

    // Game rules for dynamic UI rendering
    private GetGameRulesResponse? _gameRules;

    // Current table state (from SignalR)
    private TableStatePublicDto? _tableState;

    // Phase category helper properties - driven by SignalR state
    private bool IsBettingPhase =>
    _tableState?.CurrentPhaseCategory?.Equals("Betting", StringComparison.OrdinalIgnoreCase) == true;

    private bool IsDrawingPhase =>
    _tableState?.CurrentPhaseCategory?.Equals("Drawing", StringComparison.OrdinalIgnoreCase) == true
    || _isDrawPhase; // Fallback to existing logic

    private bool IsDecisionPhase =>
    _tableState?.CurrentPhaseCategory?.Equals("Decision", StringComparison.OrdinalIgnoreCase) == true;

    private bool IsSetupPhase =>
    _tableState?.CurrentPhaseCategory?.Equals("Setup", StringComparison.OrdinalIgnoreCase) == true;

    private bool IsResolutionPhase =>
    _tableState?.CurrentPhaseCategory?.Equals("Resolution", StringComparison.OrdinalIgnoreCase) == true;

    private bool IsSpecialPhase =>
    _tableState?.CurrentPhaseCategory?.Equals("Special", StringComparison.OrdinalIgnoreCase) == true;

    private bool IsBuyCardOfferPhase =>
    string.Equals(_tableState?.CurrentPhase, "BuyCardOffer", StringComparison.OrdinalIgnoreCase);

    private bool IsCurrentPlayerBuyCardDecisionMaker =>
    _currentPlayerSeatIndex >= 0 &&
    _currentPlayerSeatIndex == (_tableState?.CurrentActorSeatIndex ?? -1);

    private bool ShouldShowBuyCardOverlay =>
    IsBuyCardOfferPhase &&
    IsCurrentPlayerBuyCardDecisionMaker &&
    _buyCardOffer is not null &&
    IsParticipatingInHand &&
    (IsBuyCardTriggerCardVisible() || _dealAnimationPausedForBuyCardOffer) &&
    (!IsCardDealingVisualInProgress || _dealAnimationPausedForBuyCardOffer);

    private bool IsTollboothOfferPhase =>
    string.Equals(_tableState?.CurrentPhase, "TollboothOffer", StringComparison.OrdinalIgnoreCase);

    private bool ShouldShowTollboothOfferOverlay =>
    IsTollboothOfferPhase &&
    _tollboothOffer is not null &&
    _tollboothOffer.IsMyTurnToChoose &&
    IsParticipatingInHand;

    private bool IsCardDealingVisualInProgress =>
    _dealAnimationInProgress || _flyingCard is not null;

    // Drop or Stay phase detection (for Kings and Lows)
    private bool IsDropOrStayPhase =>
    string.Equals(_gameResponse?.CurrentPhase, "DropOrStay", StringComparison.OrdinalIgnoreCase);

    // Keep or Trade phase detection (for Screw Your Neighbor)
    private bool IsKeepOrTradePhase =>
    string.Equals(_gameResponse?.CurrentPhase, "KeepOrTrade", StringComparison.OrdinalIgnoreCase);

    // In-Between turn phase detection
    private bool IsInBetweenTurnPhase =>
    string.Equals(_gameResponse?.CurrentPhase, "InBetweenTurn", StringComparison.OrdinalIgnoreCase);

    private bool ShouldShowInBetweenOverlay =>
    IsInBetween && IsInBetweenTurnPhase && _isPlayerTurn && IsParticipatingInHand &&
    !IsCardDealingVisualInProgress;

    // Reveal phase detection (for Screw Your Neighbor)
    private bool IsRevealPhase =>
    string.Equals(_gameResponse?.CurrentPhase, "Reveal", StringComparison.OrdinalIgnoreCase);

    // Draw Complete phase detection (for Kings and Lows - delay before showdown)
    private bool IsDrawCompletePhase =>
    string.Equals(_gameResponse?.CurrentPhase, "DrawComplete", StringComparison.OrdinalIgnoreCase);

    // Player vs Deck phase detection (for Kings and Lows)
    private bool IsPlayerVsDeckPhase =>
    string.Equals(_gameResponse?.CurrentPhase, "PlayerVsDeck", StringComparison.OrdinalIgnoreCase);

    // Pot matching phase detection (for Kings and Lows)
    private bool IsPotMatchingPhase =>
    string.Equals(_gameResponse?.CurrentPhase, "PotMatching", StringComparison.OrdinalIgnoreCase);

    // Dealer's Choice phase detection
    private bool IsDealersChoicePhase =>
    string.Equals(_gameResponse?.CurrentPhase, "WaitingForDealerChoice", StringComparison.OrdinalIgnoreCase);

    private bool IsDealersChoiceWaitingForSelectedGameStart =>
    _isDealersChoice &&
    string.Equals(_gameResponse?.CurrentPhase, GamePhase.WaitingToStart.ToString(), StringComparison.OrdinalIgnoreCase) &&
    !string.IsNullOrWhiteSpace(_gameTypeCode) &&
    !string.Equals(_gameTypeCode, "DEALERSCHOICE", StringComparison.OrdinalIgnoreCase);

    private bool IsCurrentPlayerDealersChoiceDealer =>
    _isDealersChoice && _dealersChoiceDealerPosition.HasValue && _currentPlayerSeatIndex == _dealersChoiceDealerPosition.Value;

    // Whether the current player is a loser in pot matching.
    private bool IsCurrentPlayerLoser => _showdownResult?.PlayerHands?.Any(h =>
    (string.Equals(h.PlayerName, _loggedInUserEmail, StringComparison.OrdinalIgnoreCase) ||
    string.Equals(h.PlayerName, _currentPlayerName, StringComparison.OrdinalIgnoreCase)) &&
    h.IsWinner == false) == true;

    // Helper property to check if current player is participating in the current hand
    private bool IsParticipatingInHand =>
    _isSeated &&
    _currentPlayerSeatIndex >= 0 &&
    _currentPlayerSeatIndex < _seats.Count &&
    !_seats[_currentPlayerSeatIndex].IsFolded &&
    (!_seats[_currentPlayerSeatIndex].IsSittingOut || _seats[_currentPlayerSeatIndex].Cards.Count > 0);

    private bool IsSittingOut =>
    _currentPlayerSeatIndex >= 0 &&
    _currentPlayerSeatIndex < _seats.Count &&
    _seats[_currentPlayerSeatIndex].IsSittingOut;

    private bool IsInWaitingState =>
    _gameResponse?.CurrentPhase == GamePhase.WaitingForPlayers.ToString() ||
    _gameResponse?.CurrentPhase == GamePhase.WaitingToStart.ToString();

    private bool IsGameActive =>
    _gameResponse is not null && _gameResponse.CurrentPhase != GamePhase.Ended.ToString();

    private bool ShowGameVariantBadges =>
    !_isLoading && !string.IsNullOrWhiteSpace(_gameTypeCode);

    private bool ShouldShowHostControls =>
    !_isLoading && _isHost;

    private bool ShouldShowHostStartControls =>
    ShouldShowHostControls && IsInWaitingState;

    private bool ShouldShowHostInGameControls =>
    ShouldShowHostControls && IsGameActive && !IsInWaitingState;

    private bool ShouldShowActionPanel =>
    _isPlayerTurn &&
    IsBettingPhase &&
    CurrentPlayerHandCards.Count > 0 &&
    !IsCardDealingVisualInProgress;

    private bool ShouldShowDecisionPanel =>
    _isPlayerTurn &&
    IsDecisionPhase &&
    !(HasDropOrStay && IsDropOrStayPhase) &&
    !(HasKeepOrTrade && IsKeepOrTradePhase) &&
    !IsInBetween &&
    !IsCardDealingVisualInProgress;

    private bool ShouldShowDrawPanel =>
    !IsCardDealingVisualInProgress &&
    !IsTollboothOfferPhase &&
    ((IsDrawingPhase && (_isPlayerDrawTurn || _isPlayerTurn)) || _forceShowDrawPanel || IsDrawCompletePhase) &&
    IsParticipatingInHand;

    private bool ShouldShowDrawWaitingIndicator =>
    IsDrawingPhase &&
    !IsTollboothOfferPhase &&
    !(_isPlayerDrawTurn || _isPlayerTurn) &&
    _currentDrawPlayer is not null;

    private bool ShouldShowWaitingForPlayersOverlay =>
    !_isPausedForChipCheck &&
    _gameResponse?.CurrentPhase == GamePhase.WaitingForPlayers.ToString() &&
    (!_isSeated || IsSittingOut || !_isReady);

    private bool ShouldShowPausedOverlay =>
    _isPaused &&
    _gameResponse is not null &&
    _gameResponse.CurrentPhase != GamePhase.WaitingForPlayers.ToString() &&
    _gameResponse.CurrentPhase != GamePhase.Ended.ToString() &&
    _gameResponse.CurrentPhase != GamePhase.WaitingToStart.ToString() &&
    _gameResponse.CurrentPhase != GamePhase.WaitingForDealerChoice.ToString();

    // Special rules helper properties - driven by SignalR state
    private bool HasWildCards =>
    _tableState?.SpecialRules?.HasWildCards == true;

    private bool HasSevensSplit =>
    _tableState?.SpecialRules?.HasSevensSplit == true;

    private bool HasDropOrStay =>
    _tableState?.SpecialRules?.HasDropOrStay == true;

    private bool HasKeepOrTrade =>
    _tableState?.SpecialRules?.HasKeepOrTrade == true;

    private bool IsCurrentPlayerScrewYourNeighborDealer =>
    IsScrewYourNeighbor &&
    _currentPlayerSeatIndex >= 0 &&
    _currentPlayerSeatIndex == _dealerSeatIndex;

    private string? CurrentScrewYourNeighborTradeTargetName
    {
        get
        {
            if (!TryGetCurrentScrewYourNeighborTradeTarget(out var tradeTarget))
            {
                return null;
            }

            return string.IsNullOrWhiteSpace(tradeTarget.PlayerFirstName)
                ? tradeTarget.PlayerName
                : tradeTarget.PlayerFirstName;
        }
    }

    private bool IsCurrentPlayerTradeBlockedByTargetKing
    {
        get
        {
            if (!IsScrewYourNeighbor || IsCurrentPlayerScrewYourNeighborDealer)
            {
                return false;
            }

            if (!TryGetCurrentScrewYourNeighborTradeTarget(out var tradeTarget))
            {
                return false;
            }

            return tradeTarget.Cards.Any(card =>
                card.IsFaceUp &&
                string.Equals(card.Rank, "K", StringComparison.OrdinalIgnoreCase));
        }
    }

    private bool TryGetCurrentScrewYourNeighborTradeTarget(out SeatInfo tradeTarget)
    {
        tradeTarget = default!;

        if (!TryGetScrewYourNeighborTradeTarget(_currentPlayerSeatIndex, out var targetSeatIndex))
        {
            return false;
        }

        if (targetSeatIndex < 0 || targetSeatIndex >= _seats.Count)
        {
            return false;
        }

        tradeTarget = _seats[targetSeatIndex];
        return tradeTarget.IsOccupied;
    }

    private bool TryGetScrewYourNeighborTradeTarget(int sourceSeatIndex, out int targetSeatIndex)
    {
        targetSeatIndex = -1;

        if (_seats.Count == 0 || sourceSeatIndex < 0)
        {
            return false;
        }

        var eligibleSeats = _seats
            .Where(seat => seat.IsOccupied && !seat.IsSittingOut && !seat.IsFolded)
            .OrderBy(seat => seat.SeatIndex)
            .ToList();

        if (eligibleSeats.Count <= 1)
        {
            return false;
        }

        var sourceSeatPosition = eligibleSeats.FindIndex(seat => seat.SeatIndex == sourceSeatIndex);
        if (sourceSeatPosition < 0)
        {
            return false;
        }

        targetSeatIndex = eligibleSeats[(sourceSeatPosition + 1) % eligibleSeats.Count].SeatIndex;
        return true;
    }

    private bool HasPotMatching =>
    _tableState?.SpecialRules?.HasPotMatching == true;

    // Get available actions for current phase
    private IReadOnlyList<string> CurrentPhaseAvailableActions =>
    _tableState?.CurrentPhaseAvailableActions ?? [];

    private bool IsCompletePhase => string.Equals(_gameResponse?.CurrentPhase, "Complete",
    StringComparison.OrdinalIgnoreCase);

    private bool IsEndedPhase => string.Equals(_gameResponse?.CurrentPhase, GamePhase.Ended.ToString(),
    StringComparison.OrdinalIgnoreCase);

    private bool IsGameCompleted => _gameResponse?.Status == GameStatus.Completed;

    // Screw Your Neighbor and In-Between should only show showdown once the game has fully ended.
    private bool ShouldDisplayShowdownOverlay => (!IsScrewYourNeighbor && !IsInBetween) || IsEndedPhase || IsGameCompleted || (IsScrewYourNeighbor && IsCompletePhase && _tableState?.NextHandStartsAtUtc is null);

    private bool IsShowdownPhase => string.Equals(_gameResponse?.CurrentPhase, GamePhase.Showdown.ToString(),
    StringComparison.OrdinalIgnoreCase)
    || string.Equals(_gameResponse?.CurrentPhase, "Showdown", StringComparison.OrdinalIgnoreCase)
    || IsPotMatchingPhase
    || IsCompletePhase
    || IsEndedPhase;

    protected override async Task OnInitializedAsync()
    {
        if (_initialized)
        {
            return;
        }

        _initialized = true;
        InitializeSeats();
        CaptureAutoJoinIntentFromUri();

        // Subscribe to SignalR events
        GameHubClient.OnTableStateUpdated += HandleTableStateUpdatedAsync;
        GameHubClient.OnPrivateStateUpdated += HandlePrivateStateUpdatedAsync;
        GameHubClient.OnPlayerJoined += HandlePlayerJoinedAsync;
        GameHubClient.OnConnectionStateChanged += HandleConnectionStateChanged;
        GameHubClient.OnActionTimerUpdated += HandleActionTimerUpdatedAsync;
        GameHubClient.OnPlayerActionPerformed += HandlePlayerActionPerformedAsync;
        GameHubClient.OnTableSettingsUpdated += HandleTableSettingsUpdatedAsync;
        GameHubClient.OnTableToastNotification += HandleTableToastNotificationAsync;
        NotificationHubClient.OnGameJoinRequestResolved += HandleJoinRequestResolvedAsync;

        // Load initial data FIRST so seats and _gameResponse are populated
        // before SignalR sends its state snapshot
        await LoadDataAsync();

        // Auto-join intent is handled once after initial seat data is loaded.
        await TriggerAutoJoinIntentIfRequestedAsync();

        // Connect to SignalR hub and join game - the snapshot will now
        // be processed correctly since seats are already populated
        await ConnectAndJoinGameAsync();
    }

    private void CaptureAutoJoinIntentFromUri()
    {
        _shouldAutoJoinFromQuery = HasAutoJoinIntent(NavigationManager.Uri);
    }

    private async Task TriggerAutoJoinIntentIfRequestedAsync()
    {
        if (_autoJoinIntentConsumed || !_shouldAutoJoinFromQuery)
        {
            return;
        }

        _autoJoinIntentConsumed = true;
        if (_isSeated)
        {
            return;
        }

        await BeginAutoSeatJoinAsync();
    }

    private static bool HasAutoJoinIntent(string? uriValue)
    {
        if (string.IsNullOrWhiteSpace(uriValue) ||
            !Uri.TryCreate(uriValue, UriKind.Absolute, out var uri) ||
            string.IsNullOrWhiteSpace(uri.Query))
        {
            return false;
        }

        var query = uri.Query.TrimStart('?');
        foreach (var segment in query.Split('&', StringSplitOptions.RemoveEmptyEntries))
        {
            var parts = segment.Split('=', 2, StringSplitOptions.TrimEntries);
            if (parts.Length == 0 || !parts[0].Equals("autojoin", StringComparison.OrdinalIgnoreCase))
            {
                continue;
            }

            var value = parts.Length > 1 ? Uri.UnescapeDataString(parts[1]) : "1";
            return value.Equals("1", StringComparison.OrdinalIgnoreCase)
                || value.Equals("true", StringComparison.OrdinalIgnoreCase);
        }

        return false;
    }

    protected override async Task OnAfterRenderAsync(bool firstRender)
    {
        if (_isJsInteropReady)
        {
            return;
        }

        try
        {
            _isDealSoundMuted = await JSRuntime.InvokeAsync<bool>("cardGamesAudio.getMuted");
            _isJsInteropReady = true;
            await TryPlayTableSoundEffectsAsync(_tableState?.SoundEffects);
            await FlushPendingScrewYourNeighborTradeAnimationsAsync();
            await InvokeStateHasChangedAsync();
        }
        catch (InvalidOperationException ex) when (ex.Message.Contains("statically rendered", StringComparison.OrdinalIgnoreCase))
        {
            // Prerender phase: JS interop is not available yet.
            // We'll try again on the next render when the circuit becomes interactive.
        }
        catch (JSDisconnectedException)
        {
            // Circuit is not currently connected for JS interop.
        }
        catch (Exception ex)
        {
            Logger.LogDebug(ex, "Unable to load persisted deal-card mute state");
        }
    }

    private async Task ConnectAndJoinGameAsync()
    {
        try
        {
            await GameHubClient.ConnectAsync();
            _isConnected = GameHubClient.IsConnected;
            await GameHubClient.JoinGameAsync(GameId);
            Logger.LogInformation("Connected to SignalR and joined game {GameId}", GameId);
        }
        catch (Exception ex)
        {
            Logger.LogError(ex, "Failed to connect to SignalR hub for game {GameId}", GameId);
            _isConnected = false;
        }
    }

    private void HandleConnectionStateChanged(Microsoft.AspNetCore.SignalR.Client.HubConnectionState state)
    {
        _isConnected = state == Microsoft.AspNetCore.SignalR.Client.HubConnectionState.Connected;
        _isReconnecting = state == Microsoft.AspNetCore.SignalR.Client.HubConnectionState.Reconnecting;
        _ = InvokeStateHasChangedAsync();
    }

    private async Task HandlePlayerJoinedAsync(PlayerJoinedDto notification)
    {
        if (notification.GameId != GameId)
        {
            return;
        }

        Logger.LogDebug("Player {PlayerName} joined at seat {SeatIndex}", notification.PlayerName, notification.SeatIndex);

        // Show toast notification to other players
        await ShowToastAsync(notification.Message, "info");
    }

    private async Task HandleTableToastNotificationAsync(TableToastNotificationDto notification)
    {
        if (notification.GameId != GameId)
        {
            return;
        }

        await ShowToastAsync(notification.Message, notification.Type, notification.DurationMs);
    }

    private async Task ShowToastAsync(string message, string type = "info", int durationMs = 4000)
    {
        var toast = new ToastMessage(message, type);
        _toastMessages.Add(toast);
        await InvokeStateHasChangedAsync();

        // Auto-dismiss after duration
        _ = Task.Run(async () =>
        {
            await Task.Delay(durationMs);
            _toastMessages.Remove(toast);
            await InvokeStateHasChangedAsync();
        });
    }

    private async Task HandleTableStateUpdatedAsync(TableStatePublicDto state)
    {
        if (state.GameId != GameId)
        {
            return;
        }

        Logger.LogDebug("Received table state update for game {GameId}, phase: {Phase}", state.GameId, state.CurrentPhase);

        // Store the table state for phase category checks
        _tableState = state;
        await TryPlayTableSoundEffectsAsync(state.SoundEffects);

        // In-Between: show bet controls once boundary cards arrive via SignalR
        if (_ibWaitingForBoundaryCards && state.CommunityCards?.Any(c => c.IsFaceUp) == true)
        {
            _ibWaitingForBoundaryCards = false;
            _ibShowBetControls = true;
            _ibAceChoiceRequired = false;
        }

        if (_dealAnimationInProgress && _dealAnimationPausedForBuyCardOffer)
        {
            var currentOfferSeat = state.CurrentActorSeatIndex;
            var offerAdvanced = !IsBuyCardOfferPhase || currentOfferSeat != _dealPausedBuyCardOfferSeatIndex;
            if (offerAdvanced)
            {
                _dealAnimationPausedForBuyCardOffer = false;
                _dealPausedBuyCardOfferSeatIndex = -1;
                _dealTimer?.Change(100, DealAnimationDelayMs);
            }
        }

        if (_dealAnimationInProgress)
        {
            _dealTargetState = state;
        }

        // Update deck hand description from state
        if (state.PlayerVsDeck is not null)
        {
            _deckHandDescription = FormatHandDescriptionForDisplay(state.PlayerVsDeck.DeckHandDescription);
        }

        // Update game type code for routing to correct API
        if (!string.IsNullOrWhiteSpace(state.GameTypeCode))
        {
            var gameTypeChanged = _gameTypeCode != state.GameTypeCode;
            _gameTypeCode = state.GameTypeCode;

            // Load game rules if not loaded or game type changed (e.g. Dealer's Choice rotation)
            if ((_gameRules is null || gameTypeChanged) && !string.IsNullOrWhiteSpace(_gameTypeCode))
            {
                _gameRules = await GameRulesService.GetGameRulesAsync(_gameTypeCode);
            }
        }

        // Update game response fields from public state
        // Handle case where _gameResponse is not yet loaded from API
        if (_gameResponse is not null)
        {
            var previousHandNumber = _gameResponse.CurrentHandNumber;
            var previousPhase = _gameResponse.CurrentPhase;

            _gameResponse = _gameResponse with
            {
                CurrentPhase = state.CurrentPhase,
                CurrentPhaseDescription = state?.CurrentPhaseDescription ?? "",
                Name = state.Name ?? _gameResponse.Name,
                GameTypeName = _isDealersChoice ? state.GameTypeName : (state.GameTypeName ?? _gameResponse.GameTypeName),
                Ante = state.Ante,
                MinBet = state.MinBet,
                DealerPosition = state.DealerSeatIndex,
                CurrentPlayerIndex = state.CurrentActorSeatIndex,
                CurrentHandNumber = state.CurrentHandNumber,
                AreOddsVisibleToAllPlayers = state.AreOddsVisibleToAllPlayers ?? _gameResponse.AreOddsVisibleToAllPlayers
            };

            _ante = state.Ante;
			NormalizeJoinBuyIn();
            _pot = state.TotalPot;
            _dealerSeatIndex = state.DealerSeatIndex;
            _currentActorSeatIndex = state.CurrentActorSeatIndex;
            _isPaused = state.IsPaused;

            // Update Dealer's Choice state from SignalR
            _isDealersChoice = state.IsDealersChoice;
            _dealersChoiceDealerPosition = state.DealersChoiceDealerPosition;
            _actionTimerState = state.ActionTimer;

            // Dealer's Choice: load available games when entering WaitingForDealerChoice
            if (_isDealersChoice
                && string.Equals(state.CurrentPhase, "WaitingForDealerChoice", StringComparison.OrdinalIgnoreCase)
                && !string.Equals(previousPhase, "WaitingForDealerChoice", StringComparison.OrdinalIgnoreCase)
                && _dcAvailableGames.Count == 0)
            {
                _ = LoadAvailableGamesAsync();
            }

            // Update chip check pause state (for Kings and Lows)
            if (state.ChipCheckPause is not null)
            {
                _isPausedForChipCheck = state.ChipCheckPause.IsPaused;
                _isRebuyGracePause = string.Equals(state.ChipCheckPause.PauseType, "RebuyGrace", StringComparison.OrdinalIgnoreCase);
                _chipCheckPauseEndsAt = state.ChipCheckPause.PauseEndsAt;
                _chipCheckPotAmount = state.ChipCheckPause.PotAmountToCover;
            }
            else
            {
                _isPausedForChipCheck = false;
                _isRebuyGracePause = false;
                _chipCheckPauseEndsAt = null;
                _chipCheckPotAmount = 0;
            }

            // Clean up old hand cards when starting a new hand to ensure animations work correctly
            if (state.CurrentHandNumber != previousHandNumber)
            {
                foreach (var s in _seats)
                {
                    s.Cards = [];
                    s.HandEvaluationDescription = null;
                }
                _synKeepOrTradeDecision = null;
                _synChipAnimationFired = false;
                ClearSynBonusChips();

                _previousFaceUpCommunityCardCount = 0;
                _communityCardRevealInProgress = false;
                _deferCommunityCardsForDealAnimation = false;
                _communityCardRevealTimer?.Dispose();
                _communityCardRevealTimer = null;
                ResetDrawPanelStateForNewHand();
            }

            var hasIncomingAllInRunout = HasActiveAllInRunout(state);

            // Check for Card Deal Animation trigger (all supported variants)
            // Detect ANY increase in cards for active players.
            // All-in runout owns the dealing sequence for that update and must not overlap
            // with the generic stud/card deal animation.
            if (UsesCardDealAnimation && !_dealAnimationInProgress && !_runoutAnimationInProgress && !hasIncomingAllInRunout)
            {
                // Identify active players who are not folded in the NEW state (since if folded, they don't get new cards)
                var activeSeatIndices = _seats
                    .Where(s => s.IsOccupied && !s.IsSittingOut)
                    .Select(s => s.SeatIndex)
                    .ToList();

                if (activeSeatIndices.Count > 0)
                {
                    // Find a player who is active in both states and check if their card count increased
                    foreach (var seatIdx in activeSeatIndices)
                    {
                        var currentSeat = _seats.FirstOrDefault(s => s.SeatIndex == seatIdx);
                        var newSeat = state.Seats.FirstOrDefault(s => s.SeatIndex == seatIdx);

                        // Only consider non-folded players in the new state (folded players stop receiving cards)
                        // Exception: Initial deal (current count 0) everyone gets cards even if technically not "folded" yet
                        if (currentSeat != null && newSeat != null && !newSeat.IsFolded)
                        {
                            var currentCount = currentSeat.Cards.Count;
                            var newCount = newSeat.Cards.Count;

                            if (newCount > currentCount)
                            {
                                // Trigger animation for the difference
                                StartCardDealAnimation(state, currentCount, newCount - currentCount, seatIdx);
                                break; // Only trigger once
                            }
                        }
                    }
                }
            }

            // IMPORTANT: Check for All-In Runout BEFORE updating seats
            // This ensures we capture initial card counts and set the animation flag
            // before UpdateSeatsFromPublicState would overwrite cards with full runout cards
            if (hasIncomingAllInRunout && !_runoutAnimationInProgress)
            {
                _allInRunoutState = state.AllInRunout;

                // Capture the initial card count for each seat BEFORE the runout cards are added
                // This is the number of cards dealt before everyone went all-in
                _runoutInitialCardCounts.Clear();
                foreach (var seatIndex in state.AllInRunout.RunoutCardsBySeat?.Keys ?? [])
                {
                    if (seatIndex >= 0 && seatIndex < _seats.Count && _seats[seatIndex].IsOccupied)
                    {
                        _runoutInitialCardCounts[seatIndex] = _seats[seatIndex].Cards.Count;
                    }
                }

                // Set the animation flag BEFORE UpdateSeatsFromPublicState
                _runoutAnimationInProgress = true;
                _runoutCurrentStreetIndex = 0;
                _runoutCurrentSeatIndex = 0;
                _runoutVisibleCardCount.Clear();

                // Initialize visible runout card count to 0 for each seat
                foreach (var seatIndex in state.AllInRunout.RunoutCardsBySeat?.Keys ?? [])
                {
                    _runoutVisibleCardCount[seatIndex] = 0;
                }

                Logger.LogInformation("Detected all-in runout for game {GameId}, starting table animation", state.GameId);
            }

            // Update seats from public state
            // Note: If _runoutAnimationInProgress is true, this will preserve cards for runout seats
            UpdateSeatsFromPublicState(state.Seats);

            var currentFaceUpCommunityCardCount = state.CommunityCards?.Count(c => c.IsFaceUp) ?? 0;
            var newlyFaceUpCommunityCards = Math.Max(0, currentFaceUpCommunityCardCount - _previousFaceUpCommunityCardCount);

            // Tollbooth: defer community card reveal until the player deal animation completes
            if (IsTollbooth && _dealAnimationInProgress && newlyFaceUpCommunityCards > 0)
            {
                _deferCommunityCardsForDealAnimation = true;
            }
            else if (newlyFaceUpCommunityCards > 1)
            {
                // Multiple new community cards (e.g., flop) — stagger the reveal
                _communityCardRevealCount = _previousFaceUpCommunityCardCount + 1; // Show first card immediately
                _communityCardRevealTarget = currentFaceUpCommunityCardCount;
                _communityCardRevealInProgress = true;
                await PlayDealCardSoundAsync(1); // Sound for first card
                _communityCardRevealTimer?.Dispose();
                _communityCardRevealTimer = new Timer(RevealNextCommunityCard, null, 250, Timeout.Infinite);
            }
            else if (newlyFaceUpCommunityCards == 1)
            {
                await PlayDealCardSoundAsync(1);
            }

            _previousFaceUpCommunityCardCount = currentFaceUpCommunityCardCount;

            // Start the runout timer AFTER UpdateSeatsFromPublicState (if we just detected runout)
            if (_runoutAnimationInProgress && _runoutDealTimer is null)
            {
                // Start the timer to deal cards one by one (2 second delay before first card)
                _runoutDealTimer = new Timer(DealNextRunoutCard, null, TimeSpan.FromSeconds(2), Timeout.InfiniteTimeSpan);
            }

            // Update previous phase category for next state update
            var currentPhaseCategory = state.CurrentPhaseCategory;
            _previousPhaseCategory = currentPhaseCategory;

            // Handle continuous play state transitions
            var isStartPhase = state.CurrentPhase == "CollectingAntes" || state.CurrentPhase == "Dealing" ||
            state.CurrentPhase == "DropOrStay" || state.CurrentPhase == "WaitingForPlayers";

            var wasStartPhase = previousPhase == "CollectingAntes" || previousPhase == "Dealing" ||
            previousPhase == "DropOrStay" || previousPhase == "WaitingForPlayers";

            var handChanged = state.CurrentHandNumber != previousHandNumber;

            var enteredScrewYourNeighborShowdown =
                IsScrewYourNeighbor &&
                IsShowdownPhase &&
                !string.Equals(previousPhase, "Showdown", StringComparison.OrdinalIgnoreCase);

            if (enteredScrewYourNeighborShowdown && !_showdownDelayUntil.HasValue)
            {
                ScheduleShowdownOverlayAfterDelay(TimeSpan.FromSeconds(ScrewYourNeighborDealerTradeShowdownDelaySeconds));
            }

            // Animate chip(s) flying from losing player seats to the pot when entering SYN showdown.
            // Use the showdown PlayerResults to identify losers (IsWinner == false) rather than
            // chip-count comparison, because SYN chips are deducted at ante time — not at showdown.
            if (enteredScrewYourNeighborShowdown && state.Showdown is not null && !_synChipAnimationFired)
            {
                _synChipAnimationFired = true;
                var loserSeatIndices = state.Showdown.PlayerResults
                    .Where(r => !r.IsWinner)
                    .Select(r => r.SeatPosition)
                    .ToList();

                Logger.LogInformation("SYN Showdown chip-loss animation for seats: [{Seats}]",
                    string.Join(", ", loserSeatIndices));

                if (loserSeatIndices.Count > 0)
                {
                    // Add bonus chips so the UI keeps showing the pre-loss stack count
                    // until the chip-to-pot animation finishes.
                    foreach (var seatIndex in loserSeatIndices)
                    {
                        _synBonusChipsBySeat[seatIndex] = _ante;
                    }

                    _ = InvokeAsync(() => AnimateScrewYourNeighborChipLossAsync(loserSeatIndices));
                }
            }

            // Handle showdown state - check if entering showdown/complete phase
            // Pass whether we should delay showing the overlay (only for deck draw animation now)
            // Re-evaluate delay flag AFTER Klondike/SYN delays have been scheduled above
            var shouldDelayShowdownOverlay = _delayShowdownOverlay ||
            (_showdownDelayUntil.HasValue && DateTimeOffset.UtcNow < _showdownDelayUntil.Value);

            // If runout animation is in progress, delay the showdown
            if (_runoutAnimationInProgress)
            {
                shouldDelayShowdownOverlay = true;
            }

            if (state.Showdown is not null)
            {
                _showdownStateSnapshot = state.Showdown;
                UpdateShowdownFromPublicState(state.Showdown, shouldDelayShowdownOverlay || _runoutAnimationInProgress);
            }

            var showdownCommunityCards = GetShowdownCommunityCards();
            if (showdownCommunityCards.Count > 0)
            {
                _showdownCommunityCardsSnapshot = showdownCommunityCards;
            }

            // If entering showdown or complete phase, load showdown results via API when
            // the server has not already provided a complete showdown payload.
            if (IsShowdownPhase &&
                _showdownResult is null &&
                !_delayShowdownOverlay &&
                !_runoutAnimationInProgress &&
                !(_showdownDelayUntil.HasValue && DateTimeOffset.UtcNow < _showdownDelayUntil.Value) &&
                !(IsScrewYourNeighbor && state.Showdown is { IsComplete: true }))
            {
                await TryLoadShowdownAsync();
            }

            // If deck just finished drawing (via SignalR update from another player's action),
            // set a delay for showing the showdown overlay
            if (state.PlayerVsDeck?.HasDeckDrawn == true && !_hasDeckDrawn && !_showdownDelayUntil.HasValue &&
            !IsCurrentPlayerDeckDecisionMaker)
            {
                // Non-decision maker: delay showdown overlay by 5 seconds to let them see the animation
                ScheduleShowdownOverlayAfterDelay(TimeSpan.FromSeconds(5));
                _hasDeckDrawn = true;
            }

            // Handle continuous play countdown from the server-authored deadline.
            _isResultsPhase = state.IsResultsPhase;
            SyncCountdownWithServer(state);

            bool keepCountdownRunning = _showShowdownOverlay && _countdownDeadlineUtc.HasValue && _secondsUntilNextHand > 0;

            if ((_isResultsPhase || IsEndedPhase || IsGameCompleted || keepCountdownRunning) && _countdownDeadlineUtc.HasValue)
            {
                StartCountdownTimer();
            }
            else if (!_isResultsPhase && !IsEndedPhase && !IsGameCompleted && !keepCountdownRunning)
            {
                StopCountdownTimer();
                _countdownDeadlineUtc = null;
                _secondsUntilNextHand = 0;
                // Clear showdown overlay when new hand starts
                if (!IsShowdownPhase && !IsGameCompleted)
                {
                    _showShowdownOverlay = false;
                    _showdownResult = null;
                    _showdownStateSnapshot = null;
                    _showdownCommunityCardsSnapshot = [];
                    ResetRabbitHuntState();

                    // Only reset game state if we are truly starting a new hand (Setup/Dealing phases)
                    // This prevents UI state (like visible cards during draw delay) from being cleared
                    // just because a SignalR update arrived for a subsequent phase like SecondBettingRound
                    var isNewHand = state.CurrentPhaseCategory?.Equals("Setup", StringComparison.OrdinalIgnoreCase) == true;

                    if (isNewHand || (isStartPhase && !wasStartPhase) || handChanged)
                    {
                        isNewHand = true;
                    }
                    else
                    {
                        isNewHand = false;
                    }

                    if (isNewHand)
                    {
                        // Reset Kings and Lows specific state for new hand
                        _hasDropOrStayDecided = false;
                        _dropOrStayDecision = null;
                        _hasDeckDrawn = false;
                        _deckHandDescription = null;
                        _deckDrawSelectedIndices.Clear();
                        _delayShowdownOverlay = false;
                        _showdownDelayUntil = null;
                        _previousPhaseCategory = null;
                        _isAcknowledgingPotMatch = false;
                        _synAnimatedTradeKeys.Clear();
                        _playedTableSoundKeys.Clear();

                        // Reset All-In Runout state for new hand
                        _allInRunoutState = null;
                        _runoutAnimationInProgress = false;
                        _runoutDealTimer?.Dispose();
                        _runoutDealTimer = null;
                        _runoutCurrentStreetIndex = 0;
                        _runoutCurrentSeatIndex = 0;
                        _runoutVisibleCardCount.Clear();
                        _runoutInitialCardCounts.Clear();

                        // Reset DrawPanel state for new hand
                        ResetDrawPanelStateForNewHand();
                    }
                }
            }
        }
        else
        {
            // SignalR update arrived before LoadDataAsync completed.
            // Apply minimal public state so table visuals (pot, seats, dealer/current actor)
            // remain accurate even before _gameResponse is initialized.
            Logger.LogDebug("Received SignalR update before initial data loaded, applying minimal public state update");

            _ante = state.Ante;
			NormalizeJoinBuyIn();
            _pot = state.TotalPot;
            _dealerSeatIndex = state.DealerSeatIndex;
            _currentActorSeatIndex = state.CurrentActorSeatIndex;
            _isPaused = state.IsPaused;

            _isDealersChoice = state.IsDealersChoice;
            _dealersChoiceDealerPosition = state.DealersChoiceDealerPosition;
            _actionTimerState = state.ActionTimer;

            if (state.ChipCheckPause is not null)
            {
                _isPausedForChipCheck = state.ChipCheckPause.IsPaused;
                _isRebuyGracePause = string.Equals(state.ChipCheckPause.PauseType, "RebuyGrace", StringComparison.OrdinalIgnoreCase);
                _chipCheckPauseEndsAt = state.ChipCheckPause.PauseEndsAt;
                _chipCheckPotAmount = state.ChipCheckPause.PotAmountToCover;
            }
            else
            {
                _isPausedForChipCheck = false;
                _isRebuyGracePause = false;
                _chipCheckPauseEndsAt = null;
                _chipCheckPotAmount = 0;
            }

            UpdateSeatsFromPublicState(state.Seats);
        }

        // Update phase-dependent state
        _isDrawPhase = state.CurrentPhase == "DrawPhase";

        // Note: Hand history is now updated from private state (HandlePrivateStateUpdatedAsync)
        // because it contains complete player results for all participants in each hand

        // Recalculate derived state
        _seatedPlayerCount = _seats.Count(s => s.IsOccupied);
        var activePlayers = _seats.Where(s => s.IsOccupied && !s.IsSittingOut);
        _canStartGame = activePlayers.Count() >= (_gameResponse?.MinimumNumberOfPlayers ?? 2) &&
        activePlayers.All(s => s.IsReady);

        // Update current player info
        _currentPlayerSeatIndex = _seats.FirstOrDefault(s => s.IsOccupied &&
        !string.IsNullOrWhiteSpace(_loggedInUserEmail) &&
        string.Equals(s.PlayerName, _loggedInUserEmail, StringComparison.OrdinalIgnoreCase))?.SeatIndex ?? -1;

        _isSeated = _currentPlayerSeatIndex >= 0;
        _isReady = _isSeated && _seats[_currentPlayerSeatIndex].IsReady;

        if (_currentPrivateHand.Count > 0)
        {
            CalculateHandOdds(_currentPrivateHand);
        }

        // Redirect if the player requested to leave and is no longer seated (hand completed)
        if (_hasRequestedLeave && !_isSeated)
        {
            _hasRequestedLeave = false;
            await GameHubClient.LeaveGameAsync(GameId);
            NavigateToExitDestination();
            return;
        }

        await InvokeStateHasChangedAsync();
    }

    private async Task HandlePrivateStateUpdatedAsync(PrivateStateDto state)
    {
        if (state.GameId != GameId)
        {
            return;
        }

        _currentPrivateHand = state.Hand;

        Logger.LogDebug("Received private state update for player {PlayerName} at seat {SeatPosition}",
        state.PlayerName, state.SeatPosition);

        // Use the seat position from private state as the authoritative source.
        // This is more reliable than name matching since the server knows the exact seat.
        var targetSeatIndex = state.SeatPosition;

        // Update currentPlayerSeatIndex if not set or if it differs from the authoritative position
        if (_currentPlayerSeatIndex < 0 || _currentPlayerSeatIndex != targetSeatIndex)
        {
            _currentPlayerSeatIndex = targetSeatIndex;
            _currentPlayerName = state.PlayerName;
            _isSeated = true;

            // Mark the correct seat as current player
            if (targetSeatIndex >= 0 && targetSeatIndex < _seats.Count)
            {
                _seats[targetSeatIndex].IsCurrentPlayer = true;
            }
        }

        // Update player's cards using the authoritative seat position
        // Always update cards from private state - even if empty (player is sitting out or between hands)
        // BUT: Skip card updates during runout animation - the animation controls card visibility
        if (targetSeatIndex >= 0 && targetSeatIndex < _seats.Count)
        {
            var previousCardCount = _seats[targetSeatIndex].Cards.Count;

            var isInRunoutAnimation = _runoutAnimationInProgress &&
            _allInRunoutState?.RunoutCardsBySeat?.ContainsKey(targetSeatIndex) == true;

            if (isInRunoutAnimation || _dealAnimationInProgress)
            {
                Logger.LogDebug(
                "Skipping private state card update for seat {SeatIndex} during animation (Runout={Runout}, Deal={Deal})",
                targetSeatIndex, isInRunoutAnimation, _dealAnimationInProgress);

                if (_dealAnimationInProgress && state.Hand is { Count: > 0 })
                {
                    _dealPendingPrivateState = state;
                }
            }
            else if (state.Hand is { Count: > 0 })
            {
                Logger.LogInformation(
                "Updating seat {SeatIndex} cards from private state: {CardCount} cards, first card: {FirstCard}",
                targetSeatIndex,
                state.Hand.Count,
                $"{state.Hand[0].Rank} of {state.Hand[0].Suit}");

                _seats[targetSeatIndex].Cards = state.Hand.Select(c => new CardInfo
                {
                    Rank = c.Rank,
                    Suit = c.Suit,
                    IsFaceUp = c.IsPubliclyVisible,
                    IsPubliclyVisible = c.IsPubliclyVisible,
                    IsSelected = false,
                    DealOrder = c.DealOrder,
                    IsWild = IsWildCard(c.Rank, c.Suit),
                    IsShowcaseCard = c.IsShowcaseCard
                }).ToList();

                ApplyKingsAndLowsLowestCardWildFallback(_seats[targetSeatIndex].Cards);

                var formattedPrivateHandDescription =
                    FormatHandDescriptionForDisplay(state.HandEvaluationDescription);

                _seats[targetSeatIndex].HandEvaluationDescription =
                    IsScrewYourNeighbor
                        ? _synKeepOrTradeDecision
                            ?? formattedPrivateHandDescription
                            ?? _seats[targetSeatIndex].HandEvaluationDescription
                        : formattedPrivateHandDescription
                            ?? FormatHandDescriptionForDisplay(BuildFallbackHandDescription(state.Hand));

                var newlyDealtCardCount = Math.Max(0, state.Hand.Count - previousCardCount);
                if (targetSeatIndex == _currentPlayerSeatIndex && newlyDealtCardCount > 0)
                {
                    await PlayDealCardSoundAsync(newlyDealtCardCount);
                }
            }
            else
            {
                // Clear cards when hand is empty (player is sitting out or new hand hasn't dealt yet)
                Logger.LogInformation(
                "Clearing seat {SeatIndex} cards from private state: Hand is empty",
                targetSeatIndex);

                _seats[targetSeatIndex].Cards = [];
                _seats[targetSeatIndex].HandEvaluationDescription = null;
            }

            _seats[targetSeatIndex].IsCurrentPlayer = true;
        }

        // Update available actions
        if (state.AvailableActions is not null)
        {
            _availableActions = new AvailableActionsResponse(
            callAmount: state.AvailableActions.CallAmount,
            canAllIn: state.AvailableActions.CanAllIn,
            canBet: state.AvailableActions.CanBet,
            canCall: state.AvailableActions.CanCall,
            canCheck: state.AvailableActions.CanCheck,
            canFold: state.AvailableActions.CanFold,
            canRaise: state.AvailableActions.CanRaise,
            maxBet: state.AvailableActions.MaxBet,
            minBet: state.AvailableActions.MinBet,
            minRaise: state.AvailableActions.MinRaise
            );

            var wasPlayerTurn = _isPlayerTurn;
            _isPlayerTurn = state.IsMyTurn;
            if (state.IsMyTurn)
            {
                _legalActions = MapLegalActionsFromPrivateState(state.AvailableActions);
                _currentMinBet = Math.Max(_gameResponse?.MinBet ?? 0, state.AvailableActions.MinBet);
                _currentMaxBet = state.AvailableActions.MaxBet;
                _currentBetToCall = state.AvailableActions.CallAmount;

                if (!wasPlayerTurn)
                {
                    await PlayTurnAlertSoundAsync();
                }
            }
        }
        else
        {
            // Special non-betting decision phases (like Screw Your Neighbor KeepOrTrade)
            // do not populate AvailableActions, but still require turn-driven overlays.
            var wasPlayerTurn = _isPlayerTurn;
            _isPlayerTurn = state.IsMyTurn && (_isDrawPhase || IsKeepOrTradePhase || IsInBetweenTurnPhase);
            if (_isPlayerTurn && !wasPlayerTurn)
            {
                await PlayTurnAlertSoundAsync();
            }

            // Clear all In-Between UI flags when it's no longer this player's turn
            // so stale state (e.g. from a timer auto-pass) doesn't bleed into the next turn.
            if (!_isPlayerTurn && IsInBetween)
            {
                _ibWaitingForReveal = false;
                _ibWaitingForBoundaryCards = false;
                _ibShowBetControls = false;
                _ibAceChoiceRequired = false;
                _ibShowDealPrompt = false;
                _ibLastTurnDescription = null;
            }

            // Auto-deal boundary cards when it becomes this player's InBetween turn
            // and no action panel is already showing.
            if (_isPlayerTurn && IsInBetween && !_isProcessingInBetween
                && !_ibShowBetControls && !_ibAceChoiceRequired && !_ibShowDealPrompt
                && !_ibWaitingForReveal && !_ibWaitingForBoundaryCards)
            {
                _ibLastTurnDescription = null;
                await HandleInBetweenDealCardsAsync();
            }
            if (!_isDrawPhase)
            {
                _legalActions = [];
            }
        }

        // Update draw phase state
        if (state.Draw is not null)
        {
            _isPlayerDrawTurn = state.Draw.IsMyTurnToDraw;
            _maxDiscards = state.Draw.MaxDiscards;

            // Sync local drawing state with server truth, but preserve local animation state
            if (!_isDrawAnimating && !_isSubmittingDraw)
            {
                _cardsDrawn = state.Draw.HasDrawnThisRound;
            }

            if (state.Draw.IsMyTurnToDraw && !_isDrawAnimating && !_forceShowDrawPanel)
            {
                UpdateCurrentPlayerDrawCardsFromPrivateState(state.Hand, preserveSelection: true);
            }
            else if ((_isDrawAnimating || _forceShowDrawPanel) && _currentPlayerDrawCards.Count == 0)
            {
                // Preserve whatever is currently being animated. If we lost the list for any reason,
                // fall back to the latest private hand state to keep the panel populated.
                UpdateCurrentPlayerDrawCardsFromPrivateState(state.Hand, preserveSelection: true);
            }
        }
        else if (IsDrawingPhase)
        {
            // Sync with general turn flag in drawing phase if draw-specific data is missing (common for sitting out players)
            _isPlayerDrawTurn = state.IsMyTurn;

            // Ensure DrawPanel has cards if it's the player's turn to draw but state.Draw was null
            if (_isPlayerDrawTurn && !_isDrawAnimating && !_forceShowDrawPanel && _currentPlayerDrawCards.Count == 0 && state.Hand
            is not null)
            {
                UpdateCurrentPlayerDrawCardsFromPrivateState(state.Hand, preserveSelection: true);
            }
        }

        // Update Drop or Stay phase state (Kings and Lows)
        if (state.DropOrStay is not null)
        {
            _hasDropOrStayDecided = state.DropOrStay.HasDecidedThisRound;
            _dropOrStayDecision = state.DropOrStay.Decision;
        }

        _buyCardOffer = state.BuyCardOffer;
        _tollboothOffer = state.TollboothOffer;

        // Populate cards for display in DropOrStayOverlay
        // Check both the property AND the phase string directly from state to handle reconnection scenarios
        // where _gameResponse might not be populated yet
        var currentPhase = _gameResponse?.CurrentPhase ?? string.Empty;
        if ((IsDropOrStayPhase || string.Equals(currentPhase, "DropOrStay", StringComparison.OrdinalIgnoreCase))
        && state.Hand is not null && state.Hand.Count > 0)
        {
            UpdateCurrentPlayerDrawCardsFromPrivateState(state.Hand, preserveSelection: false);
        }
        // Populate cards for KeepOrTrade overlay (Screw Your Neighbor)
        else if ((IsKeepOrTradePhase || string.Equals(currentPhase, "KeepOrTrade", StringComparison.OrdinalIgnoreCase))
        && state.Hand is not null && state.Hand.Count > 0)
        {
            UpdateCurrentPlayerDrawCardsFromPrivateState(state.Hand, preserveSelection: false);
        }
        // Also populate cards for PlayerVsDeck phase when reconnection
        // This ensures the player can see the deck cards after reconnection
        else if ((IsPlayerVsDeckPhase || string.Equals(currentPhase, "PlayerVsDeck", StringComparison.OrdinalIgnoreCase))
        && state.Hand is not null && state.Hand.Count > 0 && _currentPlayerDrawCards.Count == 0)
        {
            // Populate player's own cards for reference during player-vs-deck phase
            UpdateCurrentPlayerDrawCardsFromPrivateState(state.Hand, preserveSelection: false);
        }
        // Populate cards for DrawComplete phase so all players can see their new hands
        else if ((IsDrawCompletePhase || string.Equals(currentPhase, "DrawComplete", StringComparison.OrdinalIgnoreCase))
        && state.Hand is not null && state.Hand.Count > 0)
        {
            // Show the player's new hand during DrawComplete phase
            UpdateCurrentPlayerDrawCardsFromPrivateState(state.Hand, preserveSelection: false);
        }

        // Calculate hand odds from current cards
        CalculateHandOdds(state.Hand);

        // Update hand history from private state (contains personalized player results)
        if (state.HandHistory is not null && state.HandHistory.Count > 0)
        {
            _handHistoryEntries = state.HandHistory.ToList();
        }

        // Update chip history from private state
        if (state.ChipHistory is not null)
        {
            _chipHistory = state.ChipHistory;
        }

        ApplyPendingCurrentPlayerDrawCardsIfReady();

        await InvokeStateHasChangedAsync();
    }

    private void UpdateSeatsFromPublicState(IReadOnlyList<SeatPublicDto> publicSeats)
    {
        Logger.LogDebug("UpdateSeatsFromPublicState: Processing {SeatCount} seats, runoutAnimationInProgress={RunoutInProgress}",
        publicSeats.Count, _runoutAnimationInProgress);

        var updatedSeatIndices = new HashSet<int>();

        foreach (var publicSeat in publicSeats)
        {
            if (publicSeat.SeatIndex >= 0 && publicSeat.SeatIndex < _seats.Count)
            {
                updatedSeatIndices.Add(publicSeat.SeatIndex);

                // Check if this is the current player by multiple criteria:
                // 1. Email match (primary)
                // 2. Seat index match (for when Player.Name differs from email)
                // 3. Player name match against currentPlayerName (set from private state)
                var isCurrentPlayerByEmail = !string.IsNullOrWhiteSpace(_loggedInUserEmail) &&
                string.Equals(publicSeat.PlayerName, _loggedInUserEmail, StringComparison.OrdinalIgnoreCase);
                var isCurrentPlayerBySeatIndex = _currentPlayerSeatIndex >= 0 &&
                publicSeat.SeatIndex == _currentPlayerSeatIndex;
                var isCurrentPlayerByName = !string.IsNullOrWhiteSpace(_currentPlayerName) &&
                string.Equals(publicSeat.PlayerName, _currentPlayerName, StringComparison.OrdinalIgnoreCase);
                var isCurrentPlayer = isCurrentPlayerByEmail || isCurrentPlayerBySeatIndex || isCurrentPlayerByName;

                // Check if this seat is part of the runout animation
                var isInRunoutAnimation = _runoutAnimationInProgress &&
                _allInRunoutState?.RunoutCardsBySeat?.ContainsKey(publicSeat.SeatIndex) == true;

                // Preserve face-up cards for current player (will be updated by private state)
                // This prevents briefly showing face-down cards before private state arrives
                // BUT: Don't preserve cards if the player is sitting out (they shouldn't have cards)
                // ALSO: Preserve cards during runout animation - the animation controls card visibility
                // ALSO: Preserve cards during initial deal animation - the animation controls card visibility
                var existingCards = _seats[publicSeat.SeatIndex].Cards;
                var shouldPreserveCards = (isCurrentPlayer && existingCards.Any(c => c.IsFaceUp || (!string.IsNullOrWhiteSpace(c.Rank) && !string.IsNullOrWhiteSpace(c.Suit))) && !publicSeat.IsSittingOut)
                || isInRunoutAnimation
                || _dealAnimationInProgress;

                Logger.LogDebug(
                "UpdateSeatsFromPublicState: Seat {SeatIndex}, PlayerName={PlayerName}, " +
                "isCurrentPlayerByEmail={ByEmail}, isCurrentPlayerBySeatIndex={BySeat}, " +
                "isCurrentPlayerByName={ByName}, existingCards={ExistingCount}, " +
                "existingFaceUp={FaceUpCount}, shouldPreserve={ShouldPreserve}, publicCardCount={PublicCount}, isSittingOut={IsSittingOut}, isInRunout={IsInRunout}, isDealAnim={IsDealAnim}",
                publicSeat.SeatIndex, publicSeat.PlayerName,
                isCurrentPlayerByEmail, isCurrentPlayerBySeatIndex, isCurrentPlayerByName,
                existingCards.Count, existingCards.Count(c => c.IsFaceUp),
                shouldPreserveCards, publicSeat.Cards.Count, publicSeat.IsSittingOut, isInRunoutAnimation, _dealAnimationInProgress);

                List<CardInfo> cards;
                if (shouldPreserveCards)
                {
                    cards = existingCards.Select(c => c with
                    {
                        IsWild = IsWildCard(c.Rank, c.Suit),
                        IsShowcaseCard = c.IsShowcaseCard
                    }).ToList();

                    var shouldAppendMissingPublicCards =
                        isCurrentPlayer &&
                        !_dealAnimationInProgress &&
                        !isInRunoutAnimation &&
                        publicSeat.Cards.Count > cards.Count;

                    if (shouldAppendMissingPublicCards)
                    {
                        foreach (var publicCard in publicSeat.Cards.OrderBy(c => c.DealOrder))
                        {
                            if (cards.Any(existing => existing.DealOrder == publicCard.DealOrder))
                            {
                                continue;
                            }

                            cards.Add(new CardInfo
                            {
                                Rank = publicCard.IsFaceUp ? publicCard.Rank : null,
                                Suit = publicCard.IsFaceUp ? publicCard.Suit : null,
                                IsFaceUp = publicCard.IsFaceUp,
                                IsPubliclyVisible = publicCard.IsFaceUp,
                                IsSelected = false,
                                DealOrder = publicCard.DealOrder,
                                IsWild = publicCard.IsFaceUp && IsWildCard(publicCard.Rank, publicCard.Suit),
                                IsShowcaseCard = existingCards.FirstOrDefault(existing => existing.DealOrder == publicCard.DealOrder)?.IsShowcaseCard == true
                            });
                        }

                        cards = cards.OrderBy(c => c.DealOrder).ToList();
                    }
                }
                else
                {
                    cards = publicSeat.Cards.Select(c =>
                    {
                        var existingCard = isCurrentPlayer
                            ? existingCards.FirstOrDefault(ec => ec.DealOrder == c.DealOrder)
                            : null;

                        var rank = c.IsFaceUp ? c.Rank : existingCard?.Rank;
                        var suit = c.IsFaceUp ? c.Suit : existingCard?.Suit;

                        return new CardInfo
                        {
                            Rank = rank,
                            Suit = suit,
                            IsFaceUp = c.IsFaceUp,
                            IsPubliclyVisible = c.IsFaceUp,
                            IsSelected = false,
                            DealOrder = c.DealOrder,
                          IsWild = IsWildCard(rank, suit),
                            IsShowcaseCard = existingCard?.IsShowcaseCard == true
                        };
                    }).ToList();
                }

                ApplyKingsAndLowsLowestCardWildFallback(cards);

                var formattedPublicHandEvaluationDescription =
                    FormatHandDescriptionForDisplay(publicSeat.HandEvaluationDescription);

                // Preserve LastActionDescription if it hasn't expired (5 seconds)
                var existingSeat = _seats[publicSeat.SeatIndex];

                // For Kings and Lows, show "Dropped" status for folded players throughout the hand
                string? lastActionDescription = null;
                DateTimeOffset? lastActionTime = null;

                if (IsKingsAndLows && publicSeat.IsFolded)
                {
                    lastActionDescription = "Dropped";
                    lastActionTime = existingSeat.LastActionTime ?? DateTimeOffset.UtcNow;
                }
                else if (_isPausedForChipCheck && IsPlayerShortOnChips(publicSeat.SeatIndex, publicSeat.Chips))
                {
                    lastActionDescription = _isRebuyGracePause ? "Rebuy needed" : "Waiting for chips";
                    lastActionTime = DateTimeOffset.UtcNow;
                }
                else
                {
                    // For other games, preserve the action description for 5 seconds
                    var shouldPreserveAction = !string.IsNullOrWhiteSpace(existingSeat.LastActionDescription) &&
                    existingSeat.LastActionTime.HasValue &&
                    (DateTimeOffset.UtcNow - existingSeat.LastActionTime.Value).TotalSeconds < 5;

                    lastActionDescription = shouldPreserveAction ? existingSeat.LastActionDescription : null;
                    lastActionTime = shouldPreserveAction ? existingSeat.LastActionTime : null;
                }

                var screwYourNeighborDecision = IsScrewYourNeighbor
                    ? formattedPublicHandEvaluationDescription
                        ?? GetScrewYourNeighborDecisionDescription(lastActionDescription)
                        ?? GetScrewYourNeighborDecisionDescription(publicSeat.LastAction?.ActionDescription)
                    : null;

                var resolvedHandEvaluationDescription = IsScrewYourNeighbor
                    ? (isCurrentPlayer
                        ? _synKeepOrTradeDecision
                        : null)
                        ?? existingSeat.HandEvaluationDescription
                        ?? screwYourNeighborDecision
                    : formattedPublicHandEvaluationDescription
                        ?? existingSeat.HandEvaluationDescription;

                if (IsScrewYourNeighbor &&
                    !string.Equals(existingSeat.HandEvaluationDescription, "Traded", StringComparison.OrdinalIgnoreCase) &&
                    string.Equals(resolvedHandEvaluationDescription, "Traded", StringComparison.OrdinalIgnoreCase))
                {
                    QueueScrewYourNeighborTradeAnimation(publicSeat.SeatIndex);
                }

                _seats[publicSeat.SeatIndex] = new SeatInfo
                {
                    SeatIndex = publicSeat.SeatIndex,
                    IsOccupied = publicSeat.IsOccupied,
                    PlayerName = publicSeat.PlayerName,
                    PlayerAvatarUrl = publicSeat.PlayerAvatarUrl,
                    PlayerFirstName = publicSeat.PlayerFirstName,
                    Chips = publicSeat.Chips,
                    IsReady = publicSeat.IsReady,
                    IsCurrentPlayer = isCurrentPlayer,
                    IsFolded = publicSeat.IsFolded,
                    IsAllIn = publicSeat.IsAllIn,
                    IsDisconnected = publicSeat.IsDisconnected,
                    IsSittingOut = publicSeat.IsSittingOut,
                    SittingOutReason = publicSeat.SittingOutReason,
                    CurrentBet = publicSeat.CurrentBet,
                    HasDecidedDropOrStay = publicSeat.HasDecidedDropOrStay,
                    Cards = cards,
                    LastActionDescription = lastActionDescription,
                    LastActionTime = lastActionTime,
                    HandEvaluationDescription = resolvedHandEvaluationDescription
                };
            }
        }

        // Clear seats that were occupied but are missing from the update (player left)
        for (var i = 0; i < _seats.Count; i++)
        {
            if (_seats[i].IsOccupied && !updatedSeatIndices.Contains(i))
            {
                _seats[i] = new SeatInfo { SeatIndex = i };
            }
        }
    }

    private static bool HasActiveAllInRunout(TableStatePublicDto state)
    {
        return state.AllInRunout is { IsActive: true };
    }

    private void UpdateShowdownFromPublicState(ShowdownPublicDto showdown, bool delayShowingOverlay = false)
    {
        // Build showdown result from public state ONLY when showdown.IsComplete is true.
        // This means the server has already performed the showdown, set NextHandStartsAt,
        // and transitioned to Complete phase.
        //
        // CRITICAL: Do NOT populate _showdownResult when IsShowdownPhase is true but IsComplete is false!
        // Non-inline showdown variants still rely on the showdown API to finalize results.
        //
        // For Kings and Lows and PotMatching phase, we also allow processing as those have special flows.
        // IMPORTANT: If we're delaying the overlay (e.g., for animation), don't set _showdownResult
        // so that TryLoadShowdownAsync can fetch complete data from the API later
        // Also skip if we're in the middle of processing a deck draw - we want the API data instead
        if (delayShowingOverlay || _isProcessingDeckDraw)
        {
            return;
        }

        // Only process showdown from SignalR when:
        // 1. We don't already have a result
        // 2. AND (the showdown is complete OR we're in PotMatching phase for Kings and Lows)
        // Do NOT process when IsShowdownPhase is true but IsComplete is false - let TryLoadShowdownAsync call the API
        var hasWinnerInPublicResults = showdown.PlayerResults.Any(r => r.IsWinner || r.AmountWon > 0);

        var shouldProcessShowdown = _showdownResult is null &&
        (showdown.IsComplete || IsPotMatchingPhase) &&
        hasWinnerInPublicResults;

        if (shouldProcessShowdown)
        {
            var payouts = showdown.PlayerResults
            .Where(r => r.IsWinner)
            .ToDictionary(r => r.PlayerName, r => r.AmountWon);

            var highHandPayouts = showdown.PlayerResults
            .Where(r => r.IsHighHandWinner && r.HighHandAmountWon > 0)
            .ToDictionary(r => r.PlayerName, r => r.HighHandAmountWon);

            var highHandWinners = showdown.PlayerResults
            .Where(r => r.IsHighHandWinner && r.HighHandAmountWon > 0)
            .Select(r => r.PlayerName)
            .ToList();

            var sevensPayouts = showdown.PlayerResults
            .Where(r => r.IsSevensWinner && r.SevensAmountWon > 0)
            .ToDictionary(r => r.PlayerName, r => r.SevensAmountWon);

            var sevensWinners = showdown.PlayerResults
            .Where(r => r.IsSevensWinner && r.SevensAmountWon > 0)
            .Select(r => r.PlayerName)
            .ToList();

            var playerHands = showdown.PlayerResults
            .Select(r => new ShowdownPlayerHand(
            amountWon: r.AmountWon,
            cards: ConvertCardsToShowdownCards(r.Cards),
            handDescription: r.HandDescription ?? "",
            handStrength: null,
            handType: r.HandRanking ?? "",
            highHandAmountWon: r.HighHandAmountWon,
            isHighHandWinner: r.IsHighHandWinner,
            isSevensWinner: r.IsSevensWinner,
            isWinner: r.IsWinner,
            playerFirstName: r.PlayerFirstName,
            playerName: r.PlayerName,
            sevensAmountWon: r.SevensAmountWon,
            wildCardIndexes: r.WildCardIndexes?.ToList() ?? [])
            {
                BestCardIndexes = r.BestCardIndexes?.ToList()
            })
            .ToList();

            var publicShowdownResult = new PerformShowdownSuccessful(
            currentPhase: "Complete",
            gameId: GameId,
            payouts: payouts,
            highHandWinners: highHandWinners,
            sevensPoolRolledOver: showdown.SevensPoolRolledOver,
            highHandPayouts: highHandPayouts,
            playerHands: playerHands,
            sevensPayouts: sevensPayouts,
            wonByFold: false,
            sevensWinners: sevensWinners);

            if (!HasUsableShowdownResult(publicShowdownResult))
            {
                return;
            }

            _showdownResult = publicShowdownResult;

            // Only show overlay immediately if we're not delaying
            if (!delayShowingOverlay)
            {
                _showShowdownOverlay = ShouldDisplayShowdownOverlay;

                _isResultsPhase = true;
                RefreshCountdownFromCurrentTableState();

                if (_countdownDeadlineUtc.HasValue)
                {
                    StartCountdownTimer();
                }
            }

            _showdownWon = _showdownResult.Payouts?.Keys.Any(p =>
            string.Equals(p, _loggedInUserEmail, StringComparison.OrdinalIgnoreCase) ||
            string.Equals(p, _currentPlayerName, StringComparison.OrdinalIgnoreCase)) == true
            || highHandWinners.Any(p =>
            string.Equals(p, _loggedInUserEmail, StringComparison.OrdinalIgnoreCase) ||
            string.Equals(p, _currentPlayerName, StringComparison.OrdinalIgnoreCase));

            _showdownLost = !_showdownWon;
        }
    }

    private static ICollection<ShowdownCard> ConvertCardsToShowdownCards(IReadOnlyList<CardPublicDto>? cards)
    {
        if (cards is null || cards.Count == 0)
        {
            return [];
        }

        return cards
        .Where(c => c.IsFaceUp && !string.IsNullOrWhiteSpace(c.Rank) && !string.IsNullOrWhiteSpace(c.Suit))
        .Select(c => new ShowdownCard(
        suit: ParseCardSuit(c.Suit),
        symbol: ParseCardSymbol(c.Rank)
        ))
        .ToList();
    }

    private static bool HasUsableShowdownResult(PerformShowdownSuccessful? result)
    {
        if (result is null)
        {
            return false;
        }

        var hasWinnerFromHands = result.PlayerHands?.Any(h =>
            !string.IsNullOrWhiteSpace(h.PlayerName) &&
            (h.IsWinner == true || h.AmountWon.GetValueOrDefault() > 0)) == true;

        var hasWinnerFromPayouts = result.Payouts?.Any(kvp =>
            !string.IsNullOrWhiteSpace(kvp.Key) && kvp.Value > 0) == true;

        var hasAnyCards = result.PlayerHands?.Any(h => h.Cards is { Count: > 0 }) == true;

        var hasWinnerIdentity = hasWinnerFromHands || hasWinnerFromPayouts;

        if (result.WonByFold == true)
        {
            return hasWinnerIdentity;
        }

        return hasWinnerIdentity && hasAnyCards;
    }

    private static CardSuit? ParseCardSuit(string? suit)
    {
        if (string.IsNullOrWhiteSpace(suit))
        {
            return null;
        }

        return suit.Trim().ToLowerInvariant() switch
        {
            "hearts" or "h" => CardSuit.Hearts,
            "diamonds" or "d" => CardSuit.Diamonds,
            "spades" or "s" => CardSuit.Spades,
            "clubs" or "c" => CardSuit.Clubs,
            _ => null
        };
    }

    private static CardSymbol? ParseCardSymbol(string? rank)
    {
        if (string.IsNullOrWhiteSpace(rank))
        {
            return null;
        }

        return rank.Trim().ToUpperInvariant() switch
        {
            "2" or "TWO" or "DEUCE" => CardSymbol.Deuce,
            "3" or "THREE" => CardSymbol.Three,
            "4" or "FOUR" => CardSymbol.Four,
            "5" or "FIVE" => CardSymbol.Five,
            "6" or "SIX" => CardSymbol.Six,
            "7" or "SEVEN" => CardSymbol.Seven,
            "8" or "EIGHT" => CardSymbol.Eight,
            "9" or "NINE" => CardSymbol.Nine,
            "10" or "T" or "TEN" => CardSymbol.Ten,
            "J" or "JACK" => CardSymbol.Jack,
            "Q" or "QUEEN" => CardSymbol.Queen,
            "K" or "KING" => CardSymbol.King,
            "A" or "ACE" => CardSymbol.Ace,
            _ => null
        };
    }

    private static List<BettingActionType> MapLegalActionsFromPrivateState(AvailableActionsDto available)
    {
        var actions = new List<BettingActionType>();

        if (available.CanFold) actions.Add(BettingActionType.Fold);
        if (available.CanCheck) actions.Add(BettingActionType.Check);
        if (available.CanCall) actions.Add(BettingActionType.Call);
        if (available.CanBet) actions.Add(BettingActionType.Bet);
        if (available.CanRaise) actions.Add(BettingActionType.Raise);
        if (available.CanAllIn) actions.Add(BettingActionType.AllIn);

        return actions;
    }

    private async Task<string?> GetLoggedInUserEmailAsync()
    {
        var authState = await AuthenticationStateProvider.GetAuthenticationStateAsync();
        var user = authState.User;

        if (user.Identity?.IsAuthenticated != true)
        {
            return null;
        }

        // Common claim locations across providers.
        return user.FindFirstValue(ClaimTypes.Email)
        ?? user.FindFirstValue("email")
        ?? user.FindFirstValue("preferred_username")
        ?? user.Identity?.Name;
    }

    private void InitializeSeats()
    {
        _seats = Enumerable.Range(0, 8)
        .Select(i => new SeatInfo { SeatIndex = i })
        .ToList();
    }

    private GetGameResponse? _gameResponse;
    private IReadOnlyList<string>? _allowedDealerChoiceGameCodes;
    private async Task LoadDataAsync()
    {
        try
        {
            _loggedInUserEmail ??= await GetLoggedInUserEmailAsync();

            // Use the generic Games API to load game data - it returns the game type code
            var gameResponse = await GamesApiClient.GetGameAsync(GameId);
            if (gameResponse is { IsSuccessStatusCode: true, Content: not null })
            {
                var gamesResponse = gameResponse.Content;
                _gameTypeCode = gamesResponse.GameTypeCode;
                _leagueId = gamesResponse.LeagueId;

                _gameResponse = CreateLocalGameResponse(gamesResponse);
                _allowedDealerChoiceGameCodes = ParseAllowedDealerChoiceGameCodes(gamesResponse.GameSettings);

                _currentActorSeatIndex = _gameResponse.CurrentPlayerIndex;
                _dealerSeatIndex = _gameResponse.DealerPosition;
                _ante = _gameResponse.Ante ?? _ante;
				InitializeJoinBuyInFromTable();

                // Capture Dealer's Choice fields
                _isDealersChoice = gamesResponse.IsDealersChoice;
                _dealersChoiceDealerPosition = gamesResponse.DealersChoiceDealerPosition;

                // Pre-load available games for DC tables
                if (_isDealersChoice && _dcAvailableGames.Count == 0)
                {
                    _ = LoadAvailableGamesAsync();
                }
            }

            await GetPlayersAndUpdateSeatsAsync();

            // Load game rules for dynamic UI rendering
            if (!string.IsNullOrWhiteSpace(_gameTypeCode))
            {
                _gameRules = await GameRulesService.GetGameRulesAsync(_gameTypeCode);
            }

            // Hand history is provided via SignalR in the initial state and updates

            if (IsShowdownPhase)
            {
                await TryLoadShowdownAsync();
            }

            // SignalR will handle state updates - no polling needed
            // Initial state snapshot is sent when joining the game via SignalR
        }
        finally
        {
            _seatedPlayerCount = _seats.Count(s => s.IsOccupied);
            var activePlayers = _seats.Where(s => s.IsOccupied && !s.IsSittingOut);
            _canStartGame = activePlayers.Count() >= (_gameResponse?.MinimumNumberOfPlayers ?? 2) &&
            activePlayers.All(s => s.IsReady);

            // Determine if current user is the host (game creator)
            _isHost = !string.IsNullOrWhiteSpace(_loggedInUserEmail) &&
            !string.IsNullOrWhiteSpace(_gameResponse?.CreatedByName) &&
            string.Equals(_loggedInUserEmail, _gameResponse.CreatedByName, StringComparison.OrdinalIgnoreCase);

            // Determine current player seat by matching logged-in email to seat PlayerName (case-insensitive).
            _currentPlayerName ??= _loggedInUserEmail;
            _currentPlayerSeatIndex = _seats.FirstOrDefault(s => s.IsOccupied &&
            !string.IsNullOrWhiteSpace(_currentPlayerName) &&
            string.Equals(s.PlayerName, _currentPlayerName, StringComparison.OrdinalIgnoreCase))?.SeatIndex ?? -1;

            _isSeated = _currentPlayerSeatIndex >= 0;
            _isReady = _isSeated && _seats[_currentPlayerSeatIndex].IsReady;
            _isLoading = false;
            await InvokeStateHasChangedAsync();
        }
    }

    /// <summary>
    /// Applies the centralized <see cref="TableRefreshPolicy"/> for a given interaction.
    /// This is the single place the page turns a refresh decision into a concrete action,
    /// keeping post-action behavior consistent and avoiding refetches that would only
    /// duplicate hub broadcasts. The full policy is documented in <c>docs/TableRefreshPolicy.md</c>.
    /// </summary>
    private async Task ApplyRefreshPolicyAsync(TableInteraction interaction)
    {
        var kind = TableRefreshPolicy.ResolveRefreshKind(interaction);
        switch (kind)
        {
            case TableRefreshKind.Full:
                await LoadDataAsync();
                break;
            case TableRefreshKind.Slice:
                await ApplySliceRefreshAsync(interaction);
                break;
            case TableRefreshKind.HubDriven:
            case TableRefreshKind.LocalOnly:
            default:
                // Nothing to fetch: an authoritative hub broadcast or a local transition
                // already covers this interaction, so an immediate refetch would be wasteful.
                Logger.LogDebug(
                    "Refresh policy for {Interaction} is {Kind}; skipping direct refetch",
                    interaction,
                    kind);
                break;
        }
    }

    private Task ApplySliceRefreshAsync(TableInteraction interaction) => interaction switch
    {
        TableInteraction.TableSettingsPush => RefreshOddsVisibilityFromServerAsync(),
        _ => Task.CompletedTask,
    };

    private static GetGameResponse CreateLocalGameResponse(GetGameResponse gamesResponse)
    {
        return new GetGameResponse(
            ante: gamesResponse.Ante,
            bigBet: gamesResponse.BigBet,
            bigBlind: gamesResponse.BigBlind,
            bringIn: gamesResponse.BringIn,
            bringInPlayerIndex: gamesResponse.BringInPlayerIndex,
            canContinue: gamesResponse.CanContinue,
            createdAt: gamesResponse.CreatedAt,
            createdById: gamesResponse.CreatedById,
            createdByName: gamesResponse.CreatedByName,
            currentDrawPlayerIndex: gamesResponse.CurrentDrawPlayerIndex,
            currentHandNumber: gamesResponse.CurrentHandNumber,
            currentPhase: gamesResponse.CurrentPhase,
            currentPhaseDescription: gamesResponse.CurrentPhaseDescription,
            currentPlayerIndex: gamesResponse.CurrentPlayerIndex,
            dealerPosition: gamesResponse.DealerPosition,
            endedAt: gamesResponse.EndedAt,
            gameSettings: gamesResponse.GameSettings,
            gameTypeCode: gamesResponse.GameTypeCode,
            gameTypeId: gamesResponse.GameTypeId,
            gameTypeName: gamesResponse.GameTypeName,
            id: gamesResponse.Id,
            maximumNumberOfPlayers: gamesResponse.MaximumNumberOfPlayers,
            minBet: gamesResponse.MinBet,
            minimumNumberOfPlayers: gamesResponse.MinimumNumberOfPlayers,
            name: gamesResponse.Name,
            randomSeed: gamesResponse.RandomSeed,
            rowVersion: gamesResponse.RowVersion,
            smallBet: gamesResponse.SmallBet,
            smallBlind: gamesResponse.SmallBlind,
            startedAt: gamesResponse.StartedAt,
            status: gamesResponse.Status,
            updatedAt: gamesResponse.UpdatedAt)
        {
            AreOddsVisibleToAllPlayers = gamesResponse.AreOddsVisibleToAllPlayers,
            LeagueId = gamesResponse.LeagueId,
            MaxBuyIn = gamesResponse.MaxBuyIn,
            RequiresJoinApproval = gamesResponse.RequiresJoinApproval,
            TournamentBuyIn = gamesResponse.TournamentBuyIn
        };
    }

    private void NavigateToExitDestination()
    {
        NavigationManager.NavigateTo(GetExitDestination(_leagueId));
    }

    private static string GetExitDestination(Guid? leagueId)
    {
        return leagueId.HasValue
            ? $"/leagues/{leagueId.Value}"
            : "/lobby";
    }

    private async Task GetPlayersAndUpdateSeatsAsync()
    {
        var playersResponse = await GamesApiClient.GetGamePlayersAsync(GameId);

        if (playersResponse is { IsSuccessStatusCode: true, Content: not null })
        {
            var players = playersResponse.Content.ToList();
            // Reset seats to ensure clean state
            InitializeSeats();

            foreach (var player in players)
            {
                if (player.SeatPosition < 0 || player.SeatPosition >= _seats.Count)
                {
                    continue;
                }

                var currentUserMatch = _currentPlayerName ?? _loggedInUserEmail;
                var isCurrentPlayer = !string.IsNullOrWhiteSpace(currentUserMatch) &&
                string.Equals(player.PlayerName, currentUserMatch, StringComparison.OrdinalIgnoreCase);

                // Capture the current player's ID for API calls
                if (isCurrentPlayer)
                {
                    _currentPlayerId = player.PlayerId;
                }

                _seats[player.SeatPosition] = new SeatInfo
                {
                    SeatIndex = player.SeatPosition,
                    IsOccupied = true,
                    PlayerName = player.PlayerName,
                    PlayerAvatarUrl = player.PlayerAvatarUrl,
                    PlayerFirstName = player.PlayerFirstName,
                    Chips = player.StartingChips,
                    IsReady = player.Status == GamePlayerStatus.Active,
                    IsCurrentPlayer = isCurrentPlayer,
                    IsFolded = player.HasFolded,
                    IsSittingOut = player.IsSittingOut,
                    SittingOutReason = player.IsSittingOut && player.HasFolded ? "Observing" : (player.IsSittingOut ? "Sitting out" : null),
                    Cards = player.Hand.OrderBy(c => c.DealOrder).Select(c => new CardInfo
                    {
                        Rank = c.Symbol.ToCardRankString(),
                        Suit = c.Suit.ToCardSuitString(),
                        IsFaceUp = isCurrentPlayer,
                        IsSelected = false,
                        IsWild = isCurrentPlayer && IsWildCard(c.Symbol.ToCardRankString(), c.Suit.ToCardSuitString())
                    }).ToList()
                };
            }

            _currentPlayerSeatIndex = _seats.FirstOrDefault(s => s.IsCurrentPlayer)?.SeatIndex ?? _currentPlayerSeatIndex;
        }
        else
        {
            // Handle error (e.g., show message)
        }
    }

    private bool _isLeavingTable = false;
    private bool _showNoChipsModal;

    private void HandleNoChipsModalClose()
    {
        _showNoChipsModal = false;
    }

    private void CloseJoinBuyInModal()
    {
        _showJoinBuyInModal = false;
        _pendingJoinSeatIndex = null;
        _joinBuyInBalance = 0;
        _joinBuyInBalanceCap = null;
    }

    private void ShowJoinApprovalPendingModal(Guid? joinRequestId, string? hostName)
    {
        _showJoinApprovalPendingModal = true;
        _pendingJoinRequestId = joinRequestId;
        _joinApprovalHostName = string.IsNullOrWhiteSpace(hostName) ? "the host" : hostName;
        _pendingApprovalBuyIn = _joinBuyInAmount;
    }

    private void CloseJoinApprovalPendingModal()
    {
        _showJoinApprovalPendingModal = false;
        _pendingJoinRequestId = null;
        _joinApprovalHostName = null;
        _pendingApprovalBuyIn = 0;
    }

    private async Task ToggleSitOutAsync()
    {
        var newState = !IsSittingOut;
        await TableActions.ExecuteAsync<ToggleSitOutSuccessful>(
            async () =>
            {
                var response = await GamesApiClient.ToggleSitOutAsync(
                    GameId,
                    new ToggleSitOutRequest { IsSittingOut = newState });
                return TableActionResult<ToggleSitOutSuccessful>.From(response);
            },
            onSuccess: result => result.Content is not null
                ? ShowToastAsync(result.Content.Message, "info")
                : Task.CompletedTask,
            new TableActionOptions
            {
                ActionName = "toggle sit out",
                ShowFailureToast = false,
                ShowExceptionToast = true,
                FailureMessage = "Failed to update status",
            });
    }

    private async Task LeaveTableAsync()
    {
        if (_isLeavingTable)
            return; // Prevent double-clicks

        _isLeavingTable = true;

        try
        {
            // Call the new Leave Game API endpoint
            var response = await GamesApiClient.LeaveGameAsync(GameId);

            if (response.IsSuccessStatusCode && response.Content != null)
            {
                var result = response.Content;

                if (!result.Immediate)
                {
                    // Player is in active hand - show toast message
                    _hasRequestedLeave = true;
                    await ShowToastAsync(
                    result.Message ?? "You will leave after the current hand completes.",
                    "info",
                    durationMs: 6000);

                    // Stay on page until hand completes
                    // Background service will handle actual removal
                    return;
                }

                // Immediate leave - disconnect from SignalR and navigate to lobby
                await GameHubClient.LeaveGameAsync(GameId);
                NavigateToExitDestination();
            }
            else
            {
                // If not seated or already left, just navigate to lobby
                await GameHubClient.LeaveGameAsync(GameId);
                NavigateToExitDestination();
            }
        }
        catch (Exception ex)
        {
            Logger.LogError(ex, "Error leaving table {GameId}", GameId);
            await ShowToastAsync("An error occurred while leaving the table.", "error");
        }
        finally
        {
            _isLeavingTable = false;
        }
    }

    private async Task HandleSeatClickAsync(int seatIndex)
    {
        await BeginAutoSeatJoinAsync();
    }

    private bool TryGetAutoJoinSeatIndex(out int seatIndex)
    {
        seatIndex = _seats
            .Where(s => !s.IsOccupied)
            .OrderBy(s => s.SeatIndex)
            .Select(s => s.SeatIndex)
            .FirstOrDefault(-1);

        return seatIndex >= 0;
    }

    private async Task BeginAutoSeatJoinAsync()
    {
        if (_isSeated)
        {
            return;
        }

        if (IsStartedScrewYourNeighborTable)
        {
            await ShowToastAsync("Screw Your Neighbor does not allow players to join after the game has started.", "error");
            return;
        }

        if (!TryGetAutoJoinSeatIndex(out var autoSeatIndex))
        {
            await ShowToastAsync("No seats are currently available.", "error");
            return;
        }

        try
        {
            var cashierSummary = await ProfileApi.GetCashierSummaryAsync();
            if (!cashierSummary.IsSuccessStatusCode || cashierSummary.Content is null)
            {
                await ShowToastAsync("Unable to load your cashier balance right now.", "error");
                return;
            }

            var currentBalance = cashierSummary.Content.CurrentBalance;
            if (currentBalance <= 0)
            {
                // Attempt rejoin: player may have unreturned chips from a previous session at this table.
                // The server will reactivate their Left record if eligible; otherwise it returns an error.
                if (!await TryRejoinAsync(autoSeatIndex))
                {
                    _showNoChipsModal = true;
                }
                return;
            }

            _joinBuyInBalance = currentBalance;
            _joinBuyInBalanceCap = currentBalance;
            _pendingJoinSeatIndex = autoSeatIndex;

            InitializeJoinBuyInFromTable();
            _joinBuyInAmount = Math.Min(GetRecommendedJoinBuyIn(), GetJoinBuyInMax());
            NormalizeJoinBuyIn();
            _showJoinBuyInModal = true;
        }
        catch (Exception ex)
        {
            Logger.LogWarning(ex, "Failed to check chip balance before joining seat");
            await ShowToastAsync("Unable to load your cashier balance right now.", "error");
        }
    }

    /// <summary>
    /// Attempts to rejoin a game where the player previously left with unreturned chips.
    /// The server detects the Left record and reactivates it without a wallet debit.
    /// Returns true if the rejoin succeeded, false if the server rejected the attempt.
    /// </summary>
    private async Task<bool> TryRejoinAsync(int seatIndex)
    {
        try
        {
            // StartingChips=1 is a placeholder; the server ignores it for rejoin and uses the previous chip stack.
            var request = new Api.Clients.JoinGameRequest(seatIndex, 1);
            var response = await GamesApiClient.JoinGameAsync(GameId, request);

            if (!response.IsSuccessStatusCode)
            {
                return false;
            }

            // Rejoin succeeded — set local state; SignalR table state update will fill in the correct chip count.
            _loggedInUserEmail ??= await GetLoggedInUserEmailAsync();
            var canPlay = response.Content?.CanPlayCurrentHand ?? false;
            var responseSeatIndex = response.Content?.SeatIndex ?? seatIndex;
            _currentPlayerId = response.Content?.PlayerId;

            _seats[responseSeatIndex] = new SeatInfo
            {
                SeatIndex = responseSeatIndex,
                IsOccupied = true,
                PlayerName = response.Content?.PlayerName ?? _loggedInUserEmail ?? "You",
                PlayerAvatarUrl = response.Content?.PlayerAvatarUrl,
                PlayerFirstName = response.Content?.PlayerFirstName,
                IsCurrentPlayer = true,
                IsReady = false,
                IsFolded = !canPlay,
                SittingOutReason = !canPlay ? "Observing" : null
            };
            _isSeated = true;
            _currentPlayerSeatIndex = responseSeatIndex;

            Logger.LogInformation(
                "Successfully rejoined game {GameId} at seat {SeatIndex}",
                GameId, responseSeatIndex);

            await ShowToastAsync("Welcome back! Your seat has been restored.", "success");
            return true;
        }
        catch (Exception ex)
        {
            Logger.LogWarning(ex, "Rejoin attempt failed for game {GameId}", GameId);
            return false;
        }
    }

    private async Task SubmitJoinBuyInAsync()
    {
        if (_isJoiningSeat || !_pendingJoinSeatIndex.HasValue)
            return;

        if (IsStartedScrewYourNeighborTable)
        {
            CloseJoinBuyInModal();
            await ShowToastAsync("Screw Your Neighbor does not allow players to join after the game has started.", "error");
            return;
        }

        var seatIndex = _pendingJoinSeatIndex.Value;
        if (seatIndex < 0 || seatIndex >= _seats.Count || _seats[seatIndex].IsOccupied)
        {
            if (!TryGetAutoJoinSeatIndex(out seatIndex))
            {
                CloseJoinBuyInModal();
                await ShowToastAsync("No seats are currently available.", "error");
                return;
            }

            _pendingJoinSeatIndex = seatIndex;
        }

        try
        {
            NormalizeJoinBuyIn();

            if (_joinBuyInAmount <= 0)
            {
                await ShowToastAsync("Buy-in amount must be greater than 0.", "error");
                return;
            }

            if (_joinBuyInBalanceCap.HasValue && _joinBuyInAmount > _joinBuyInBalanceCap.Value)
            {
                _joinBuyInAmount = _joinBuyInBalanceCap.Value;
                NormalizeJoinBuyIn();
                await ShowToastAsync("Buy-in cannot exceed your account balance.", "error");
                return;
            }

            _isJoiningSeat = true;

            // Call the API to join the game at this seat
            var request = new Api.Clients.JoinGameRequest(seatIndex, _joinBuyInAmount);

            var response = await GamesApiClient.JoinGameAsync(GameId, request);

            if (response.StatusCode == HttpStatusCode.Accepted)
            {
                var hostName = TryGetHeaderValue(response.Headers, "X-Host-Name");
                var joinRequestIdValue = TryGetHeaderValue(response.Headers, "X-Join-Request-Id");
                Guid? joinRequestId = Guid.TryParse(joinRequestIdValue, out var parsedJoinRequestId)
                    ? parsedJoinRequestId
                    : null;

                CloseJoinBuyInModal();
                ShowJoinApprovalPendingModal(joinRequestId, hostName);
                await ShowToastAsync($"Waiting for {(_joinApprovalHostName ?? "the host")} to approve your request.", "info");
                return;
            }

            if (!response.IsSuccessStatusCode)
            {
                Logger.LogWarning("Failed to join game at seat {SeatIndex}: {StatusCode} - {Error}",
                seatIndex, response.StatusCode, response.Error?.Content);

                var joinMessage = ExtractErrorMessage(response.Error);
                if (IsNoChipsMessage(joinMessage))
                {
                    CloseJoinBuyInModal();
                    _showNoChipsModal = true;
                    return;
                }

                await ShowToastAsync(joinMessage, "error");
                return;
            }

            // Update local state - SignalR will also broadcast the update
            _loggedInUserEmail ??= await GetLoggedInUserEmailAsync();
            var canPlay = response.Content?.CanPlayCurrentHand ?? false;

            // Store the player ID for API calls (e.g., Drop or Stay decision)
            _currentPlayerId = response.Content?.PlayerId;

            _seats[seatIndex] = new SeatInfo
            {
                SeatIndex = seatIndex,
                IsOccupied = true,
                PlayerName = response.Content?.PlayerName ?? _loggedInUserEmail ?? "You",
                PlayerAvatarUrl = response.Content?.PlayerAvatarUrl,
                PlayerFirstName = response.Content?.PlayerFirstName,
                Chips = _joinBuyInAmount,
                IsCurrentPlayer = true,
                IsReady = false,
                IsFolded = !canPlay,
                SittingOutReason = !canPlay ? "Observing" : null
            };
            _isSeated = true;
            _currentPlayerSeatIndex = seatIndex;
            CloseJoinBuyInModal();

            Logger.LogInformation(
            "Successfully joined game {GameId} at seat {SeatIndex}. CanPlayCurrentHand: {CanPlay}",
            GameId, seatIndex, response.Content?.CanPlayCurrentHand);
        }
        catch (Exception ex)
        {
            Logger.LogError(ex, "Error joining game at seat {SeatIndex}", seatIndex);
			await ShowToastAsync("An error occurred while joining the table.", "error");
        }
        finally
        {
            _isJoiningSeat = false;
        }
    }

    private async Task HandleJoinRequestResolvedAsync(GameJoinRequestResolvedDto notification)
    {
        if (notification.GameId != GameId)
        {
            return;
        }

        if (_pendingJoinRequestId.HasValue && notification.JoinRequestId != _pendingJoinRequestId.Value)
        {
            return;
        }

        CloseJoinApprovalPendingModal();

        if (string.Equals(notification.Status, "Approved", StringComparison.OrdinalIgnoreCase))
        {
            await ApplyRefreshPolicyAsync(TableInteraction.JoinRequestApproved);
            await ShowToastAsync("The host approved your join request.", "success");
        }
        else
        {
            await ShowToastAsync(notification.Reason ?? "The host denied your join request.", "error");
        }
    }

    private void InitializeJoinBuyInFromTable()
    {
        if (_joinBuyInInitializedFromTable)
        {
            NormalizeJoinBuyIn();
            return;
        }

        _joinBuyInAmount = HasFixedTournamentBuyIn ? _gameResponse!.TournamentBuyIn!.Value : GetRecommendedJoinBuyIn();
        _joinBuyInInitializedFromTable = true;
        NormalizeJoinBuyIn();
    }

    private void NormalizeJoinBuyIn()
    {
        var min = GetJoinBuyInMin();
        var max = GetJoinBuyInMax();
        var step = GetJoinBuyInStep();

        var clamped = Math.Clamp(_joinBuyInAmount, min, max);
        if (step > 1)
        {
            var snapped = ((clamped - min) / step) * step + min;
            clamped = Math.Clamp(snapped, min, max);
        }

        _joinBuyInAmount = clamped;
    }

    private void ClampJoinBuyIn()
    {
        _joinBuyInAmount = Math.Clamp(_joinBuyInAmount, GetJoinBuyInMin(), GetJoinBuyInMax());
    }

    private int GetRecommendedJoinBuyIn()
    {
        if (HasFixedTournamentBuyIn)
        {
            return _gameResponse!.TournamentBuyIn!.Value;
        }

        if (IsScrewYourNeighbor)
        {
            return GetJoinBuyInMin();
        }

        var min = GetJoinBuyInMin();
        var step = GetJoinBuyInStep();
        return Math.Clamp(step * 10, min, GetJoinBuyInMax());
    }

    private int GetJoinBuyInMin()
    {
        if (HasFixedTournamentBuyIn)
        {
            return _gameResponse!.TournamentBuyIn!.Value;
        }

        var minBet = _gameResponse?.MinBet ?? 0;
        var ante = _gameResponse?.Ante ?? _ante;

        if (IsScrewYourNeighbor)
        {
            var synMin = Math.Max(1, ante * 3);
            if (_joinBuyInBalanceCap.HasValue && _joinBuyInBalanceCap.Value > 0 && _joinBuyInBalanceCap.Value < synMin)
            {
                return _joinBuyInBalanceCap.Value;
            }

            return synMin;
        }

        var tableMin = Math.Max(1, Math.Max(minBet, ante));

        if (_joinBuyInBalanceCap.HasValue && _joinBuyInBalanceCap.Value > 0 && _joinBuyInBalanceCap.Value < tableMin)
        {
            return _joinBuyInBalanceCap.Value;
        }

        return tableMin;
    }

    private int GetJoinBuyInStep()
    {
        if (HasFixedTournamentBuyIn)
        {
            return 1;
        }

        var minBet = _gameResponse?.MinBet ?? 0;
        var ante = _gameResponse?.Ante ?? _ante;

        if (IsScrewYourNeighbor)
        {
            return Math.Max(1, ante * 3);
        }

        var tableStep = Math.Max(minBet, ante);
        var step = tableStep > 0 ? tableStep : 25;

        if (_joinBuyInBalanceCap.HasValue && _joinBuyInBalanceCap.Value > 0 && _joinBuyInBalanceCap.Value < step)
        {
            return 1;
        }

        return step;
    }

    private int GetJoinBuyInMax()
    {
        if (HasFixedTournamentBuyIn)
        {
            return _gameResponse!.TournamentBuyIn!.Value;
        }

        if (IsScrewYourNeighbor)
        {
            return GetJoinBuyInMin();
        }

        var min = GetJoinBuyInMin();
        var max = Math.Max(min, _joinBuyInBalance);

        if (_joinBuyInBalanceCap.HasValue)
        {
            max = Math.Min(max, _joinBuyInBalanceCap.Value);
        }

        if (_gameResponse?.MaxBuyIn is int tableMaxBuyIn && tableMaxBuyIn > 0)
        {
            max = Math.Min(max, tableMaxBuyIn);
        }

        return max;
    }

    private string GetJoinBuyInSliderStyle()
    {
        var min = GetJoinBuyInMin();
        var max = GetJoinBuyInMax();

        if (max <= min)
        {
            return "--join-buy-in-progress: 100%;";
        }

        var clamped = Math.Clamp(_joinBuyInAmount, min, max);
        var percent = (int)Math.Round((double)(clamped - min) * 100 / (max - min));
        return $"--join-buy-in-progress: {percent}%;";
    }

    private static bool IsNoChipsMessage(string message)
    {
        if (string.IsNullOrWhiteSpace(message))
        {
            return false;
        }

        return message.Contains("no chips", StringComparison.OrdinalIgnoreCase)
            || message.Contains("zero balance", StringComparison.OrdinalIgnoreCase)
            || message.Contains("balance is 0", StringComparison.OrdinalIgnoreCase)
            || message.Contains("balance is zero", StringComparison.OrdinalIgnoreCase)
            || message.Contains("0 chips", StringComparison.OrdinalIgnoreCase)
            || message.Contains("zero chips", StringComparison.OrdinalIgnoreCase);
    }

    private async Task SetReadyAsync()
    {
        if (IsSittingOut)
        {
            await ToggleSitOutAsync();
        }

        // TODO: Send ready request to server
        _isReady = true;
        if (_currentPlayerSeatIndex >= 0)
        {
            _seats[_currentPlayerSeatIndex].IsReady = true;
        }
        await Task.CompletedTask;
    }

    private async Task StartGameAsync()
    {
        if (!_isHost || !_canStartGame)
        {
            return;
        }

        if (IsDealersChoiceWaitingForSelectedGameStart)
        {
            await ShowToastAsync("Game is starting…", "info");
            return;
        }

        _isSubmittingAction = false;
        _isPlayerTurn = false;

        // Dealer's Choice: call the generic StartHand endpoint.
        // The backend detects DC and transitions to WaitingForDealerChoice phase
        // instead of dealing cards. SignalR will broadcast the phase change
        // and the DealerChoiceModal will be shown to the current dealer.
        if (_isDealersChoice)
        {
            var startHandResponse = await GamesApiClient.GenericStartHandAsync(GameId);
            if (!startHandResponse.IsSuccessStatusCode)
            {
                Logger.LogWarning("Failed to start Dealer's Choice hand: {StatusCode}", startHandResponse.StatusCode);
                await ShowToastAsync("Failed to start game. Please try again.", "error");
                return;
            }

            // Don't proceed to deal — the backend sets the phase to WaitingForDealerChoice
            // and SignalR will trigger the dealer selection modal
            await ResyncTableAfterManualStartAsync();
            return;
        }

        if (IsTwosJacksManWithTheAxe)
        {
            var startHandResponse = await TwosJacksManWithTheAxeApi.TwosJacksManWithTheAxeStartHandAsync(GameId);
            if (!startHandResponse.IsSuccessStatusCode)
            {
                return;
            }

            var collectAntesResponse = await TwosJacksManWithTheAxeApi.TwosJacksManWithTheAxeCollectAntesAsync(GameId);
            if (!collectAntesResponse.IsSuccessStatusCode)
            {
                return;
            }

            var dealHandsResponse = await TwosJacksManWithTheAxeApi.TwosJacksManWithTheAxeDealHandsAsync(GameId);
            if (!dealHandsResponse.IsSuccessStatusCode)
            {
                return;
            }
        }
        else if (IsPairPressure)
        {
            var startHandResponse = await PairPressureApi.PairPressureStartHandAsync(GameId);
            if (!startHandResponse.IsSuccessStatusCode)
            {
                return;
            }

            var collectAntesResponse = await PairPressureApi.PairPressureCollectAntesAsync(GameId);
            if (!collectAntesResponse.IsSuccessStatusCode)
            {
                return;
            }

            var dealHandsResponse = await PairPressureApi.PairPressureDealHandsAsync(GameId);
            if (!dealHandsResponse.IsSuccessStatusCode)
            {
                return;
            }
        }
        else if (IsSevenCardStud)
        {
            var startHandResponse = await SevenCardStudApi.SevenCardStudStartHandAsync(GameId);
            if (!startHandResponse.IsSuccessStatusCode)
            {
                return;
            }

            var collectAntesResponse = await SevenCardStudApi.SevenCardStudCollectAntesAsync(GameId);
            if (!collectAntesResponse.IsSuccessStatusCode)
            {
                return;
            }

            var dealHandsResponse = await SevenCardStudApi.SevenCardStudDealHandsAsync(GameId);
            if (!dealHandsResponse.IsSuccessStatusCode)
            {
                return;
            }
        }
        else if (IsRazz)
        {
            var startHandResponse = await SevenCardStudApi.SevenCardStudStartHandAsync(GameId);
            if (!startHandResponse.IsSuccessStatusCode)
            {
                return;
            }

            var collectAntesResponse = await SevenCardStudApi.SevenCardStudCollectAntesAsync(GameId);
            if (!collectAntesResponse.IsSuccessStatusCode)
            {
                return;
            }

            var dealHandsResponse = await SevenCardStudApi.SevenCardStudDealHandsAsync(GameId);
            if (!dealHandsResponse.IsSuccessStatusCode)
            {
                return;
            }
        }
        else if (IsGoodBadUgly)
        {
            var startHandResponse = await SevenCardStudApi.SevenCardStudStartHandAsync(GameId);
            if (!startHandResponse.IsSuccessStatusCode)
            {
                return;
            }

            var collectAntesResponse = await SevenCardStudApi.SevenCardStudCollectAntesAsync(GameId);
            if (!collectAntesResponse.IsSuccessStatusCode)
            {
                return;
            }

            var dealHandsResponse = await GoodBadUglyApi.GoodBadUglyDealHandsAsync(GameId);
            if (!dealHandsResponse.IsSuccessStatusCode)
            {
                return;
            }
        }
        else if (IsBaseball)
        {
            var startHandResponse = await BaseballApi.BaseballStartHandAsync(GameId);
            if (!startHandResponse.IsSuccessStatusCode)
            {
                return;
            }

            var collectAntesResponse = await BaseballApi.BaseballCollectAntesAsync(GameId);
            if (!collectAntesResponse.IsSuccessStatusCode)
            {
                return;
            }

            var dealHandsResponse = await BaseballApi.BaseballDealHandsAsync(GameId);
            if (!dealHandsResponse.IsSuccessStatusCode)
            {
                return;
            }
        }
        else if (IsFollowTheQueen)
        {
            var startHandResponse = await FollowTheQueenApi.FollowTheQueenStartHandAsync(GameId);
            if (!startHandResponse.IsSuccessStatusCode)
            {
                return;
            }

            var collectAntesResponse = await FollowTheQueenApi.FollowTheQueenCollectAntesAsync(GameId);
            if (!collectAntesResponse.IsSuccessStatusCode)
            {
                return;
            }

            var dealHandsResponse = await FollowTheQueenApi.FollowTheQueenDealHandsAsync(GameId);
            if (!dealHandsResponse.IsSuccessStatusCode)
            {
                return;
            }
        }
        else if (IsTollbooth)
        {
            var startHandResponse = await TollboothApi.TollboothStartHandAsync(GameId);
            if (!startHandResponse.IsSuccessStatusCode)
            {
                return;
            }

            var collectAntesResponse = await TollboothApi.TollboothCollectAntesAsync(GameId);
            if (!collectAntesResponse.IsSuccessStatusCode)
            {
                return;
            }

            var dealHandsResponse = await TollboothApi.TollboothDealHandsAsync(GameId);
            if (!dealHandsResponse.IsSuccessStatusCode)
            {
                return;
            }
        }
        else if (IsHoldEm || IsRedRiver || IsKlondike)
        {
            // League tournaments use the generic start endpoint because launched scheduled events
            // already flow through the generic handler in backend coverage.
            IApiResponse startResponse = ShouldStartHoldEmFamilyGameViaGenericEndpoint()
                ? await GamesApiClient.GenericStartHandAsync(GameId)
                : await HoldEmApi.HoldEmStartHandAsync(GameId);

            if (!startResponse.IsSuccessStatusCode)
            {
                Logger.LogWarning("Failed to start Hold'Em hand: {StatusCode}", startResponse.StatusCode);
                await ShowToastAsync("Failed to start game. Please try again.", "error");
                return;
            }
        }
        else if (IsHoldTheBaseball)
        {
            var startResponse = await GamesApiClient.HoldTheBaseballStartHandAsync(GameId);
            if (!startResponse.IsSuccessStatusCode)
            {
                Logger.LogWarning("Failed to start Hold the Baseball hand: {StatusCode}", startResponse.StatusCode);
                await ShowToastAsync("Failed to start game. Please try again.", "error");
                return;
            }
        }
        else if (IsOmaha || IsNebraska || IsSouthDakota || IsBobBarker)
        {
            // Omaha-family variants use the generic start endpoint until dedicated web client routes are introduced.
            var startResponse = await GamesApiClient.GenericStartHandAsync(GameId);
            if (!startResponse.IsSuccessStatusCode)
            {
                Logger.LogWarning("Failed to start Omaha-family hand: {StatusCode}", startResponse.StatusCode);
                await ShowToastAsync("Failed to start game. Please try again.", "error");
                return;
            }
        }
        else if (IsIrishStyleHoldEm)
        {
            // Irish-style Hold'Em variants use the generic start endpoint (same as Omaha).
            var startResponse = await GamesApiClient.GenericStartHandAsync(GameId);
            if (!startResponse.IsSuccessStatusCode)
            {
                Logger.LogWarning("Failed to start Irish-style Hold'Em hand: {StatusCode}", startResponse.StatusCode);
                await ShowToastAsync("Failed to start game. Please try again.", "error");
                return;
            }
        }
        else if (IsKingsAndLows)
        {
            // Kings and Lows has a simpler flow - StartHand handles ante collection and dealing
            var startHandResponse = await KingsAndLowsApi.KingsAndLowsStartHandAsync(GameId);
            if (!startHandResponse.IsSuccessStatusCode)
            {
                return;
            }
        }
        else if (IsScrewYourNeighbor)
        {
            // Screw Your Neighbor skips ante collection; generic StartHand deals 1 card and transitions to KeepOrTrade.
            var startResponse = await GamesApiClient.GenericStartHandAsync(GameId);
            if (!startResponse.IsSuccessStatusCode)
            {
                Logger.LogWarning("Failed to start Screw Your Neighbor hand: {StatusCode}", startResponse.StatusCode);
                await ShowToastAsync("Failed to start game. Please try again.", "error");
                return;
            }
        }
        else if (IsInBetween)
        {
            // In-Between: generic StartHand collects antes and transitions to InBetweenTurn phase.
            var startResponse = await GamesApiClient.GenericStartHandAsync(GameId);
            if (!startResponse.IsSuccessStatusCode)
            {
                Logger.LogWarning("Failed to start In-Between game: {StatusCode}", startResponse.StatusCode);
                await ShowToastAsync("Failed to start game. Please try again.", "error");
                return;
            }
            // Reset In-Between UI state for new game (auto-deal triggered by turn state update)
            _ibAceChoiceRequired = false;
            _ibShowBetControls = false;
            _ibShowDealPrompt = false;
            _ibLastTurnDescription = null;
            _isProcessingInBetween = false;
            _ibWaitingForReveal = false;
            _ibWaitingForBoundaryCards = false;
        }
        else if (IsFiveCardDraw)
        {
            var startHandResponse = await FiveCardDrawApiClient.FiveCardDrawStartHandAsync(GameId);
            if (!startHandResponse.IsSuccessStatusCode)
            {
                return;
            }

            var collectAntesResponse = await FiveCardDrawApiClient.FiveCardDrawCollectAntesAsync(GameId);
            if (!collectAntesResponse.IsSuccessStatusCode)
            {
                return;
            }

            var dealHandsResponse = await FiveCardDrawApiClient.FiveCardDrawDealHandsAsync(GameId);
            if (!dealHandsResponse.IsSuccessStatusCode)
            {
                return;
            }
        }
        else
        {
            throw new NotSupportedException(
                $"No StartNewHand routing configured for game type '{_gameTypeCode}'. " +
                "Add an explicit branch for this game type in StartNewHandAsync.");
        }

        await ResyncTableAfterManualStartAsync();
    }

    /// <summary>
    /// Refreshes the current table snapshot after a successful manual game start.
    /// </summary>
    private async Task ResyncTableAfterManualStartAsync()
    {
        var wasConnected = GameHubClient.IsConnected;

        try
        {
            if (wasConnected)
            {
                await GameHubClient.JoinGameAsync(GameId);
                return;
            }

            await ConnectAndJoinGameAsync();
            if (ResolvePostStartResyncTarget(wasConnected, GameHubClient.IsConnected) != PostStartResyncTarget.HttpReload)
            {
                return;
            }
        }
        catch (Exception ex)
        {
            Logger.LogWarning(ex, "Failed to resync table state via SignalR after manually starting game {GameId}", GameId);
        }

        Logger.LogInformation("Falling back to HTTP table reload after manual game start for game {GameId}", GameId);
        await ApplyRefreshPolicyAsync(TableInteraction.ManualStartFallback);
    }

    private static PostStartResyncTarget ResolvePostStartResyncTarget(bool wasConnected, bool isConnectedAfterReconnectAttempt)
    {
        if (wasConnected)
        {
            return PostStartResyncTarget.HubSnapshot;
        }

        return isConnectedAfterReconnectAttempt
            ? PostStartResyncTarget.HubReconnect
            : PostStartResyncTarget.HttpReload;
    }

    private enum PostStartResyncTarget
    {
        HubSnapshot,
        HubReconnect,
        HttpReload
    }

    // Polling methods removed - SignalR handles state updates

    private async Task TryLoadShowdownAsync()
    {
        Logger.LogDebug("TryLoadShowdownAsync called. CurrentPhase={Phase}, IsShowdownPhase={IsShowdown}",
        _gameResponse?.CurrentPhase, IsShowdownPhase);

        if (_showdownLoadInFlight)
        {
            Logger.LogDebug("TryLoadShowdownAsync: Showdown load already in flight, skipping duplicate request");
            return;
        }

        if (!IsShowdownPhase && !IsGameCompleted)
        {
            Logger.LogDebug("TryLoadShowdownAsync: Not in showdown phase, returning early");
            return;
        }

        if (IsScrewYourNeighbor && _tableState?.Showdown is { IsComplete: true } synShowdown)
        {
            var isDelayActive = _showdownDelayUntil.HasValue && DateTimeOffset.UtcNow < _showdownDelayUntil.Value;
            if (isDelayActive)
            {
                Logger.LogDebug("TryLoadShowdownAsync: SYN showdown already available in public state, waiting for delay to expire");
                return;
            }

            UpdateShowdownFromPublicState(synShowdown);
            if (_showdownResult is not null)
            {
                await InvokeStateHasChangedAsync();
                return;
            }
        }

        // If showdown result is already loaded (e.g., from SignalR update), validate first.
        // Incomplete payloads can happen on non-winner clients when public state is filtered.
        if (_showdownResult is not null)
        {
            if (!HasUsableShowdownResult(_showdownResult))
            {
                Logger.LogDebug("TryLoadShowdownAsync: Existing showdown payload incomplete; refreshing from API.");
                _showdownResult = null;
            }
            else
            {
            Logger.LogDebug("TryLoadShowdownAsync: Showdown result already loaded, showing overlay");
            _showShowdownOverlay = ShouldDisplayShowdownOverlay;

            _showdownStateSnapshot ??= _tableState?.Showdown;
            var showdownCommunityCards = GetShowdownCommunityCards();
            if (showdownCommunityCards.Count > 0)
            {
                _showdownCommunityCardsSnapshot = showdownCommunityCards;
            }

            _isResultsPhase = true;
            RefreshCountdownFromCurrentTableState();

            if (_countdownDeadlineUtc.HasValue)
            {
                StartCountdownTimer();
            }

            await InvokeStateHasChangedAsync();
            return;
            }
        }

        _showdownLoadInFlight = true;

        try
        {
            _loggedInUserEmail ??= await GetLoggedInUserEmailAsync();

            Refit.IApiResponse<PerformShowdownSuccessful>? showdownResponse;
            if (IsTwosJacksManWithTheAxe)
            {
                showdownResponse = await TwosJacksManWithTheAxeApi.TwosJacksManWithTheAxePerformShowdownAsync(GameId);
            }
            else if (IsPairPressure)
            {
                showdownResponse = await PairPressureApi.PairPressurePerformShowdownAsync(GameId);
            }
            else if (IsSevenCardStud)
            {
                showdownResponse = await SevenCardStudApi.SevenCardStudPerformShowdownAsync(GameId);
            }
            else if (IsRazz)
            {
                showdownResponse = await SevenCardStudApi.SevenCardStudPerformShowdownAsync(GameId);
            }
            else if (IsGoodBadUgly)
            {
                showdownResponse = await GoodBadUglyApi.GoodBadUglyPerformShowdownAsync(GameId);
            }
            else if (IsBaseball)
            {
                showdownResponse = await BaseballApi.BaseballPerformShowdownAsync(GameId);
            }
            else if (IsKingsAndLows)
            {
                showdownResponse = await KingsAndLowsApi.KingsAndLowsPerformShowdownAsync(GameId);
            }
            else if (IsHoldEm)
            {
                showdownResponse = await HoldEmApi.HoldEmPerformShowdownAsync(GameId);
            }
            else if (IsHoldTheBaseball)
            {
                showdownResponse = await GamesApiClient.GenericPerformShowdownAsync(GameId);
            }
            else if (IsOmaha || IsNebraska || IsSouthDakota || IsBobBarker || IsKlondike)
            {
                showdownResponse = await GamesApiClient.GenericPerformShowdownAsync(GameId);
            }
            else if (IsIrishStyleHoldEm)
            {
                showdownResponse = await GamesApiClient.GenericPerformShowdownAsync(GameId);
            }
            else if (IsScrewYourNeighbor)
            {
                showdownResponse = await GamesApiClient.GenericPerformShowdownAsync(GameId);
            }
            else if (IsInBetween)
            {
                // In-Between has no showdown — game ends when pot reaches zero.
                Logger.LogDebug("TryLoadShowdownAsync: In-Between game has no showdown, skipping.");
                return;
            }
            else if (IsTollbooth)
            {
                showdownResponse = await TollboothApi.TollboothPerformShowdownAsync(GameId);
            }
            else if (IsFiveCardDraw)
            {
                showdownResponse = await FiveCardDrawApiClient.FiveCardDrawPerformShowdownAsync(GameId);
            }
            else
            {
                throw new NotSupportedException(
                    $"No showdown routing configured for game type '{_gameTypeCode}'. " +
                    "Add an explicit branch for this game type in TryLoadShowdownAsync.");
            }

            Logger.LogDebug("TryLoadShowdownAsync: Showdown API response - IsSuccess={IsSuccess}, HasContent={HasContent}",
            showdownResponse?.IsSuccessStatusCode, showdownResponse?.Content is not null);

            if (showdownResponse is not { IsSuccessStatusCode: true, Content: not null })
            {
                Logger.LogWarning("TryLoadShowdownAsync: Showdown API failed or returned no content. StatusCode={StatusCode}, Error={Error}",
                showdownResponse?.StatusCode, showdownResponse?.Error?.Content);
                return;
            }

            _showdownResult = showdownResponse.Content;
            _showdownStateSnapshot ??= _tableState?.Showdown;

            var showdownCommunityCards = GetShowdownCommunityCards();
            if (showdownCommunityCards.Count > 0)
            {
                _showdownCommunityCardsSnapshot = showdownCommunityCards;
            }

            // If a showdown overlay delay is active (e.g., SYN 7-second pause), populate the
            // result but don't show the overlay yet — the ScheduleShowdownOverlayAfterDelay
            // callback will reveal it when the timer expires.
            var isDelayActive = _showdownDelayUntil.HasValue && DateTimeOffset.UtcNow < _showdownDelayUntil.Value;
            if (!isDelayActive)
            {
                _showShowdownOverlay = ShouldDisplayShowdownOverlay;
                Logger.LogDebug("TryLoadShowdownAsync: Set _showShowdownOverlay=true, _showdownResult loaded");

                _isResultsPhase = true;
                RefreshCountdownFromCurrentTableState();

                if (_countdownDeadlineUtc.HasValue)
                {
                    StartCountdownTimer();
                }
            }
            else
            {
                Logger.LogDebug("TryLoadShowdownAsync: Showdown loaded but delay active, overlay deferred");
            }

            if (string.IsNullOrWhiteSpace(_loggedInUserEmail) && string.IsNullOrWhiteSpace(_currentPlayerName))
            {
                _showdownWon = false;
                _showdownLost = false;
                return;
            }

            // Check against both email and display name since payout keys may use either
            var iWon = _showdownResult.Payouts?.Keys.Any(p =>
            string.Equals(p, _loggedInUserEmail, StringComparison.OrdinalIgnoreCase) ||
            string.Equals(p, _currentPlayerName, StringComparison.OrdinalIgnoreCase)) == true;
            _showdownWon = iWon;
            _showdownLost = !iWon;

            await InvokeStateHasChangedAsync();
        }
        finally
        {
            _showdownLoadInFlight = false;
        }
    }

    private void CloseShowdownOverlay()
    {
        _showShowdownOverlay = false;
        ResetRabbitHuntState();
    }

    private ShowdownPublicDto? GetOverlayShowdownState()
    {
        return _tableState?.Showdown ?? _showdownStateSnapshot;
    }

    private IReadOnlyList<ShowdownCard> GetOverlayShowdownCommunityCards()
    {
        var currentCommunityCards = GetShowdownCommunityCards();
        return currentCommunityCards.Count > 0 ? currentCommunityCards : _showdownCommunityCardsSnapshot;
    }

    private async Task LoadRabbitHuntAsync()
    {
        if (_rabbitHuntLoadInFlight || !CanRequestRabbitHunt())
        {
            return;
        }

        _rabbitHuntLoadInFlight = true;
        _rabbitHuntError = null;

        try
        {
            var response = await GamesApiClient.GetRabbitHuntAsync(GameId);
            if (response is { IsSuccessStatusCode: true, Content: not null })
            {
                _rabbitHuntResult = response.Content;
                return;
            }

            _rabbitHuntError = GetRabbitHuntErrorMessage(response?.StatusCode);
        }
        catch (Exception ex)
        {
            Logger.LogWarning(ex, "Failed to load Rabbit Hunt for game {GameId}", GameId);
            _rabbitHuntError = "Unable to load Rabbit Hunt right now.";
        }
        finally
        {
            _rabbitHuntLoadInFlight = false;
            await InvokeStateHasChangedAsync();
        }
    }

    private bool CanRequestRabbitHunt()
    {
        if (!SupportsRabbitHunt || _showdownResult is null)
        {
            return false;
        }

        if (_rabbitHuntResult is not null && _rabbitHuntResult.HandNumber == GetCurrentOverlayHandNumber())
        {
            return false;
        }

        var communityCards = GetVisibleCommunityCards();
        if (communityCards is null)
        {
            return true;
        }

        if (communityCards.Any(card => !card.IsFaceUp))
        {
            return true;
        }

        var faceUpCardCount = communityCards.Count(card => card.IsFaceUp && !(IsBobBarker && card.DealOrder == 0));
        if (faceUpCardCount < GetProjectedRabbitHuntBoardCount())
        {
            return true;
        }

        if (IsRedRiver)
        {
            var riverCard = communityCards.FirstOrDefault(card => card.DealOrder == 5 && card.IsFaceUp);
            if (riverCard is not null && IsRedSuit(riverCard.Suit) && !communityCards.Any(card => card.DealOrder == 6 && card.IsFaceUp))
            {
                return true;
            }
        }

        return false;
    }

    private int GetProjectedRabbitHuntBoardCount()
    {
        if (IsKlondike)
        {
            return 6;
        }

        if (IsSouthDakota)
        {
            return 3;
        }

        return 5;
    }

    private int GetCurrentOverlayHandNumber()
    {
        return _tableState?.CurrentHandNumber
            ?? _gameResponse?.CurrentHandNumber
            ?? _handNumber;
    }

    private string GetRabbitHuntErrorMessage(HttpStatusCode? statusCode)
    {
        return statusCode switch
        {
            HttpStatusCode.Conflict => "Rabbit Hunt is not available for this hand.",
            HttpStatusCode.Forbidden => "Only seated players can use Rabbit Hunt.",
            HttpStatusCode.NotFound => "The hand could not be found.",
            HttpStatusCode.Unauthorized => "You must be signed in to use Rabbit Hunt.",
            _ => "Unable to load Rabbit Hunt right now."
        };
    }

    private void ResetRabbitHuntState()
    {
        _rabbitHuntResult = null;
        _rabbitHuntError = null;
        _rabbitHuntLoadInFlight = false;
    }

    private bool IsRedSuit(string? suit)
    {
        return string.Equals(suit, nameof(CardSuit.Hearts), StringComparison.OrdinalIgnoreCase)
            || string.Equals(suit, nameof(CardSuit.Diamonds), StringComparison.OrdinalIgnoreCase);
    }

    // Note: StartRunoutAnimation has been inlined into HandleTableStateUpdatedAsync
    // to ensure the animation flag is set BEFORE UpdateSeatsFromPublicState is called.

    /// <summary>
    /// Timer callback that deals the next card in the runout sequence.
    /// Cards are dealt one at a time to one player at a time with 2-second delays.
    /// Example: P1 gets card, 2s delay, P2 gets card, 2s delay, P3 gets card, 2s delay, P1 gets next card...
    /// </summary>
    private async void DealNextRunoutCard(object? state)
    {
        if (_allInRunoutState?.RunoutCardsBySeat is null)
        {
            CompleteRunoutAnimation();
            return;
        }

        // Get the list of seat indices with cards to deal (sorted for consistent order)
        var seatIndices = _allInRunoutState.RunoutCardsBySeat.Keys
        .Where(s => s >= 0 && s < _seats.Count && _seats[s].IsOccupied && !_seats[s].IsFolded)
        .OrderBy(s => s)
        .ToList();

        if (seatIndices.Count == 0)
        {
            CompleteRunoutAnimation();
            return;
        }

        // Calculate how many cards each player needs to receive
        var maxRunoutCards = _allInRunoutState.RunoutCardsBySeat.Values
        .Select(cards => cards.Count)
        .DefaultIfEmpty(0)
        .Max();

        // Check if we've completed all streets
        if (_runoutCurrentStreetIndex >= maxRunoutCards)
        {
            // All cards dealt - wait 2 seconds then show showdown
            _runoutDealTimer?.Change(Timeout.InfiniteTimeSpan, Timeout.InfiniteTimeSpan);
            _ = Task.Run(async () =>
            {
                await Task.Delay(TimeSpan.FromSeconds(2));
                CompleteRunoutAnimation();
                await InvokeStateHasChangedAsync();
            });
            return;
        }

        // Get the current seat to deal to
        if (_runoutCurrentSeatIndex >= seatIndices.Count)
        {
            // Finished dealing to all players for this street, move to next street
            _runoutCurrentSeatIndex = 0;
            _runoutCurrentStreetIndex++;

            // Check again if we've completed all streets
            if (_runoutCurrentStreetIndex >= maxRunoutCards)
            {
                _runoutDealTimer?.Change(Timeout.InfiniteTimeSpan, Timeout.InfiniteTimeSpan);
                _ = Task.Run(async () =>
                {
                    await Task.Delay(TimeSpan.FromSeconds(2));
                    CompleteRunoutAnimation();
                    await InvokeStateHasChangedAsync();
                });
                return;
            }
        }

        // Deal ONE card to ONE player
        var currentSeatIndex = seatIndices[_runoutCurrentSeatIndex];
        if (_allInRunoutState.RunoutCardsBySeat.TryGetValue(currentSeatIndex, out var runoutCards))
        {
            var currentVisible = _runoutVisibleCardCount.GetValueOrDefault(currentSeatIndex, 0);
            if (currentVisible < runoutCards.Count)
            {
                var runoutCard = runoutCards[currentVisible];

                // Start flying card animation for runout
                _flyingCard = new TableCanvas.FlyingCardInfo(
                    TargetSeatIndex: currentSeatIndex,
                    CardIndex: currentVisible
                );

                await InvokeStateHasChangedAsync();

                // Wait for the flying animation to complete
                await Task.Delay(550);

                // Clear the flying card
                _flyingCard = null;

                _runoutVisibleCardCount[currentSeatIndex] = currentVisible + 1;
                UpdateSeatWithRunoutCards(currentSeatIndex, currentVisible + 1);

                Logger.LogDebug("Runout: Dealt card {CardNum} to seat {SeatIndex} (street {Street})",
                currentVisible + 1, currentSeatIndex, _runoutCurrentStreetIndex + 1);
            }
        }

        // Move to next player
        _runoutCurrentSeatIndex++;

        await InvokeStateHasChangedAsync();

        // Schedule the next card deal in 2 seconds
        _runoutDealTimer?.Change(TimeSpan.FromSeconds(2), Timeout.InfiniteTimeSpan);
    }

    /// <summary>
    /// Updates a seat's cards by appending the next dealt runout card.
    /// The initial cards are preserved, and runout cards are added in deal order,
    /// including face-down placeholders so later streets keep the correct positions.
    /// </summary>
    private void UpdateSeatWithRunoutCards(int seatIndex, int visibleRunoutCards)
    {
        if (seatIndex < 0 || seatIndex >= _seats.Count)
        {
            return;
        }

        if (_allInRunoutState?.RunoutCardsBySeat is null ||
        !_allInRunoutState.RunoutCardsBySeat.TryGetValue(seatIndex, out var runoutCards))
        {
            return;
        }

        // Get the initial card count for this seat (captured when runout started)
        var initialCardCount = _runoutInitialCardCounts.GetValueOrDefault(seatIndex, 0);

        var seat = _seats[seatIndex];
        var existingCards = seat.Cards.ToList();

        Logger.LogDebug("UpdateSeatWithRunoutCards: Seat {SeatIndex}, initialCardCount={InitialCount}, existingCards={ExistingCount}, visibleRunoutCards={VisibleRunout}, totalRunoutCards={TotalRunout}",
        seatIndex, initialCardCount, existingCards.Count, visibleRunoutCards, runoutCards.Count);

        // Build the new cards list: initial cards + visible runout cards
        var allCards = new List<CardInfo>();

        // First, add the initial cards (cards dealt before runout)
        // These should already be in existingCards if we're preserving correctly
        for (var i = 0; i < Math.Min(initialCardCount, existingCards.Count); i++)
        {
            allCards.Add(existingCards[i]);
        }

        // Then add the runout cards that have been dealt so far, preserving face-down placeholders.
        for (var i = 0; i < visibleRunoutCards && i < runoutCards.Count; i++)
        {
            var runoutCard = runoutCards[i];
            allCards.Add(CreateRunoutCardInfo(runoutCard, initialCardCount, i));
        }

        _seats[seatIndex].Cards = allCards;

        Logger.LogDebug("UpdateSeatWithRunoutCards: Seat {SeatIndex} now has {CardCount} cards",
        seatIndex, allCards.Count);
    }

    private CardInfo CreateRunoutCardInfo(CardPublicDto runoutCard, int initialCardCount, int runoutCardIndex)
    {
        var isFaceUp = runoutCard.IsFaceUp;

        return new CardInfo
        {
            Rank = isFaceUp ? runoutCard.Rank : null,
            Suit = isFaceUp ? runoutCard.Suit : null,
            IsFaceUp = isFaceUp,
            IsPubliclyVisible = isFaceUp,
            IsSelected = false,
            DealOrder = initialCardCount + runoutCardIndex,
            IsWild = isFaceUp && IsWildCard(runoutCard.Rank, runoutCard.Suit)
        };
    }

    private void StartCardDealAnimation(TableStatePublicDto state, int startCardIndex, int numberOfCardsToDeal, int detectedSeatIndex = -1)
    {
        Logger.LogInformation("Starting card deal animation for game {GameId}. StartIdx={Start}, Count={Count}, DetectedSeat={Seat}", 
            GameId, startCardIndex, numberOfCardsToDeal, detectedSeatIndex);

        _dealAnimationInProgress = true;
        _dealAnimationPausedForBuyCardOffer = false;
        _dealPausedBuyCardOfferSeatIndex = -1;
        _dealTargetState = state;
        _dealCurrentCardIndex = startCardIndex; // Start dealing at this card index (e.g., 0 for initial, 3 for 4th street)
        _dealTargetEndIndex = startCardIndex + numberOfCardsToDeal; // Stop when index reaches this value
        _dealCurrentSeatIdx = 0;

        // Determine deal order: Start from Dealer + 1
        // Filter to seats that are occupied, not sitting out, AND NOT Folded in the NEW state
        // (Folded players do not receive new cards in Stud, except initially everyone is not folded)
        // For Tollbooth offers, only animate to the seat that actually received the card (detectedSeatIndex),
        // since CurrentActorSeatIndex has already advanced to the next player by the time the state arrives.
        var isTollboothOffer = IsTollbooth && detectedSeatIndex >= 0;
        var activePublicSeats = state.Seats
            .Where(s => s.IsOccupied && !s.IsSittingOut && !s.IsFolded)
            .Where(s => !isTollboothOffer || s.SeatIndex == detectedSeatIndex)
            .ToList();

        if (activePublicSeats.Count == 0)
        {
            CompleteCardDealAnimation();
            return;
        }

        // Create ordered list starting after dealer
        var dealerPos = state.DealerSeatIndex;
        _dealOrderedSeatIndices = activePublicSeats
            .Select(s => s.SeatIndex)
            .OrderBy(idx => (idx - dealerPos - 1 + 8) % 8) // Circular order after dealer
            .ToList();

        // Cache target cards from the NEW state to know rank/suit/faceup status
        _dealTargetCards.Clear();
        foreach (var seat in activePublicSeats)
        {
            _dealTargetCards[seat.SeatIndex] = seat.Cards
                .Select(c => new CardInfo { Rank = c.Rank, Suit = c.Suit, IsFaceUp = c.IsFaceUp })
                .ToList();
        }

        // Start timer with game-specific delay between cards
        _dealTimer = new Timer(DealNextCard, null, 100, DealAnimationDelayMs);
    }

    private async void DealNextCard(object? state)
    {
        if (!_dealAnimationInProgress) return;

        // Clear any previous flying card
        _flyingCard = null;

        // If we've completed all players for the current card index
        if (_dealCurrentSeatIdx >= _dealOrderedSeatIndices.Count)
        {
            // Move to next card in the sequence (e.g. from 1st card to 2nd card)
            _dealCurrentCardIndex++;
            _dealCurrentSeatIdx = 0;
        }

        // Check if we are done dealing all assigned cards
        if (_dealCurrentCardIndex >= _dealTargetEndIndex)
        {
            await InvokeAsync(() =>
            {
                _flyingCard = null;
                CompleteCardDealAnimation();
                StateHasChanged();
            });
            return;
        }

        // Deal card to current seat
        var seatIndex = _dealOrderedSeatIndices[_dealCurrentSeatIdx];
        var dealtFaceUp = false;

        // Find the seat in the current display state
        var seatListIndex = _seats.FindIndex(s => s.SeatIndex == seatIndex);

        if (seatListIndex >= 0)
        {
            var seat = _seats[seatListIndex];

            // Determine properties from the TARGET state
            string? rank = null;
            string? suit = null;
            bool isFaceUp = false;

            if (_dealTargetCards.TryGetValue(seatIndex, out var targetCards) && 
                _dealCurrentCardIndex < targetCards.Count)
            {
                var targetCard = targetCards[_dealCurrentCardIndex];
                rank = targetCard.Rank;
                suit = targetCard.Suit;
                isFaceUp = targetCard.IsFaceUp;
            }

            dealtFaceUp = isFaceUp;

            // Start flying card animation
            _flyingCard = new TableCanvas.FlyingCardInfo(
                TargetSeatIndex: seatIndex,
                CardIndex: _dealCurrentCardIndex
            );

            await InvokeStateHasChangedAsync();

            // Wait for the flying animation to complete (600ms matches CSS animation duration)
            await Task.Delay(550);

            // Clear the flying card and add card to the seat
            _flyingCard = null;

            // Create the card
            var newCard = new CardInfo
            {
                Rank = rank,
                Suit = suit,
                IsFaceUp = isFaceUp,
                IsPubliclyVisible = isFaceUp,
                IsSelected = false,
                DealOrder = _dealCurrentCardIndex,
                IsWild = IsWildCard(rank, suit)
            };

            // Safely add card. Check if we are replacing or adding.
            // Normally adding. But safeguard if list is somehow already large?
            // Since we preserved existingCards, usually we just append.
            var newHand = seat.Cards.ToList();
            if (newHand.Count > _dealCurrentCardIndex)
            {
                // This shouldn't happen if logic is correct, but if it does, update existing
                newHand[_dealCurrentCardIndex] = newCard;
            }
            else
            {
                newHand.Add(newCard);
            }

            _seats[seatListIndex] = seat with { Cards = newHand };

            if (seatIndex == _currentPlayerSeatIndex)
            {
                await PlayDealCardSoundAsync(1);
            }

            Logger.LogDebug("Deal Animation: CardIdx {CardIdx}, Seat {SeatIndex}, FaceUp={FaceUp}",
                _dealCurrentCardIndex, seatIndex, isFaceUp);
        }

        _dealCurrentSeatIdx++;

        if (ShouldPauseDealForBuyCardOffer(seatIndex, dealtFaceUp))
        {
            _dealAnimationPausedForBuyCardOffer = true;
            _dealPausedBuyCardOfferSeatIndex = seatIndex;
            _dealTimer?.Change(Timeout.InfiniteTimeSpan, Timeout.InfiniteTimeSpan);
            await InvokeStateHasChangedAsync();
            return;
        }

        await InvokeStateHasChangedAsync();
    }

    /// <summary>
    /// Timer callback that reveals the next community card during a staggered flop reveal.
    /// </summary>
    private async void RevealNextCommunityCard(object? state)
    {
        _communityCardRevealCount++;
        await PlayDealCardSoundAsync(1);

        if (_communityCardRevealCount >= _communityCardRevealTarget)
        {
            _communityCardRevealInProgress = false;
            _communityCardRevealTimer?.Dispose();
            _communityCardRevealTimer = null;

            _ = TryPlayPendingTurnAlertSoundAsync();
        }
        else
        {
            _communityCardRevealTimer?.Change(250, Timeout.Infinite);
        }

        await InvokeStateHasChangedAsync();
    }

    /// <summary>
    /// Returns community cards with staggered visibility during flop reveal animation.
    /// During a reveal, only the first <see cref="_communityCardRevealCount"/> face-up cards are included.
    /// When Tollbooth deal animation is in progress, community cards are hidden until the deal completes.
    /// </summary>
    private IReadOnlyList<CardPublicDto>? GetVisibleCommunityCards()
    {
        if (_deferCommunityCardsForDealAnimation)
            return null;

        var cards = _tableState?.CommunityCards;
        if (cards is null || !_communityCardRevealInProgress)
            return cards;

        var result = new List<CardPublicDto>();
        int faceUpCount = 0;
        foreach (var card in cards)
        {
            if (card.IsFaceUp)
            {
                faceUpCount++;
                if (faceUpCount > _communityCardRevealCount)
                    continue;
            }
            result.Add(card);
        }
        return result;
    }

    private IReadOnlyList<ShowdownCard> GetShowdownCommunityCards()
    {
        var communityCards = GetVisibleCommunityCards();
        if (IsBobBarker)
        {
            communityCards = communityCards?
                .Where(card => card.DealOrder != 0)
                .ToList();
        }

        return ConvertCardsToShowdownCards(communityCards).ToList();
    }

    private async Task PlayTurnAlertSoundAsync()
    {
        if (IsCardDealingVisualInProgress || _communityCardRevealInProgress)
        {
            _pendingTurnAlertSound = true;
            return;
        }

        _pendingTurnAlertSound = false;

        if (_isDealSoundMuted || !_isJsInteropReady)
        {
            return;
        }

        try
        {
            await JSRuntime.InvokeVoidAsync("cardGamesAudio.playSoundEffect", "/sounds/alert.mp3");
        }
        catch (Exception ex)
        {
            Logger.LogDebug(ex, "Unable to play turn alert sound");
        }
    }

    private async Task TryPlayPendingTurnAlertSoundAsync()
    {
        if (!_pendingTurnAlertSound || !_isPlayerTurn)
        {
            _pendingTurnAlertSound = false;
            return;
        }

        await PlayTurnAlertSoundAsync();
    }

    private async Task PlayDealCardSoundAsync(int cardCount)
    {
        if (cardCount <= 0 || _isDealSoundMuted)
        {
            return;
        }

        try
        {
            await JSRuntime.InvokeVoidAsync("cardGamesAudio.playDealCard", cardCount);
        }
        catch (Exception ex)
        {
            Logger.LogDebug(ex, "Unable to play deal-card sound effect");
        }
    }

    private async Task TryPlayTableSoundEffectsAsync(IReadOnlyList<TableSoundEffectDto>? soundEffects)
    {
        if (soundEffects is null || soundEffects.Count == 0 || _isDealSoundMuted || !_isJsInteropReady)
        {
            return;
        }

        foreach (var soundEffect in soundEffects)
        {
            await TryPlayTableSoundEffectAsync(soundEffect);
        }
    }

    private async Task TryPlayTableSoundEffectAsync(TableSoundEffectDto soundEffect)
    {
        if (string.IsNullOrWhiteSpace(soundEffect.CueKey) || string.IsNullOrWhiteSpace(soundEffect.Source))
        {
            return;
        }

        if (!_playedTableSoundKeys.Add(soundEffect.CueKey))
        {
            return;
        }

        try
        {
            await JSRuntime.InvokeVoidAsync("cardGamesAudio.playSoundEffect", soundEffect.Source);
        }
        catch (Exception ex)
        {
            _playedTableSoundKeys.Remove(soundEffect.CueKey);
            Logger.LogDebug(ex, "Unable to play {SoundEventKey} sound effect", soundEffect.EventKey);
        }
    }

    private async Task ToggleDealSoundMuteAsync()
    {
        _isDealSoundMuted = !_isDealSoundMuted;

        try
        {
            await JSRuntime.InvokeVoidAsync("cardGamesAudio.setMuted", _isDealSoundMuted);
        }
        catch (Exception ex)
        {
            Logger.LogDebug(ex, "Unable to toggle deal-card sound mute state");
        }
    }

    private void ToggleGameInfoOverlay()
    {
        _showGameInfoOverlay = !_showGameInfoOverlay;
    }

    private void CloseGameInfoOverlay()
    {
        _showGameInfoOverlay = false;
    }

    private string? GetGameInfoImageName()
    {
        var code = _tableState?.GameTypeCode ?? _gameTypeCode;
        if (string.IsNullOrWhiteSpace(code) || string.Equals(code, "DEALERSCHOICE", StringComparison.OrdinalIgnoreCase))
        {
            return _isDealersChoice ? "dealerschoice.png" : null;
        }
        return $"{code.ToLowerInvariant()}.png";
    }

    private string GetGameInfoTypeContext()
    {
        var gameTypeName = _tableState?.GameTypeName;
        if (string.IsNullOrWhiteSpace(gameTypeName))
        {
            gameTypeName = _gameResponse?.GameTypeName;
        }

        if (_isDealersChoice)
        {
            return string.IsNullOrWhiteSpace(gameTypeName)
                ? "Dealer's Choice"
                : $"Dealer's Choice — {gameTypeName}";
        }

        return string.IsNullOrWhiteSpace(gameTypeName)
            ? (_gameTypeCode ?? "Unknown")
            : gameTypeName;
    }

    private IReadOnlyList<string> GetGameInfoContextItems()
    {
        var contextItems = new List<string>();

        if (_gameResponse?.CurrentHandNumber > 0)
        {
            contextItems.Add($"Hand: {_gameResponse.CurrentHandNumber}");
        }

        var phaseDescription = _tableState?.CurrentPhaseDescription ?? _gameResponse?.CurrentPhaseDescription;
        if (!string.IsNullOrWhiteSpace(phaseDescription))
        {
            contextItems.Add($"Phase: {phaseDescription}");
        }

        if (_isDealersChoice)
        {
            var dealersChoiceDealer = GetDealersChoiceDealerName();
            if (!string.IsNullOrWhiteSpace(dealersChoiceDealer))
            {
                contextItems.Add($"Dealer's Choice dealer: {dealersChoiceDealer}");
            }
        }

        var drawingSpecialRules = GetDrawingSpecialRules();
        if (!string.IsNullOrWhiteSpace(drawingSpecialRules))
        {
            contextItems.Add($"Draw rules: {drawingSpecialRules}");
        }

        if (HasWildCards)
        {
            var wildCardLabels = GetWildCardsForDisplay()
                .Select(wildCard => wildCard.Label)
                .Where(label => !string.IsNullOrWhiteSpace(label))
                .Distinct(StringComparer.OrdinalIgnoreCase)
                .ToList();

            if (wildCardLabels.Count > 0)
            {
                contextItems.Add($"Wild cards: {string.Join(", ", wildCardLabels)}");
            }
        }

        return contextItems;
    }

    private void CompleteCardDealAnimation()
    {
        Logger.LogInformation("Card deal animation complete");

        _dealAnimationPausedForBuyCardOffer = false;
        _dealPausedBuyCardOfferSeatIndex = -1;
        _dealTimer?.Dispose();
        _dealTimer = null;
        _flyingCard = null;

        // Sync with Pending State while _dealAnimationInProgress is still true
        // so that UpdateSeatsFromPublicState preserves cards already placed by the animation
        if (_dealTargetState != null)
        {
            UpdateSeatsFromPublicState(_dealTargetState.Seats);
            _dealTargetState = null;
        }

        _dealAnimationInProgress = false;

        // Tollbooth: reveal deferred community cards now that all player cards have been dealt
        // If community card reveal starts, defer the turn alert to after the reveal completes
        if (_deferCommunityCardsForDealAnimation)
        {
            _deferCommunityCardsForDealAnimation = false;
            var faceUpCount = _tableState?.CommunityCards?.Count(c => c.IsFaceUp) ?? 0;
            if (faceUpCount > 1)
            {
                _communityCardRevealCount = 1;
                _communityCardRevealTarget = faceUpCount;
                _communityCardRevealInProgress = true;
                _communityCardRevealTimer?.Dispose();
                _communityCardRevealTimer = new Timer(RevealNextCommunityCard, null, 250, Timeout.Infinite);
            }
        }

        // Play pending turn alert if no community card reveal was started
        if (!_communityCardRevealInProgress)
        {
            _ = TryPlayPendingTurnAlertSoundAsync();
        }

        // Sync with Pending Private State (for current player hole cards)
        if (_dealPendingPrivateState != null)
        {
            _ = HandlePrivateStateUpdatedAsync(_dealPendingPrivateState);
            _dealPendingPrivateState = null;
        }
    }

    /// <summary>
    /// Completes the runout animation and triggers the showdown.
    /// </summary>
    private async void CompleteRunoutAnimation()
    {
        Logger.LogInformation("All-in runout animation complete, transitioning to showdown overlay");

        _runoutAnimationInProgress = false;
        _runoutDealTimer?.Dispose();
        _runoutDealTimer = null;
        _flyingCard = null;

        // Now load and show the showdown overlay
        await TryLoadShowdownAsync();

        await InvokeStateHasChangedAsync();
    }

    private static List<BettingActionType> MapLegalActions(AvailableActionsResponse available)
    {
        var actions = new List<BettingActionType>();

        if (available.CanFold) actions.Add(BettingActionType.Fold);
        if (available.CanCheck) actions.Add(BettingActionType.Check);
        if (available.CanCall) actions.Add(BettingActionType.Call);
        if (available.CanBet) actions.Add(BettingActionType.Bet);
        if (available.CanRaise) actions.Add(BettingActionType.Raise);
        if (available.CanAllIn) actions.Add(BettingActionType.AllIn);

        return actions;
    }

    private async Task PauseGameAsync()
    {
        if (!_isHost)
        {
            return;
        }

        // TODO: Send pause request to server
        _isPaused = true;
        await Task.CompletedTask;
    }

    private async Task ToggleOddsVisibilityAsync()
    {
        if (!_isHost || _isUpdatingOddsVisibility || _gameResponse is null)
        {
            return;
        }

        _isUpdatingOddsVisibility = true;
        var nextVisibility = !_gameResponse.AreOddsVisibleToAllPlayers;

        try
        {
            var request = new CardGames.Poker.Api.Contracts.ToggleOddsVisibilityRequest
            {
                AreOddsVisibleToAllPlayers = nextVisibility
            };

            var response = await GamesApiClient.ToggleOddsVisibilityAsync(GameId, request);

            if (response is { IsSuccessStatusCode: true, Content: not null })
            {
                _gameResponse = _gameResponse with
                {
                    AreOddsVisibleToAllPlayers = response.Content.AreOddsVisibleToAllPlayers
                };
            }
            else if (response.StatusCode == System.Net.HttpStatusCode.Forbidden)
            {
                await ShowToastAsync("Only the table host can change odds visibility.", "error");
            }
            else if (response.StatusCode == System.Net.HttpStatusCode.Conflict)
            {
                await ShowToastAsync("This game has ended and odds visibility can no longer be changed.", "warning");
            }
            else
            {
                await ShowToastAsync("Unable to update odds visibility.", "error");
            }
        }
        catch (Exception ex)
        {
            Logger.LogWarning(ex, "Failed to update odds visibility for game {GameId}", GameId);
            await ShowToastAsync("Unable to update odds visibility.", "error");
        }
        finally
        {
            _isUpdatingOddsVisibility = false;
            await InvokeStateHasChangedAsync();
        }
    }

    private async Task HandleTableSettingsUpdatedAsync(TableSettingsUpdatedDto notification)
    {
        if (notification.GameId != GameId)
        {
            return;
        }

        await ApplyRefreshPolicyAsync(TableInteraction.TableSettingsPush);
        await InvokeStateHasChangedAsync();
    }

    private async Task RefreshOddsVisibilityFromServerAsync()
    {
        if (_gameResponse is null)
        {
            return;
        }

        var gameResponse = await GamesApiClient.GetGameAsync(GameId);
        if (gameResponse is { IsSuccessStatusCode: true, Content: not null })
        {
            _gameResponse = _gameResponse with
            {
                AreOddsVisibleToAllPlayers = gameResponse.Content.AreOddsVisibleToAllPlayers,
                RowVersion = gameResponse.Content.RowVersion
            };
            _allowedDealerChoiceGameCodes = ParseAllowedDealerChoiceGameCodes(gameResponse.Content.GameSettings);
        }
    }

    private static IReadOnlyList<string>? ParseAllowedDealerChoiceGameCodes(string? gameSettings)
    {
        if (string.IsNullOrWhiteSpace(gameSettings))
        {
            return null;
        }

        try
        {
            using var document = JsonDocument.Parse(gameSettings);
            if (!document.RootElement.TryGetProperty("allowedDealerChoiceGameCodes", out var allowedGameCodesElement)
                || allowedGameCodesElement.ValueKind != JsonValueKind.Array)
            {
                return null;
            }

            var allowedGameCodes = allowedGameCodesElement
                .EnumerateArray()
                .Select(element => element.GetString()?.Trim())
                .Where(code => !string.IsNullOrWhiteSpace(code))
                .Select(code => code!)
                .Distinct(StringComparer.OrdinalIgnoreCase)
                .OrderBy(code => code, StringComparer.OrdinalIgnoreCase)
                .ToArray();

            return allowedGameCodes.Length > 0 ? allowedGameCodes : null;
        }
        catch (JsonException)
        {
            return null;
        }
    }

    private async Task ResumeGameAsync()
    {
        if (!_isHost)
            return;

        // TODO: Send resume request to server
        _isPaused = false;
        await Task.CompletedTask;
    }

    private async Task EndGameAsync()
    {
        if (!_isHost)
            return;

        //TODO: Send end game request to server
        //_gameResponse.CurrentPhase = GamePhase.Ended;
        await Task.CompletedTask;
    }

    private async Task HandlePlayerActionAsync(PlayerActionRequest action)
    {
        if (!_isPlayerTurn || _isSubmittingAction)
            return;

        _isSubmittingAction = true;

        try
        {
            var request = new ProcessBettingActionRequest(action.Action, action.Amount ?? 0);
            var response = await GameApiRouter.ProcessBettingActionAsync(_gameTypeCode ?? "", GameId, request);

            if (!response.IsSuccess)
            {
                // When the server-side auto-action timer fires at the same time the client
                // submits an action, the server may have already advanced the game to a
                // resolution phase (Showdown/Complete/Ended). In that case the betting
                // endpoint returns an error because there is no active betting round.
                // Suppress the toast — SignalR will broadcast the updated state and the
                // showdown overlay will appear normally.
                // Check both the client-side phase (may already be updated via SignalR) and
                // the error message itself (the SignalR update may not have arrived yet).
                if (IsShowdownPhase || IsCompletePhase || IsEndedPhase ||
                    IsResolutionPhaseError(response.Error))
                {
                    Logger.LogDebug(
                        "Betting action failed but game is already in resolution phase ({Phase}); suppressing error toast",
                        _gameResponse?.CurrentPhase);
                    return;
                }

                var errorMessage = ExtractJsonErrorMessage(response.Error) ?? "An unknown error occurred.";
                await ShowToastAsync(errorMessage, "error");
                return;
            }

            // Betting action: TableRefreshPolicy = HubDriven. The server broadcasts the
            // updated table state over SignalR, so we deliberately do not refetch here
            // (see docs/TableRefreshPolicy.md).
        }
        finally
        {
            _isSubmittingAction = false;
        }
    }

    /// <summary>
    /// Extracts the "message" property from a JSON error body returned by the API.
    /// Returns null if the input is not valid JSON or has no "message" property.
    /// </summary>
    private static string? ExtractJsonErrorMessage(string? errorBody)
        => TableActionError.FromBody(errorBody);

    /// <summary>
    /// Checks whether an API error body indicates the game has already advanced
    /// to a resolution phase (Showdown, Complete, Ended). Used as a fallback when the
    /// SignalR state update has not yet arrived on the client.
    /// </summary>
    private static bool IsResolutionPhaseError(string? errorBody)
    {
        if (string.IsNullOrWhiteSpace(errorBody))
        {
            return false;
        }

        var message = ExtractJsonErrorMessage(errorBody) ?? errorBody;
        return message.Contains("'Showdown'", StringComparison.OrdinalIgnoreCase) ||
               message.Contains("'Complete'", StringComparison.OrdinalIgnoreCase) ||
               message.Contains("'Ended'", StringComparison.OrdinalIgnoreCase) ||
               message.Contains("'KlondikeReveal'", StringComparison.OrdinalIgnoreCase);
    }

    /// <summary>
    /// Extracts the error message from a Refit API exception.
    /// The API returns errors as JSON objects with a "message" property.
    /// </summary>
    private string ExtractErrorMessage(Refit.ApiException? exception)
        => TableActionError.FromException(exception);

    private void ResetDrawPanelStateForNewHand()
    {
        _selectedDiscardIndices.Clear();
        _currentPlayerDrawCards = [];
        _pendingCurrentPlayerDrawCards = null;
        _cardsDrawn = false;
        _newHandDescription = null;
        _isPlayerDrawTurn = false;
        _isDrawAnimating = false;
        _isDiscardingCards = false;
        _isDrawingNewCards = false;
        _isSubmittingDraw = false;
        _forceShowDrawPanel = false;
        _newCardIndices = [];
    }

    private bool IsDrawPanelAnimationInProgress =>
        _isDrawAnimating || _isDiscardingCards || _isDrawingNewCards;

    private void UpdateCurrentPlayerDrawCardsFromPrivateState(IReadOnlyList<CardPrivateDto> hand, bool preserveSelection)
    {
        var cards = BuildCurrentPlayerDrawCards(hand, preserveSelection);

        if (IsDrawPanelAnimationInProgress)
        {
            _pendingCurrentPlayerDrawCards = cards;
            return;
        }

        _currentPlayerDrawCards = cards;
        _pendingCurrentPlayerDrawCards = null;
    }

    private void ApplyPendingCurrentPlayerDrawCardsIfReady()
    {
        if (_pendingCurrentPlayerDrawCards is null || IsDrawPanelAnimationInProgress)
        {
            return;
        }

        _currentPlayerDrawCards = _pendingCurrentPlayerDrawCards;
        _pendingCurrentPlayerDrawCards = null;

        _selectedDiscardIndices.Clear();
        _newCardIndices = [];
    }

    private async Task HandleDrawActionAsync(List<int> discardIndices)
    {
        if (!_isPlayerDrawTurn || _isSubmittingDraw)
            return;

        _selectedDiscardIndices.Clear();
        foreach (var i in discardIndices)
        {
            _selectedDiscardIndices.Add(i);
        }

        _isSubmittingDraw = true;
        _isDrawAnimating = true;
        _forceShowDrawPanel = true;
        _delayShowdownOverlay = true; // Delay showdown until animation completes
        _cardsDrawn = false;
        _newHandDescription = null;

        // Initialize draw cards for animation
        _currentPlayerDrawCards = _currentPlayerDrawCards.Select(c => c with { IsSelected = false }).ToList();
        foreach (var index in discardIndices)
        {
            if (index >= 0 && index < _currentPlayerDrawCards.Count)
            {
                _currentPlayerDrawCards[index] = _currentPlayerDrawCards[index] with { IsSelected = true };
            }
        }

        StateHasChanged();

        try
        {
            // Start discard animation
            if (discardIndices.Count > 0)
            {
                _isDiscardingCards = true;

                // Mark selected cards visually for animation
                for (int i = 0; i < _currentPlayerDrawCards.Count; i++)
                {
                    if (discardIndices.Contains(i))
                    {
                        _currentPlayerDrawCards[i] = _currentPlayerDrawCards[i] with { IsSelected = true };
                    }
                }
                await InvokeStateHasChangedAsync();

                // Wait for discard animation
                await Task.Delay(600);
            }

            // Make the API call
            var playerId = _currentPlayerId ?? Guid.Empty;
            var response = await GameApiRouter.ProcessDrawAsync(_gameTypeCode ?? "", GameId, playerId, _currentPlayerSeatIndex, discardIndices);

            if (!response.IsSuccess || response.Content is null)
            {
                // Reset state on failure
                _isDiscardingCards = false;
                _isDrawAnimating = false;
                _isSubmittingDraw = false;
                Logger.LogWarning("Draw action failed: {Error}", response.Error);
                return;
            }

            var result = response.Content.Original;
            if (!string.IsNullOrEmpty(response.Content.NewHandDescription))
            {
                _newHandDescription = FormatHandDescriptionForDisplay(response.Content.NewHandDescription);
            }

            // Transition to drawing new cards animation
            _isDiscardingCards = false;

            // Handle stand pat (0 discards) case
            if (discardIndices.Count == 0)
            {
                // No cards to animate - just mark as complete
                _cardsDrawn = true;
                await InvokeStateHasChangedAsync();

                // If draw phase is complete (we're now in DrawComplete phase), keep cards visible
                if (result?.DrawComplete == true)
                {
                    _isDrawAnimating = false;
                    _forceShowDrawPanel = false;
                    _delayShowdownOverlay = false;
                    // Cards stay in currentPlayerDrawCards for DrawComplete phase display
                    await InvokeStateHasChangedAsync();

                    // Wait for the background service to transition to Showdown/Complete phase
                    // The background service has a 5-second delay, so we may need to wait
                    for (int i = 0; i < 10; i++)
                    {
                        if (IsShowdownPhase)
                        {
                            break;
                        }
                        await Task.Delay(500);
                    }

                    // Now try to load/show showdown
                    await TryLoadShowdownAsync();
                    return;
                }

                // Not the last player - no delay for stand pat
                // await Task.Delay(500);
                _isDrawAnimating = false;
                _currentPlayerDrawCards = [];
                _selectedDiscardIndices.Clear();
                _forceShowDrawPanel = false;
                _cardsDrawn = false;
                _newHandDescription = null;
                await InvokeStateHasChangedAsync();
                return;
            }

            // Handle discards (1+ cards) case
            if (discardIndices.Count > 0)
            {
                // Swap in the newly dealt cards before the draw-in animation starts
                // so the cards that animate back in display the new values.
                if (result is not null)
                {
                    var newCards = result.NewCards?.ToList() ?? [];

                    if (newCards.Count == 0)
                    {
                        // Discard-only game (e.g. Irish Hold 'Em): no replacement cards are drawn.
                        // Remove the discarded cards so they don't animate back in.
                        _currentPlayerDrawCards = _currentPlayerDrawCards
                            .Where((_, i) => !discardIndices.Contains(i))
                            .ToList();
                        _isDiscardingCards = false;
                        _cardsDrawn = true;
                        await InvokeStateHasChangedAsync();

                        // Bob Barker transitions straight into betting after the showcase pick,
                        // so don't hold the selection panel open once the server has accepted it.
                        await Task.Delay(IsBobBarker ? 200 : 3000);
                    }
                    else
                    {
                        for (var i = 0; i < discardIndices.Count && i < newCards.Count; i++)
                        {
                            var targetIndex = discardIndices[i];
                            if (targetIndex < 0 || targetIndex >= _currentPlayerDrawCards.Count)
                            {
                                continue;
                            }

                            var newCard = newCards[i];
                            var rank = newCard.Symbol?.ToCardRankString();
                            var suit = newCard.Suit?.ToCardSuitString();
                            var previousCard = _currentPlayerDrawCards[targetIndex];

                            _currentPlayerDrawCards[targetIndex] = new CardInfo
                            {
                                Rank = rank,
                                Suit = suit,
                                IsFaceUp = true,
                                IsPubliclyVisible = true,
                                IsSelected = false,
                                DealOrder = previousCard.DealOrder,
                                IsWild = IsWildForCurrentPlayerCard(rank, suit)
                            };
                        }

                        ApplyKingsAndLowsLowestCardWildFallback(_currentPlayerDrawCards);

                        await InvokeStateHasChangedAsync();

                        _isDrawingNewCards = true;
                        _newCardIndices = discardIndices;
                        await InvokeStateHasChangedAsync();

                        // Wait for the draw-in animation to complete, then switch the status out of "Drawing...".
                        await Task.Delay(700);
                        _cardsDrawn = true;
                        await InvokeStateHasChangedAsync();

                        // Keep the panel visible for 5 seconds so the player can see their new cards
                        await Task.Delay(5000);

                        _isDrawingNewCards = false;
                        _newCardIndices = [];
                    }
                }
            }

            // If draw phase is complete (we're now in DrawComplete phase), keep the cards visible
            // and let SignalR updates handle the transition to showdown
            if (result?.DrawComplete == true)
            {
                // Don't clear the cards - they'll be displayed during DrawComplete phase
                _isDrawAnimating = false;
                _forceShowDrawPanel = false;
                _cardsDrawn = false;
                _delayShowdownOverlay = false;
                // Cards stay in currentPlayerDrawCards for DrawComplete phase display
                await InvokeStateHasChangedAsync();

                // Wait for the background service to transition to Showdown/Complete phase
                // The background service has a 5-second delay matching our animation delay,
                // so we may need to wait a bit longer for the phase transition
                for (int i = 0; i < 10; i++)
                {
                    if (IsShowdownPhase)
                    {
                        break;
                    }
                    await Task.Delay(500);
                }

                // Now try to load/show showdown
                await TryLoadShowdownAsync();
                return;
            }

            // Clear animation state (for non-last players)
            _isDrawAnimating = false;
            _currentPlayerDrawCards = [];
            _selectedDiscardIndices.Clear();
            _forceShowDrawPanel = false;
            _cardsDrawn = false;
            _newHandDescription = null;

            // Now show the showdown overlay after animation is complete
            _delayShowdownOverlay = false;
            await TryLoadShowdownAsync();

            // SignalR will broadcast the updated state - no need to refresh manually
        }
        finally
        {
            _isSubmittingDraw = false;
            _isDiscardingCards = false;
            _isDrawingNewCards = false;
            _isDrawAnimating = false;
            _delayShowdownOverlay = false; // Clear delay on completion or error

            // Only clear draw panel state if we're not in DrawComplete phase
            // During DrawComplete, we want the cards to remain visible
            if (!IsDrawCompletePhase)
            {
                _forceShowDrawPanel = false;
                _cardsDrawn = false;
                _newHandDescription = null;
            }

            ApplyPendingCurrentPlayerDrawCardsIfReady();
        }
    }

    private Task ToggleDiscardSelectionAsync(int index)
    {
        if (index < 0 || index >= _currentPlayerDrawCards.Count)
        {
            return Task.CompletedTask;
        }

        if (!_selectedDiscardIndices.Add(index))
        {
            _selectedDiscardIndices.Remove(index);
        }

        _currentPlayerDrawCards[index] = _currentPlayerDrawCards[index] with
        {
            IsSelected = _selectedDiscardIndices.Contains(index)
        };

        // Recalculate odds based on cards being kept (not selected for discard)
        RecalculateDrawOddsForSelection();

        return InvokeStateHasChangedAsync();
    }

    /// <summary>
    /// Handles the draw phase timer expiry for games where standing pat is invalid
    /// (e.g., Irish Hold 'Em requires exactly 2 discards).
    /// Clears the draw panel immediately for responsive UI; the server-side auto-action
    /// handles the actual fold and broadcasts the updated state.
    /// </summary>
    private async Task HandleDrawTimeoutFoldAsync()
    {
        Logger.LogInformation("Draw timer expired — folding via timeout for game {GameId}", GameId);

        try
        {
            var response = await GameApiRouter.FoldDuringDrawAsync(GameId, _currentPlayerSeatIndex);
            if (!response.IsSuccess)
            {
                Logger.LogWarning("Fold-during-draw API call failed for game {GameId}: {Error}", GameId, response.Error);
            }
        }
        catch (Exception ex)
        {
            Logger.LogWarning(ex, "Fold-during-draw API call threw for game {GameId}; server auto-action will handle it", GameId);
        }

        ResetDrawPanelStateForNewHand();
        await InvokeStateHasChangedAsync();
    }

    /// <summary>
    /// Returns the OnTimeoutFold callback for games that fold on draw timeout, or default for others.
    /// </summary>
    private EventCallback GetDrawTimeoutFoldCallback()
    {
        return IsIrishStyleHoldEm
            ? EventCallback.Factory.Create(this, HandleDrawTimeoutFoldAsync)
            : default;
    }

    /// <summary>
    /// Recalculates the hand odds based on the current discard selection.
    /// Cards not selected for discard are kept, and the odds are calculated
    /// by simulating drawing replacement cards for the discarded ones.
    /// </summary>
    private void RecalculateDrawOddsForSelection()
    {
        try
        {
            // Get the cards being kept (not selected for discard)
            var cardsToKeep = _currentPlayerDrawCards
            .Where((card, index) => !_selectedDiscardIndices.Contains(index))
            .Select(c => ConvertToCard(c.Rank ?? "", c.Suit ?? ""))
            .Where(c => c is not null)
            .Cast<Card>()
            .ToList();

            if (cardsToKeep.Count == 0)
            {
                handOdds = [];
                return;
            }

            // Get dead cards: cards selected for discard + folded players' visible cards
            var discardedCards = _currentPlayerDrawCards
            .Where((card, index) => _selectedDiscardIndices.Contains(index))
            .Select(c => ConvertToCard(c.Rank ?? "", c.Suit ?? ""))
            .Where(c => c is not null)
            .Cast<Card>()
            .ToList();

            var foldedPlayerCards = _seats
            .Where(s => s.IsFolded && s.Cards.Any())
            .SelectMany(s => s.Cards)
            .Where(c => c.IsFaceUp && c.Rank is not null && c.Suit is not null)
            .Select(c => ConvertToCard(c.Rank!, c.Suit!))
            .Where(c => c is not null)
            .Cast<Card>()
            .ToList();

            // Discarded cards are dead cards (they won't be redrawn)
            var deadCards = discardedCards.Concat(foldedPlayerCards).ToList();

            OddsCalculator.OddsResult? result;
            if (IsTwosJacksManWithTheAxe)
            {
                result = OddsCalculator.CalculateTwosJacksManWithTheAxeDrawOdds(cardsToKeep, deadCards);
            }
            else if (IsSevenCardStudStyle)
            {
                result = OddsCalculator.CalculateStudOdds(cardsToKeep, deadCards);
            }
            else if (IsRazz)
            {
                result = OddsCalculator.CalculateRazzOdds(cardsToKeep, deadCards);
            }
            else if (IsKingsAndLows)
            {
                result = OddsCalculator.CalculateKingsAndLowsDrawOdds(cardsToKeep, deadCards);
            }
            else
            {
                result = OddsCalculator.CalculateDrawOdds(cardsToKeep, deadCards);
            }

            handOdds = ConvertToHandTypeOddsDtos(result);
        }
        catch (Exception ex)
        {
            Logger.LogWarning(ex, "Failed to recalculate hand odds for discard selection");
            handOdds = [];
        }
    }

    /// <summary>
    /// Gets the leaderboard players from the current seats.
    /// </summary>
    private IReadOnlyList<LeaderboardSection.LeaderboardPlayer> GetLeaderboardPlayers()
    {
        return _seats
        .Where(s => s.IsOccupied && s.PlayerName is not null)
        .Select(s => new LeaderboardSection.LeaderboardPlayer(
        PlayerId: s.SeatIndex.ToString(),
        PlayerName: s.PlayerName!,
        DisplayName: s.PlayerFirstName,
        Chips: s.Chips))
        .OrderByDescending(p => p.Chips)
        .ToList();
    }

    private static string GetInitials(string? name)
    {
        if (string.IsNullOrWhiteSpace(name))
            return "?";

        var parts = name.Split(' ', StringSplitOptions.RemoveEmptyEntries);
        return parts.Length switch
        {
            0 => "?",
            1 => parts[0][..1].ToUpperInvariant(),
            _ => $"{parts[0][0]}{parts[^1][0]}".ToUpperInvariant()
        };
    }

    // Enums and Models
    public enum GamePhase
    {
        [Description("Waiting for Players")] WaitingForPlayers,
        [Description("Betting Round")] Dealing,
        [Description("Pre-Draw")] PreDraw,
        [Description("Drawing")] Drawing,
        [Description("Post-Draw")] PostDraw,
        [Description("Showdown")] Showdown,
        [Description("Ended")] Ended,
        [Description("Waiting to Start")] WaitingToStart,
        [Description("Complete")] Complete,
        [Description("Second Betting Round")] SecondBettingRound,
        [Description("Draw Phase")] DrawPhase,
        [Description("First Betting Round")] FirstBettingRound,
        [Description("Collecting Antes")] CollectingAntes,
        [Description("Drop or Stay")] DropOrStay,
        [Description("Pot Matching")] PotMatching,
        [Description("Waiting for Dealer Choice")] WaitingForDealerChoice,
    }

    public record SeatInfo
    {
        public int SeatIndex { get; init; }
        public bool IsOccupied { get; set; }
        public string? PlayerName { get; set; }
        public string? PlayerFirstName { get; set; }
        public string? PlayerAvatarUrl { get; set; }
        public int Chips { get; set; }
        public bool IsReady { get; set; }
        public bool IsCurrentPlayer { get; set; }
        public bool IsFolded { get; set; }
        public bool IsAllIn { get; set; }
        public bool IsDisconnected { get; set; }
        public bool IsSittingOut { get; set; }
        public string? SittingOutReason { get; set; }
        public int CurrentBet { get; set; }
        public bool HasDecidedDropOrStay { get; set; }
        public List<CardInfo> Cards { get; set; } = [];
        public string? HandEvaluationDescription { get; set; }
        /// <summary>
        /// The last action performed by this player (e.g., "Checked", "Raised 50").
        /// Displayed temporarily in the seat pill.
        /// </summary>
        public string? LastActionDescription { get; set; }
        /// <summary>
        /// The UTC time when the last action was performed.
        /// Used to determine when to hide the action display.
        /// </summary>
        public DateTimeOffset? LastActionTime { get; set; }
    }

    public record CardInfo
    {
        public string? Rank { get; init; }
        public string? Suit { get; init; }
        public bool IsFaceUp { get; init; }
        public bool IsPubliclyVisible { get; init; }
        public bool IsSelected { get; init; }
        public int DealOrder { get; init; }
        public bool IsWild { get; init; }
        public bool IsShowcaseCard { get; init; }
    }

    public record PlayerActionRequest(BettingActionType Action, int? Amount);

    public record ToastMessage(string Message, string Type);

    private void StartCountdownTimer()
    {
        StopCountdownTimer();
        RefreshCountdownSeconds();

        if (_countdownDeadlineUtc is null)
        {
            return;
        }

        _countdownTimer = new System.Threading.Timer(async _ =>
        {
            RefreshCountdownSeconds();

            if (_secondsUntilNextHand > 0)
            {
                await InvokeStateHasChangedAsync();
            }
            else
            {
                StopCountdownTimer();
                _countdownDeadlineUtc = null;

                // If game is ended and timer reached 0, redirect to lobby
                if (IsEndedPhase || IsGameCompleted)
                {
                    await GameHubClient.LeaveGameAsync(GameId);
                    NavigateToExitDestination();
                }
                else
                {
                    // Countdown complete - hide the showdown overlay
                    // If we're paused for chip check, the ChipCoveragePauseOverlay will show instead
                    _showShowdownOverlay = false;
                    await InvokeStateHasChangedAsync();
                }
            }
        }, null, GetCountdownTimerDueTime(), TimeSpan.FromSeconds(1));
    }

    private void StopCountdownTimer()
    {
        _countdownTimer?.Dispose();
        _countdownTimer = null;
    }

    private void SyncCountdownWithServer(TableStatePublicDto state)
    {
        _serverClockOffset = state.ServerUtcNow - DateTimeOffset.UtcNow;
        _countdownDeadlineUtc = ResolveCountdownDeadlineUtc(state, IsEndedPhase, IsGameCompleted);
        RefreshCountdownSeconds();
    }

    private void RefreshCountdownFromCurrentTableState()
    {
        if (_tableState is null)
        {
            _countdownDeadlineUtc = null;
            _secondsUntilNextHand = 0;
            return;
        }

        SyncCountdownWithServer(_tableState);
    }

    private bool ShouldStartHoldEmFamilyGameViaGenericEndpoint()
    {
        return IsLeagueTournament && (IsHoldEm || IsRedRiver || IsKlondike);
    }

    private static DateTimeOffset? ResolveCountdownDeadlineUtc(TableStatePublicDto state, bool isEndedPhase, bool isGameCompleted)
    {
        if (state.NextHandStartsAtUtc.HasValue)
        {
            return state.NextHandStartsAtUtc.Value;
        }

        if ((isEndedPhase || isGameCompleted) && state.HandCompletedAtUtc.HasValue)
        {
            return state.HandCompletedAtUtc.Value.AddSeconds(CompletedGameReturnToLobbyDurationSeconds);
        }

        return null;
    }

    private void RefreshCountdownSeconds()
    {
        _secondsUntilNextHand = CalculateSecondsUntilDeadline(_countdownDeadlineUtc, DateTimeOffset.UtcNow, _serverClockOffset);
    }

    private static int CalculateSecondsUntilDeadline(DateTimeOffset? deadlineUtc, DateTimeOffset clientUtcNow, TimeSpan serverClockOffset)
    {
        if (!deadlineUtc.HasValue)
        {
            return 0;
        }

        var adjustedServerNow = clientUtcNow + serverClockOffset;
        var remaining = deadlineUtc.Value - adjustedServerNow;
        return Math.Max(0, (int)Math.Ceiling(remaining.TotalSeconds));
    }

    private TimeSpan GetCountdownTimerDueTime()
    {
        if (!_countdownDeadlineUtc.HasValue)
        {
            return Timeout.InfiniteTimeSpan;
        }

        var adjustedServerNow = DateTimeOffset.UtcNow + _serverClockOffset;
        var remaining = _countdownDeadlineUtc.Value - adjustedServerNow;

        if (remaining <= TimeSpan.Zero)
        {
            return TimeSpan.Zero;
        }

        var millisecondsUntilNextBoundary = remaining.TotalMilliseconds % 1000;
        if (millisecondsUntilNextBoundary < 1)
        {
            millisecondsUntilNextBoundary = 1000;
        }

        return TimeSpan.FromMilliseconds(millisecondsUntilNextBoundary);
    }

    public ValueTask DisposeAsync()
    {
        _disposed = true;

        // Stop countdown timer
        StopCountdownTimer();

        // Stop runout animation timer
        _runoutDealTimer?.Dispose();
        _runoutDealTimer = null;

        // Stop card deal animation timer
        _dealTimer?.Dispose();
        _dealTimer = null;

        // Stop community card reveal timer
        _communityCardRevealTimer?.Dispose();
        _communityCardRevealTimer = null;

        // Stop flying card timer
        _flyingCardTimer?.Dispose();
        _flyingCardTimer = null;
        _flyingCard = null;

        // Unsubscribe from SignalR events
        GameHubClient.OnTableStateUpdated -= HandleTableStateUpdatedAsync;
        GameHubClient.OnPrivateStateUpdated -= HandlePrivateStateUpdatedAsync;
        GameHubClient.OnPlayerJoined -= HandlePlayerJoinedAsync;
        GameHubClient.OnConnectionStateChanged -= HandleConnectionStateChanged;
        GameHubClient.OnActionTimerUpdated -= HandleActionTimerUpdatedAsync;
        GameHubClient.OnPlayerActionPerformed -= HandlePlayerActionPerformedAsync;
        GameHubClient.OnTableSettingsUpdated -= HandleTableSettingsUpdatedAsync;
        GameHubClient.OnTableToastNotification -= HandleTableToastNotificationAsync;
        NotificationHubClient.OnGameJoinRequestResolved -= HandleJoinRequestResolvedAsync;

        // Leave the game and disconnect
        if (_currentGameId.HasValue)
        {
            _ = LeaveGameSilentlyAsync(_currentGameId.Value);
        }

        return ValueTask.CompletedTask;
    }

    private async Task LeaveGameSilentlyAsync(Guid gameId)
    {
        try
        {
            await GameHubClient.LeaveGameAsync(gameId);
        }
        catch
        {
            // Ignore errors during navigation cleanup.
        }
    }

    private async Task InvokeStateHasChangedAsync()
    {
        if (_disposed) return;
        try
        {
            await InvokeAsync(StateHasChanged);
        }
        catch (ObjectDisposedException)
        {
            // Component was disposed between the check and the call — safe to ignore.
        }
    }

    private static string? TryGetHeaderValue(System.Net.Http.Headers.HttpResponseHeaders headers, string headerName)
    {
        return headers.TryGetValues(headerName, out var values)
            ? values.FirstOrDefault()
            : null;
    }

    /// <summary>
    /// Handles action timer updates from SignalR.
    /// </summary>
    private async Task HandleActionTimerUpdatedAsync(ActionTimerStateDto timerState)
    {
        _actionTimerState = timerState;
        await InvokeStateHasChangedAsync();
    }

    /// <summary>
    /// Handles player action performed notifications from SignalR.
    /// Updates the seat's last action display and starts a timer to clear it.
    /// </summary>
    private async Task HandlePlayerActionPerformedAsync(PlayerActionPerformedDto notification)
    {
        if (notification.GameId != GameId)
        {
            return;
        }

        var seatIndex = notification.SeatIndex;
        if (seatIndex < 0 || seatIndex >= _seats.Count)
        {
            return;
        }

        Logger.LogDebug("Player action performed at seat {SeatIndex}: {Action}",
        seatIndex, notification.ActionDescription);

        // Update the seat with the action
        _seats[seatIndex].LastActionDescription = notification.ActionDescription;
        _seats[seatIndex].LastActionTime = notification.PerformedAtUtc;

        // Keep/Trade is a hand-long decision in Screw Your Neighbor and should remain visible.
        if (IsScrewYourNeighbor)
        {
            var screwYourNeighborDecision = GetScrewYourNeighborDecisionDescription(notification.ActionDescription);
            if (!string.IsNullOrWhiteSpace(screwYourNeighborDecision))
            {
                _seats[seatIndex].HandEvaluationDescription = screwYourNeighborDecision;
            }
        }

        await InvokeStateHasChangedAsync();

        if (IsScrewYourNeighbor && IsScrewYourNeighborTradeAction(notification.ActionDescription))
        {
            QueueScrewYourNeighborTradeAnimation(seatIndex);
        }

        // Schedule the action to be cleared after the display duration
        var displayDuration = notification.DisplayDurationSeconds > 0
        ? notification.DisplayDurationSeconds
        : 5;

        _ = Task.Run(async () =>
        {
            await Task.Delay(TimeSpan.FromSeconds(displayDuration));

            // Only clear if this is still the same action (hasn't been replaced)
            if (seatIndex >= 0 && seatIndex < _seats.Count &&
    _seats[seatIndex].LastActionTime == notification.PerformedAtUtc)
            {
                _seats[seatIndex].LastActionDescription = null;
                _seats[seatIndex].LastActionTime = null;
                await InvokeStateHasChangedAsync();
            }
        });
    }

    private static string? GetScrewYourNeighborDecisionDescription(string? actionDescription)
    {
        if (string.IsNullOrWhiteSpace(actionDescription))
        {
            return null;
        }

        var normalizedDescription = actionDescription.Trim();

        if (string.Equals(normalizedDescription, "Trade", StringComparison.OrdinalIgnoreCase) ||
            string.Equals(normalizedDescription, "Traded", StringComparison.OrdinalIgnoreCase))
        {
            return "Traded";
        }

        if (string.Equals(normalizedDescription, "Keep", StringComparison.OrdinalIgnoreCase) ||
            string.Equals(normalizedDescription, "Kept", StringComparison.OrdinalIgnoreCase))
        {
            return "Kept";
        }

        return null;
    }

    private static bool IsScrewYourNeighborTradeAction(string? actionDescription)
    {
        return string.Equals(
            GetScrewYourNeighborDecisionDescription(actionDescription),
            "Traded",
            StringComparison.OrdinalIgnoreCase);
    }

    private async Task AnimateScrewYourNeighborTradeAsync(int sourceSeatIndex)
    {
        if (!_isJsInteropReady)
        {
            return;
        }

        var isDealerTrade = sourceSeatIndex == _dealerSeatIndex;

        if (!isDealerTrade && !TryGetScrewYourNeighborTradeTarget(sourceSeatIndex, out _))
        {
            return;
        }

        try
        {
            if (isDealerTrade)
            {
                // Dealer trades with the deck — animate card from deck to dealer's seat
                await JSRuntime.InvokeVoidAsync(
                    "cardGamesTable.animateScrewYourNeighborDeckTrade",
                    sourceSeatIndex,
                    ScrewYourNeighborTradeAnimationDurationMs);
            }
            else
            {
                TryGetScrewYourNeighborTradeTarget(sourceSeatIndex, out var targetSeatIndex);
                // Non-dealer trades with the next player — animate cards flying between seats
                await JSRuntime.InvokeVoidAsync(
                    "cardGamesTable.animateScrewYourNeighborTrade",
                    sourceSeatIndex,
                    targetSeatIndex,
                    ScrewYourNeighborTradeAnimationDurationMs);
            }
        }
        catch (JSDisconnectedException)
        {
            // Circuit ended before animation could execute.
        }
        catch (TaskCanceledException ex)
        {
            Logger.LogDebug(ex,
                "Skipped Screw Your Neighbor trade animation after JS interop was canceled for seat {SourceSeatIndex}",
                sourceSeatIndex);
        }
        catch (InvalidOperationException ex) when (ex.Message.Contains("statically rendered", StringComparison.OrdinalIgnoreCase))
        {
            Logger.LogDebug(ex,
                "Skipped Screw Your Neighbor trade animation during prerender for seat {SourceSeatIndex}",
                sourceSeatIndex);
        }
        catch (JSException ex)
        {
            Logger.LogDebug(ex,
                "Failed to run Screw Your Neighbor trade animation for seat {SourceSeatIndex}",
                sourceSeatIndex);
        }
    }

    private void QueueScrewYourNeighborTradeAnimation(int sourceSeatIndex)
    {
        var isDealerTrade = sourceSeatIndex == _dealerSeatIndex;

        // For non-dealer trades, verify there is a valid trade target
        if (!isDealerTrade && !TryGetScrewYourNeighborTradeTarget(sourceSeatIndex, out _))
        {
            return;
        }

        var handNumber = _tableState?.CurrentHandNumber ?? _gameResponse?.CurrentHandNumber ?? 0;
        var tradeKey = $"{handNumber}:{sourceSeatIndex}";
        if (!_synAnimatedTradeKeys.Add(tradeKey))
        {
            return;
        }

        if (!_isJsInteropReady)
        {
            _pendingSynTradeAnimationSeatIndices.Add(sourceSeatIndex);
            return;
        }

        // Fire-and-forget: the JS animation creates its own synthetic card elements
        // positioned over the seats (or deck) and flies them. It does not depend on the
        // current DOM card state, so it works regardless of when Blazor re-renders.
        _ = InvokeAsync(() => AnimateScrewYourNeighborTradeAsync(sourceSeatIndex));
    }

    private async Task FlushPendingScrewYourNeighborTradeAnimationsAsync()
    {
        if (!_isJsInteropReady || _pendingSynTradeAnimationSeatIndices.Count == 0)
        {
            return;
        }

        var pending = _pendingSynTradeAnimationSeatIndices.ToList();
        _pendingSynTradeAnimationSeatIndices.Clear();

        foreach (var sourceSeatIndex in pending)
        {
            await AnimateScrewYourNeighborTradeAsync(sourceSeatIndex);
        }
    }

    /// <summary>
    /// Animates a poker chip flying from each losing player's seat to the pot display.
    /// </summary>
    private const int ScrewYourNeighborChipAnimationDelayMs = 3000;

    private async Task AnimateScrewYourNeighborChipLossAsync(List<int> loserSeatIndices)
    {
        if (!_isJsInteropReady)
        {
            ClearSynBonusChips();
            return;
        }

        Logger.LogInformation(
            "AnimateScrewYourNeighborChipLossAsync: animating chip loss for {Count} loser(s) at seats [{Seats}]",
            loserSeatIndices.Count, string.Join(", ", loserSeatIndices));

        // Wait after winner/loser is revealed before animating the chip to the pot
        await Task.Delay(ScrewYourNeighborChipAnimationDelayMs);

        var staggerDelayMs = 0;
        foreach (var seatIndex in loserSeatIndices)
        {
            try
            {
                await JSRuntime.InvokeVoidAsync(
                    "cardGamesTable.animateSeatLossExplosion",
                    seatIndex,
                    staggerDelayMs);

                await JSRuntime.InvokeVoidAsync(
                    "cardGamesTable.animateChipToPot",
                    seatIndex,
                    ScrewYourNeighborTradeAnimationDurationMs,
                    staggerDelayMs);
            }
            catch (JSDisconnectedException)
            {
                ClearSynBonusChips();
                return;
            }
            catch (InvalidOperationException ex) when (ex.Message.Contains("statically rendered", StringComparison.OrdinalIgnoreCase))
            {
                ClearSynBonusChips();
                return;
            }
            catch (JSException ex)
            {
                Logger.LogDebug(ex, "Failed to animate chip loss for seat {SeatIndex}", seatIndex);
            }

            staggerDelayMs += 150;
        }

        // Wait for the last JS animation to complete before removing the bonus chips.
        // Total wait = last stagger delay + animation duration + small buffer.
        var lastStaggerMs = Math.Max(0, staggerDelayMs - 150);
        await Task.Delay(lastStaggerMs + ScrewYourNeighborTradeAnimationDurationMs + 100);

        ClearSynBonusChips();
        StateHasChanged();
    }

    private void ClearSynBonusChips()
    {
        _synBonusChipsBySeat.Clear();
    }

    private Guid? _currentGameId => GameId;

    /// <summary>
    /// Calculates the hand odds locally using the OddsCalculator.
    /// </summary>
    private void CalculateHandOdds(IReadOnlyList<CardPrivateDto> playerCards)
    {
        if (playerCards.Count == 0)
        {
            handOdds = [];
            return;
        }

        try
        {
            // Convert CardPrivateDto to Card
            var coreCards = playerCards
            .Select(c => ConvertToCard(c.Rank, c.Suit))
            .Where(c => c is not null)
            .Cast<Card>()
            .ToList();

            if (coreCards.Count == 0)
            {
                handOdds = [];
                return;
            }

            var communityCards = _tableState?.CommunityCards?
                .Where(c => c.IsFaceUp && !c.IsKlondikeCard && c.Rank is not null && c.Suit is not null)
                .Select(c => ConvertToCard(c.Rank!, c.Suit!))
                .Where(c => c is not null)
                .Cast<Card>()
                .ToList() ?? [];

            // Extract the revealed Klondike card (if any) for wild card odds calculation
            Card? klondikeCardForOdds = null;
            if (IsKlondike)
            {
                var klondikeDto = _tableState?.CommunityCards?
                    .FirstOrDefault(c => c.IsKlondikeCard && c.IsFaceUp && c.Rank is not null && c.Suit is not null);
                if (klondikeDto is not null)
                {
                    klondikeCardForOdds = ConvertToCard(klondikeDto.Rank!, klondikeDto.Suit!);
                }
            }

            // Get dead cards from folded players' visible cards
            var deadCards = _seats
            .Where(s => s.IsFolded && s.Cards.Any())
            .SelectMany(s => s.Cards)
            .Where(c => c.IsFaceUp && c.Rank is not null && c.Suit is not null)
            .Select(c => ConvertToCard(c.Rank!, c.Suit!))
            .Where(c => c is not null)
            .Cast<Card>()
            .ToList();

            var pairPressureFaceUpCardsInOrder = IsPairPressure
                ? GetPairPressureVisibleFaceUpCardsInOrder()
                : [];

            var pairPressurePlayers = IsPairPressure
                ? _seats
                    .Where(seat => seat.IsOccupied && seat.SeatIndex != _currentPlayerSeatIndex && seat.Cards.Count > 0)
                    .Select(seat => new OddsCalculator.PairPressurePlayerSnapshot
                    {
                        SeatIndex = seat.SeatIndex,
                        TotalCardsDealt = seat.Cards.Count,
                        FaceUpCardsInOrder = seat.Cards
                            .Where(card => card.IsFaceUp && card.IsPubliclyVisible && card.Rank is not null && card.Suit is not null)
                            .Select(card => ConvertToCard(card.Rank!, card.Suit!))
                            .Where(card => card is not null)
                            .Cast<Card>()
                            .ToList(),
                        ReceivesFutureCards = !seat.IsFolded && (!seat.IsSittingOut || seat.Cards.Count > 0)
                    })
                    .ToList()
                : [];

            if (IsPairPressure)
            {
                deadCards.AddRange(pairPressureFaceUpCardsInOrder.Except(coreCards));
            }

            var result = DashboardHandOddsCalculator.Calculate(
                _gameTypeCode,
                coreCards,
                communityCards,
                deadCards,
                pairPressureFaceUpCardsInOrder,
                pairPressurePlayers,
                _currentPlayerSeatIndex >= 0 ? _currentPlayerSeatIndex : null,
                _dealerSeatIndex,
                klondikeCardForOdds);

            handOdds = result is null
                ? []
                : ConvertToHandTypeOddsDtos(result);
        }
        catch (Exception ex)
        {
            Logger.LogWarning(ex, "Failed to calculate hand odds");
            handOdds = [];
        }
    }

    private static Card? ConvertToCard(string rank, string suit)
    {
        var symbolValue = rank.ToUpperInvariant() switch
        {
            "A" => 14,
            "K" => 13,
            "Q" => 12,
            "J" => 11,
            "10" or "T" => 10,
            "9" => 9,
            "8" => 8,
            "7" => 7,
            "6" => 6,
            "5" => 5,
            "4" => 4,
            "3" => 3,
            "2" => 2,
            _ => 0
        };

        if (symbolValue == 0)
        {
            return null;
        }

        var suitValue = suit.ToLowerInvariant() switch
        {
            "hearts" => Suit.Hearts,
            "diamonds" => Suit.Diamonds,
            "spades" => Suit.Spades,
            "clubs" => Suit.Clubs,
            _ => (Suit?)null
        };

        if (suitValue is null)
        {
            return null;
        }

        return new Card(suitValue.Value, symbolValue);
    }

    private static List<HandTypeOddsDto> ConvertToHandTypeOddsDtos(OddsCalculator.OddsResult result)
    {
        // Order hand types from best to worst for display
        var orderedHandTypes = new[]
        {
HandType.FiveOfAKind,
HandType.StraightFlush,
HandType.Quads,
HandType.FullHouse,
HandType.Flush,
HandType.Straight,
HandType.Trips,
HandType.TwoPair,
HandType.OnePair,
HandType.HighCard
};

        return orderedHandTypes
        .Where(ht => result.HandTypeProbabilities.ContainsKey(ht))
        .Select(ht => new HandTypeOddsDto(
        HandType: ht.ToString(),
        DisplayName: GetHandTypeDisplayName(ht),
        Probability: result.HandTypeProbabilities[ht]
        ))
        .ToList();
    }

    private static string GetHandTypeDisplayName(HandType handType) => handType switch
    {
        HandType.HighCard => "High Card",
        HandType.OnePair => "One Pair",
        HandType.TwoPair => "Two Pair",
        HandType.Trips => "Three of a Kind",
        HandType.Straight => "Straight",
        HandType.Flush => "Flush",
        HandType.FullHouse => "Full House",
        HandType.Quads => "Four of a Kind",
        HandType.StraightFlush => "Straight Flush",
        HandType.FiveOfAKind => "Five of a Kind",
        _ => handType.ToString()
    };

    private static string? BuildFallbackHandDescription(IReadOnlyList<CardPrivateDto> cards)
    {
        if (cards.Count == 0)
        {
            return null;
        }

        var rankGroups = cards
            .Where(c => !string.IsNullOrWhiteSpace(c.Rank))
            .Select(c => c.Rank!.Trim().ToUpperInvariant())
            .GroupBy(r => r)
            .Select(g => g.Count())
            .OrderByDescending(c => c)
            .ToList();

        if (rankGroups.Count == 0)
        {
            return null;
        }

        var maxGroup = rankGroups[0];
        var pairCount = rankGroups.Count(c => c >= 2);

        if (maxGroup >= 4)
        {
            return "Four of a Kind";
        }

        if (maxGroup == 3 && pairCount >= 2)
        {
            return "Full House";
        }

        if (maxGroup == 3)
        {
            return "Three of a Kind";
        }

        if (pairCount >= 2)
        {
            return "Two Pair";
        }

        if (pairCount == 1)
        {
            return "One Pair";
        }

        var highRank = cards
            .Where(c => !string.IsNullOrWhiteSpace(c.Rank))
            .Select(c => ParseRankValue(c.Rank!))
            .DefaultIfEmpty(0)
            .Max();

        return highRank > 0 ? $"{GetRankDisplayName(highRank)} High" : null;
    }

    private static string GetRankDisplayName(int rankValue) => rankValue switch
    {
        14 => "Ace",
        13 => "King",
        12 => "Queen",
        11 => "Jack",
        10 => "Ten",
        9 => "Nine",
        8 => "Eight",
        7 => "Seven",
        6 => "Six",
        5 => "Five",
        4 => "Four",
        3 => "Three",
        2 => "Two",
        _ => "High Card"
    };

    private static int ParseRankValue(string rank)
    {
        return rank.Trim().ToUpperInvariant() switch
        {
            "A" or "ACE" => 14,
            "K" or "KING" => 13,
            "Q" or "QUEEN" => 12,
            "J" or "JACK" => 11,
            "10" or "T" or "TEN" => 10,
            "9" or "NINE" => 9,
            "8" or "EIGHT" => 8,
            "7" or "SEVEN" => 7,
            "6" or "SIX" => 6,
            "5" or "FIVE" => 5,
            "4" or "FOUR" => 4,
            "3" or "THREE" => 3,
            "2" or "TWO" or "DEUCE" => 2,
            _ => 0
        };
    }

    // ============== Dynamic UI Helper Methods ==============

    /// <summary>
    /// Gets the title for the decision panel based on current game phase.
    /// </summary>
    private string GetDecisionTitle()
    {
        if (HasDropOrStay)
        {
            return "Drop or Stay?";
        }
        return _tableState?.CurrentPhaseDescription ?? "Make a Decision";
    }

    /// <summary>
    /// Gets the subtitle for the decision panel.
    /// </summary>
    private string GetDecisionSubtitle()
    {
        if (HasDropOrStay)
        {
            return "Choose to stay in the hand or drop out";
        }
        return "";
    }

    /// <summary>
    /// Gets the description for special phases.
    /// </summary>
    private string GetSpecialPhaseDescription()
    {
        if (HasPotMatching)
        {
            return "Losers must match the pot amount.";
        }
        if (IsBuyCardOfferPhase)
        {
            return "Waiting for buy card decisions.";
        }
        return "Please wait for this phase to complete.";
    }

    private CardInfo? GetBuyCardTriggerCard()
    {
        if (_buyCardOffer?.TriggerCard is null)
        {
            return null;
        }

        return new CardInfo
        {
            Rank = _buyCardOffer.TriggerCard.Rank,
            Suit = _buyCardOffer.TriggerCard.Suit,
            IsFaceUp = true,
            IsSelected = false,
            DealOrder = _buyCardOffer.TriggerCard.DealOrder
        };
    }

    private bool IsBuyCardTriggerCardVisible()
    {
        if (_buyCardOffer?.TriggerCard is null)
        {
            return false;
        }

        if (_currentPlayerSeatIndex < 0 || _currentPlayerSeatIndex >= _seats.Count)
        {
            return false;
        }

        var trigger = _buyCardOffer.TriggerCard;
        return _seats[_currentPlayerSeatIndex].Cards.Any(c =>
            c.IsFaceUp &&
            c.DealOrder == trigger.DealOrder &&
            string.Equals(c.Rank, trigger.Rank, StringComparison.OrdinalIgnoreCase) &&
            string.Equals(c.Suit, trigger.Suit, StringComparison.OrdinalIgnoreCase));
    }

    private bool ShouldPauseDealForBuyCardOffer(int seatIndex, bool isFaceUp)
    {
        if (!IsBaseball || !isFaceUp || !IsBuyCardOfferPhase)
        {
            return false;
        }

        var offerSeat = _tableState?.CurrentActorSeatIndex ?? -1;
        return offerSeat >= 0 && seatIndex == offerSeat;
    }

    /// <summary>
    /// Handles decision actions (like Drop/Stay).
    /// </summary>
    private async Task HandleDecisionAsync(string decision)
    {
        if (_isSubmittingAction)
        {
            return;
        }

        _isSubmittingAction = true;
        try
        {
            // Route to the correct API based on game type
            if (IsKingsAndLows && IsDropOrStayPhase)
            {
                // Use the dedicated Kings and Lows DropOrStay endpoint
                var request = new DropOrStayRequest(
                decision: decision,
                playerId: _currentPlayerId ?? Guid.Empty
                );

                var response = await GameApiRouter.DropOrStayAsync(_gameTypeCode ?? "", GameId, request);

                if (response.IsSuccess)
                {
                    _hasDropOrStayDecided = true;
                    _dropOrStayDecision = decision;
                    Logger.LogDebug("Drop or stay decision recorded via HandleDecisionAsync: {Decision}", decision);
                }
                else
                {
                    Logger.LogWarning("Decision failed: {Error}", response.Error);
                }
            }
            else
            {
                // For other game types, map decision to betting action
                var actionType = decision.ToUpperInvariant() switch
                {
                    "DROP" => BettingActionType.Fold,
                    "STAY" => BettingActionType.Check,
                    _ => BettingActionType.Check
                };

                var request = new ProcessBettingActionRequest(actionType, 0);
                var response = await GameApiRouter.ProcessBettingActionAsync(_gameTypeCode ?? "", GameId, request);

                if (!response.IsSuccess)
                {
                    Logger.LogWarning("Decision failed: {Error}", response.Error);
                }
            }
            // SignalR will broadcast the updated state
        }
        finally
        {
            _isSubmittingAction = false;
        }
    }

    private async Task HandleBuyCardDecisionAsync(bool accept)
    {
        if (_isProcessingBuyCard)
        {
            return;
        }

        if (!_currentPlayerId.HasValue || _currentPlayerId == Guid.Empty)
        {
            await ShowToastAsync("Unable to process buy card decision: player ID not available.", "error");
            return;
        }

        await TableActions.ExecuteAsync(
            async () =>
            {
                var request = new ProcessBuyCardRequest(_currentPlayerId.Value, accept);
                var response = await GameApiRouter.ProcessBuyCardAsync(_gameTypeCode ?? "", GameId, request);
                return TableActionResult.From(response);
            },
            new TableActionOptions
            {
                ActionName = "buy card decision",
                IsBusy = () => _isProcessingBuyCard,
                SetBusy = busy => _isProcessingBuyCard = busy,
                RethrowOnException = true,
                FailureFallbackMessage = "Failed to process buy card decision.",
            });
    }

    private async Task HandleTollboothChoiceAsync(string choice)
    {
        await TableActions.ExecuteAsync(
            async () =>
            {
                var request = new TollboothChooseCardRequest
                {
                    Choice = choice,
                    PlayerSeatIndex = _currentPlayerSeatIndex >= 0 ? _currentPlayerSeatIndex : null
                };

                var response = await GameApiRouter.TollboothChooseCardAsync(GameId, request);
                return TableActionResult.From(response);
            },
            new TableActionOptions
            {
                ActionName = "tollbooth choice",
                IsBusy = () => _isProcessingTollboothChoice,
                SetBusy = busy => _isProcessingTollboothChoice = busy,
                RefreshOnStart = true,
                RefreshOnComplete = true,
                FailureFallbackMessage = "Failed to process tollbooth choice.",
            });
    }

    /// <summary>
    /// Gets the maximum number of discards based on game rules.
    /// </summary>
    private int GetMaxDiscards()
    {
        // Use drawing config from table state if available, but ensure we respect
        // the personalized limit (e.g., having an Ace allows 4 discards instead of 3)
        var publicMax = _tableState?.DrawingConfig?.MaxDiscards ?? 3;

        // Irish Hold 'Em always discards exactly 2
        if (IsIrishHoldEm) return 2;

        // Phil's Mom always discards exactly 1
        if (IsPhilsMom) return 1;

        // Crazy Pineapple discards exactly 1 before the flop
        if (IsCrazyPineapple) return 1;

        if (IsBobBarker) return 1;

        return Math.Max(publicMax, _maxDiscards);
    }

    /// <summary>
    /// Gets the minimum number of discards required by game rules.
    /// Irish Hold 'Em requires exactly 2 discards (no stand pat, no partial).
    /// </summary>
    private int GetMinDiscards()
    {
        // Irish Hold 'Em requires exactly 2 discards after the flop
        if (IsIrishHoldEm) return 2;

        // Phil's Mom requires exactly 1 discard in each discard round
        if (IsPhilsMom) return 1;

        // Crazy Pineapple requires exactly 1 discard before the flop
        if (IsCrazyPineapple) return 1;

        if (IsBobBarker) return 1;

        return 0;
    }

    /// <summary>
    /// Gets special drawing rules message for display.
    /// </summary>
    private string? GetDrawingSpecialRules()
    {
        return _tableState?.DrawingConfig?.SpecialRules;
    }

    private string GetDrawPanelTitle()
    {
        if (IsBobBarker)
        {
            return "Choose Showcase";
        }

        return IsIrishStyleHoldEm ? "Discard Phase" : "Draw Phase";
    }

    /// <summary>
    /// Determines if a card is wild based on game rules.
    /// </summary>
    private bool IsWildCard(CardInfo card) => IsWildCard(card.Rank, card.Suit);

    private bool IsWildCard(string? rank, string? suit)
    {
        if (rank is not null && (IsBaseball || IsHoldTheBaseball))
        {
            var normalizedRank = rank.Trim().ToUpperInvariant();
            if (normalizedRank is "3" or "THREE" or "9" or "NINE")
            {
                return true;
            }
        }

        var wildRules = _tableState?.SpecialRules?.WildCardRules;
        if (wildRules is null)
        {
            return false;
        }

        // Check specific cards (e.g., "KD" for King of Diamonds)
        if (wildRules.SpecificCards is not null && rank is not null && suit is not null)
        {
            var cardCode = $"{rank}{GetSuitCode(suit)}";
            if (wildRules.SpecificCards.Any(c => c.Equals(cardCode, StringComparison.OrdinalIgnoreCase)))
            {
                return true;
            }
        }

        // Check wild ranks (e.g., all 2s, all Jacks)
        if (wildRules.WildRanks is not null && rank is not null)
        {
            if (wildRules.WildRanks.Any(r => r.Equals(rank, StringComparison.OrdinalIgnoreCase)))
            {
                return true;
            }
        }

        // Lowest card is wild is handled by server marking the card
        return false;
    }

    private List<CardInfo> BuildCurrentPlayerDrawCards(IReadOnlyList<CardPrivateDto> hand, bool preserveSelection)
    {
        var cards = hand.Select((c, i) => new CardInfo
        {
            Rank = c.Rank,
            Suit = c.Suit,
            IsFaceUp = true,
            IsSelected = preserveSelection && _selectedDiscardIndices.Contains(i),
            IsPubliclyVisible = c.IsPubliclyVisible,
            DealOrder = c.DealOrder,
            IsWild = IsWildForCurrentPlayerCard(c.Rank, c.Suit),
            IsShowcaseCard = c.IsShowcaseCard
        }).ToList();

        ApplyKingsAndLowsLowestCardWildFallback(cards);
        return cards;
    }

    private bool IsWildForCurrentPlayerCard(string? rank, string? suit)
    {
        if (IsWildCard(rank, suit))
        {
            return true;
        }

        // Kings and Lows always treats kings as wild for display/evaluation.
        return IsKingsAndLows && rank?.Trim().ToUpperInvariant() is "K" or "KING";
    }

    private void ApplyKingsAndLowsLowestCardWildFallback(List<CardInfo> cards)
    {
        if (!IsKingsAndLows || cards.Count == 0)
        {
            return;
        }

        var wildRules = _tableState?.SpecialRules?.WildCardRules;
        var applyLowestRankWild = wildRules?.LowestCardIsWild == true;

        if (applyLowestRankWild)
        {
            // When we have 5 face-up cards, use domain hand evaluation to correctly
            // determine Ace high/low. The domain evaluates both scenarios and picks
            // whichever produces the stronger hand (e.g., Ace-as-low when it enables a straight).
            var faceUpWithIndex = cards
                .Select((c, i) => (Card: c, Index: i))
                .Where(x => x.Card.IsFaceUp && !string.IsNullOrWhiteSpace(x.Card.Rank) && !string.IsNullOrWhiteSpace(x.Card.Suit))
                .ToList();

            if (faceUpWithIndex.Count >= 5)
            {
                var domainCards = faceUpWithIndex
                    .Select(x => TryMapToDomainCard(x.Card.Rank!, x.Card.Suit!))
                    .ToList();

                if (domainCards.All(c => c is not null))
                {
                    var hand = new KingsAndLowsDrawHand(domainCards.Select(c => c!).ToList());
                    var wildCards = hand.WildCards;

                    for (var i = 0; i < faceUpWithIndex.Count; i++)
                    {
                        if (wildCards.Contains(domainCards[i]!))
                        {
                            var origIndex = faceUpWithIndex[i].Index;
                            cards[origIndex] = cards[origIndex] with { IsWild = true };
                        }
                    }

                    return;
                }
            }
        }

        // Fallback for < 5 face-up cards or when domain conversion fails:
        // use rank-value comparison with Ace treated as high.
        int? lowestNonKingRankValue = null;
        if (applyLowestRankWild)
        {
            var lowest = cards
                .Where(c => c.IsFaceUp && !string.IsNullOrWhiteSpace(c.Rank))
                .Select(c => GetKingsAndLowsRankValue(c.Rank))
                .Where(value => value != int.MaxValue && value != 13)
                .DefaultIfEmpty(int.MaxValue)
                .Min();

            if (lowest != int.MaxValue)
            {
                lowestNonKingRankValue = lowest;
            }
        }

        for (var index = 0; index < cards.Count; index++)
        {
            var card = cards[index];
            var rankValue = GetKingsAndLowsRankValue(card.Rank);
            var isKing = rankValue == 13;
            var isLowestNonKing = lowestNonKingRankValue.HasValue && rankValue == lowestNonKingRankValue.Value;

            if (!isKing && !isLowestNonKing)
            {
                continue;
            }

            cards[index] = card with { IsWild = true };
        }
    }

    private static Card? TryMapToDomainCard(string rank, string suit)
    {
        var symbol = rank.Trim().ToUpperInvariant() switch
        {
            "2" or "TWO" or "DEUCE" => (Symbol?)Symbol.Deuce,
            "3" or "THREE" => Symbol.Three,
            "4" or "FOUR" => Symbol.Four,
            "5" or "FIVE" => Symbol.Five,
            "6" or "SIX" => Symbol.Six,
            "7" or "SEVEN" => Symbol.Seven,
            "8" or "EIGHT" => Symbol.Eight,
            "9" or "NINE" => Symbol.Nine,
            "10" or "T" or "TEN" => Symbol.Ten,
            "J" or "JACK" => Symbol.Jack,
            "Q" or "QUEEN" => Symbol.Queen,
            "K" or "KING" => Symbol.King,
            "A" or "ACE" => Symbol.Ace,
            _ => null
        };

        var domainSuit = suit.Trim().ToUpperInvariant() switch
        {
            "HEARTS" => (Suit?)Suit.Hearts,
            "DIAMONDS" => Suit.Diamonds,
            "SPADES" => Suit.Spades,
            "CLUBS" => Suit.Clubs,
            _ => null
        };

        if (symbol is null || domainSuit is null)
        {
            return null;
        }

        return new Card(domainSuit.Value, symbol.Value);
    }

    private static int GetKingsAndLowsRankValue(string? rank) => rank?.Trim().ToUpperInvariant() switch
    {
        "2" or "TWO" or "DEUCE" => 2,
        "3" or "THREE" => 3,
        "4" or "FOUR" => 4,
        "5" or "FIVE" => 5,
        "6" or "SIX" => 6,
        "7" or "SEVEN" => 7,
        "8" or "EIGHT" => 8,
        "9" or "NINE" => 9,
        "10" or "T" or "TEN" => 10,
        "J" or "JACK" => 11,
        "Q" or "QUEEN" => 12,
        "K" or "KING" => 13,
        "A" or "ACE" => 14,
        _ => int.MaxValue
    };

    /// <summary>
    /// Gets the suit code for wild card comparison.
    /// </summary>
    private static string GetSuitCode(string? suit) => suit?.ToUpperInvariant() switch
    {
        "CLUBS" => "C",
        "DIAMONDS" => "D",
        "HEARTS" => "H",
        "SPADES" => "S",
        _ => ""
    };

    /// <summary>
    /// Gets the wild cards for display in the TableCanvas panel.
    /// Returns game-specific wild cards:
    /// - Baseball: 3s and 9s
    /// - Follow the Queen: Queens + dynamic wild rank
        public bool IsPubliclyVisible { get; init; }
        public bool IsShowcaseCard { get; init; }
    /// - Kings and Lows: Kings + player's lowest card (personalized)
    /// - Twos Jacks Man with the Axe: 2s, Jacks, and King of Diamonds
    /// </summary>
    private IReadOnlyList<TableCanvas.WildCardDisplay> GetWildCardsForDisplay()
    {
        if (IsPairPressure && _dealAnimationInProgress)
        {
            return GetPairPressureVisibleWildCardsForDisplay();
        }

        var wildRules = _tableState?.SpecialRules?.WildCardRules;
        if (wildRules is null)
        {
            return [];
        }

        var wildCards = new List<TableCanvas.WildCardDisplay>();

        // Add specific cards first (e.g., King of Diamonds for Twos Jacks)
        if (wildRules.SpecificCards is { Count: > 0 })
        {
            foreach (var cardCode in wildRules.SpecificCards)
            {
                if (cardCode.Length >= 2)
                {
                    var rank = cardCode[..^1]; // Everything except last char
                    var suitCode = cardCode[^1..]; // Last char
                    var suitName = suitCode.ToUpperInvariant() switch
                    {
                        "C" => "Clubs",
                        "D" => "Diamonds",
                        "H" => "Hearts",
                        "S" => "Spades",
                        _ => null
                    };

                    if (suitName is not null)
                    {
                        var displayRank = GetDisplayRankName(rank);
                        wildCards.Add(new TableCanvas.WildCardDisplay(
                            Rank: rank,
                            Suit: suitName,
                            Label: $"{displayRank} of {suitName}"
                        ));
                    }
                }
            }
        }

        // Add wild ranks (all cards of certain ranks)
        if (wildRules.WildRanks is { Count: > 0 })
        {
            foreach (var rank in wildRules.WildRanks)
            {
                var label = GetWildRankLabel(rank);
                wildCards.Add(new TableCanvas.WildCardDisplay(
                    Rank: rank,
                    Suit: null,
                    Label: label
                ));
            }
        }

        // Add personalized wild card for "lowest card is wild" (Kings and Lows)
        if (wildRules.LowestCardIsWild && _isSeated && CurrentPlayerHandCards.Count > 0)
        {
            var lowestCard = GetLowestCard(CurrentPlayerHandCards);
            if (lowestCard is not null)
            {
                wildCards.Add(new TableCanvas.WildCardDisplay(
                    Rank: lowestCard.Rank,
                    Suit: null,
                    Label: "Your Lowest",
                    IsPersonalized: true
                ));
            }
        }

        return wildCards;
    }

    private IReadOnlyList<TableCanvas.WildCardDisplay> GetPairPressureVisibleWildCardsForDisplay()
    {
        var visibleFaceUpCardsInOrder = GetPairPressureVisibleFaceUpCardsInOrder();
        if (visibleFaceUpCardsInOrder.Count == 0)
        {
            return [];
        }

        var visibleWildRanks = new PairPressureWildCardRules()
            .DetermineWildRanks(visibleFaceUpCardsInOrder)
            .Select(GetPairPressureDisplayRank)
            .Where(rank => !string.IsNullOrWhiteSpace(rank))
            .ToList();

        return visibleWildRanks
            .Select(rank => new TableCanvas.WildCardDisplay(
                Rank: rank,
                Suit: null,
                Label: GetWildRankLabel(rank)))
            .ToList();
    }

    private IReadOnlyList<Card> GetPairPressureVisibleFaceUpCardsInOrder()
    {
        return StudOrderHelper.OrderFaceUpCardsInGlobalDealOrder(
                _seats.Where(seat => seat.IsOccupied && seat.Cards.Count > 0),
                _dealerSeatIndex,
                seat => seat.SeatIndex,
                seat => seat.Cards,
                card => card.IsFaceUp && card.IsPubliclyVisible && card.Rank is not null && card.Suit is not null)
            .Select(card => ConvertToCard(card.Rank!, card.Suit!))
            .Where(card => card is not null)
            .Cast<Card>()
            .ToList();
    }

    private static string GetPairPressureDisplayRank(int rankValue) => rankValue switch
    {
        >= 2 and <= 10 => rankValue.ToString(),
        11 => "J",
        12 => "Q",
        13 => "K",
        14 => "A",
        _ => string.Empty
    };

    private static string NormalizePairPressureWildRank(string? rank) => rank?.Trim().ToUpperInvariant() switch
    {
        "ACE" => "A",
        "KING" => "K",
        "QUEEN" => "Q",
        "JACK" => "J",
        "TEN" or "T" => "10",
        "NINE" => "9",
        "EIGHT" => "8",
        "SEVEN" => "7",
        "SIX" => "6",
        "FIVE" => "5",
        "FOUR" => "4",
        "THREE" => "3",
        "TWO" or "DEUCE" => "2",
        _ => rank?.Trim().ToUpperInvariant() ?? string.Empty
    };

    /// <summary>
    /// Gets a display-friendly name for a rank.
    /// </summary>
    private static string GetDisplayRankName(string? rank) => rank?.ToUpperInvariant() switch
    {
        "A" => "Ace",
        "K" => "King",
        "Q" => "Queen",
        "J" => "Jack",
        "10" or "T" => "10",
        _ => rank ?? "?"
    };

    /// <summary>
    /// Gets a label for a wild rank (e.g., "All 3s", "Queens").
    /// </summary>
    private static string GetWildRankLabel(string? rank) => rank?.ToUpperInvariant() switch
    {
        "A" => "Aces",
        "K" => "Kings",
        "Q" => "Queens",
        "J" => "Jacks",
        "2" => "Deuces",
        "3" => "Threes",
        "4" => "Fours",
        "5" => "Fives",
        "6" => "Sixes",
        "7" => "Sevens",
        "8" => "Eights",
        "9" => "Nines",
        "10" or "T" => "Tens",
        _ => $"All {rank}s"
    };

    /// <summary>
    /// Gets the lowest card from a collection of cards (for Kings and Lows).
    /// </summary>
    private static CardInfo? GetLowestCard(IReadOnlyList<CardInfo> cards)
    {
        if (cards.Count == 0)
        {
            return null;
        }

        return cards
            .Where(c => c.IsFaceUp && !string.IsNullOrWhiteSpace(c.Rank))
            .OrderBy(c => GetCardValue(c.Rank))
            .FirstOrDefault();
    }

    /// <summary>
    /// Gets the numeric value of a card rank for comparison.
    /// </summary>
    private static int GetCardValue(string? rank) => rank?.ToUpperInvariant() switch
    {
        "A" => 14,
        "K" => 13,
        "Q" => 12,
        "J" => 11,
        "10" or "T" => 10,
        "9" => 9,
        "8" => 8,
        "7" => 7,
        "6" => 6,
        "5" => 5,
        "4" => 4,
        "3" => 3,
        "2" => 2,
        _ => 0
    };

    private static string DisplayFirstName(ShowdownPlayerHand? hand)
    {
        if (!string.IsNullOrWhiteSpace(hand?.PlayerFirstName))
        {
            return hand.PlayerFirstName;
        }

        if (string.IsNullOrWhiteSpace(hand?.PlayerName))
        {
            return "Player";
        }

        var parts = hand.PlayerName.Trim().Split(' ', StringSplitOptions.RemoveEmptyEntries);
        return parts.Length > 0 ? parts[0] : hand.PlayerName;
    }

    /// <summary>
    /// Gets the list of player names who lost the showdown and must match the pot.
    /// </summary>
    private IReadOnlyList<string> GetLosingPlayers()
    {
        return _showdownResult?.PlayerHands?
            .Where(h => h.IsWinner == false)
            .Select(h => DisplayFirstName(h))
            .ToList() ?? [];
    }

    /// <summary>
    /// Handles pot matching acknowledgment.
    /// Calls the AcknowledgePotMatch API endpoint.
    /// </summary>
    private async Task HandlePotMatchAcknowledge()
    {
        if (_isAcknowledgingPotMatch)
        {
            return;
        }

        _isAcknowledgingPotMatch = true;
        await InvokeStateHasChangedAsync();

        // Leverage API router which handles game type checks or throws/returns error if not supported
        try
        {
            var response = await GameApiRouter.AcknowledgePotMatchAsync(_gameTypeCode ?? "", GameId);
            if (response.IsSuccess)
            {
                Logger.LogDebug("Pot match acknowledged successfully");
                // SignalR will update the game state
            }
            else
            {
                // Only log error if it wasn't just "not supported" for a game that doesn't use it
                // But this method guarded with !IsKingsAndLows before.
                // It's cleaner to keep the guard if the UI button is only shown for KingsAndLows.
                Logger.LogError("Failed to acknowledge pot match: {StatusCode} {Error}", response.StatusCode, response.Error);
                await ShowToastAsync("Failed to acknowledge pot match.", "error");
                
                // On error, reset the flag so user can try again
                _isAcknowledgingPotMatch = false;
                await InvokeStateHasChangedAsync();
            }
        }
        catch (Exception ex)
        {
            Logger.LogError(ex, "Error acknowledging pot match");
            await ShowToastAsync("An error occurred while acknowledging.", "error");
            
            // On error, reset the flag so user can try again
            _isAcknowledgingPotMatch = false;
            await InvokeStateHasChangedAsync();
        }
    }

    // Dealer's Choice methods

    private string? GetDealersChoiceDealerName()
    {
        if (_dealersChoiceDealerPosition is not null && _dealersChoiceDealerPosition.Value >= 0 && _dealersChoiceDealerPosition.Value < _seats.Count)
        {
            var seat = _seats[_dealersChoiceDealerPosition.Value];
            return seat.IsOccupied ? seat.PlayerName : null;
        }
        return null;
    }

    private async Task HandleDealerChoiceAsync(DealerChoiceModal.DealerChoiceSelection selection)
    {
        try
        {
            var request = new ChooseDealerGameRequest(selection.GameTypeCode, selection.Ante, selection.MinBet, selection.SmallBlind, selection.BigBlind);
            var response = await GamesApiClient.ChooseDealerGameAsync(GameId, request);
            if (response is { IsSuccessStatusCode: true, Content: not null })
            {
                Logger.LogDebug("Dealer chose game {GameType} for hand {Hand}", response.Content.GameTypeName, response.Content.HandNumber);
                // SignalR will handle the state transition
            }
            else
            {
                Logger.LogError("Failed to choose dealer game: {StatusCode}", response.StatusCode);
                await ShowToastAsync("Failed to set game variant. Please try again.", "error");
            }
        }
        catch (Exception ex)
        {
            Logger.LogError(ex, "Error choosing dealer game");
            await ShowToastAsync("An error occurred. Please try again.", "error");
        }
    }

    private async Task LoadAvailableGamesAsync()
    {
        _dcGamesLoading = true;
        try
        {
            var response = await AvailablePokerGamesApi.GetAvailablePokerGamesAsync(string.Empty);
            if (response is { IsSuccessStatusCode: true, Content: not null })
            {
                _dcAvailableGames = response.Content.ToList();
            }
        }
        catch (Exception ex)
        {
            Logger.LogError(ex, "Error loading available games for Dealer's Choice");
        }
        finally
        {
            _dcGamesLoading = false;
            await InvokeStateHasChangedAsync();
        }
    }

    // Kings and Lows - Chip Check Pause methods

    /// <summary>
    /// Gets whether the current player is short on chips for the pot.
    /// </summary>
    private bool IsCurrentPlayerShortOnChips =>
    _isRebuyGracePause
        ? IsCurrentPlayerInRebuyGraceShortPlayers()
        : _isSeated &&
          _currentPlayerSeatIndex >= 0 &&
          _currentPlayerSeatIndex < _seats.Count &&
          _seats[_currentPlayerSeatIndex].Chips < _chipCheckPotAmount;

    /// <summary>
    /// Checks if a specific player (by seat index) is short on chips for the pot.
    /// Used to display "Waiting for chips" status to other players.
    /// </summary>
    private bool IsPlayerShortOnChips(int seatIndex, int chips)
    {
        if (!_isPausedForChipCheck || _chipCheckPotAmount <= 0)
        {
            return false;
        }

        return chips < _chipCheckPotAmount;
    }

    /// <summary>
    /// Gets the current player's chip count.
    /// </summary>
    private int GetCurrentPlayerChips()
    {
        if (_currentPlayerSeatIndex >= 0 && _currentPlayerSeatIndex < _seats.Count)
        {
            return _seats[_currentPlayerSeatIndex].Chips;
        }
        return 0;
    }

    /// <summary>
    /// Gets the list of players who are short on chips.
    /// </summary>
    private IReadOnlyList<ChipCoveragePauseOverlay.ShortPlayerInfo> GetShortPlayers()
    {
        if (_isRebuyGracePause)
        {
            return _tableState?.ChipCheckPause?.ShortPlayers?
                .Select(sp => new ChipCoveragePauseOverlay.ShortPlayerInfo(
                    sp.PlayerFirstName ?? sp.PlayerName,
                    sp.CurrentChips))
                .ToList()
                ?? [];
        }

        return _seats
            .Where(s => s.IsOccupied && !s.IsSittingOut && s.Chips < _chipCheckPotAmount)
            .Select(s => new ChipCoveragePauseOverlay.ShortPlayerInfo(
                s.PlayerFirstName ?? s.PlayerName ?? "Unknown",
                s.Chips))
            .ToList();
    }

    private bool IsCurrentPlayerInRebuyGraceShortPlayers()
    {
        if (!_isSeated || _currentPlayerSeatIndex < 0)
        {
            return false;
        }

        return _tableState?.ChipCheckPause?.ShortPlayers?.Any(sp => sp.SeatIndex == _currentPlayerSeatIndex) == true;
    }

    /// <summary>
    /// Gets the number of seconds remaining on the chip check pause timer.
    /// </summary>
    private int GetChipCheckSecondsRemaining()
    {
        if (_isPausedForChipCheck &&
            _actionTimerState is { IsActive: true, PlayerSeatIndex: -1 })
        {
            return Math.Max(0, _actionTimerState.SecondsRemaining);
        }

        if (!_chipCheckPauseEndsAt.HasValue)
        {
            return 0;
        }

        var remaining = _chipCheckPauseEndsAt.Value - DateTimeOffset.UtcNow;
        return Math.Max(0, (int)remaining.TotalSeconds);
    }

    // Kings and Lows - Drop or Stay methods

    /// <summary>
    /// Gets the list of players waiting to make their drop or stay decision.
    /// </summary>
    private IReadOnlyList<string> GetPlayersWaitingForDropOrStay()
    {
        return _seats
        .Where(s => s.IsOccupied && !s.IsFolded && !s.IsSittingOut && !s.HasDecidedDropOrStay)
        .Select(s => s.PlayerFirstName ?? s.PlayerName ?? "Unknown")
        .ToList();
    }

    /// <summary>
    /// Handles drop or stay decision from the overlay.
    /// </summary>
    private async Task HandleDropOrStayDecision(string decision)
    {
        // Don't check GameTypeCode manually, rely on HasDropOrStay rule
        if (!HasDropOrStay || _isProcessingDropOrStay)
        {
            return;
        }

        _isProcessingDropOrStay = true;
        StateHasChanged();

        try
        {
            var request = new DropOrStayRequest(
            decision: decision,
            playerId: _currentPlayerId ?? Guid.Empty
            );

            var response = await KingsAndLowsApi.KingsAndLowsDropOrStayAsync(GameId, request);

            if (response.IsSuccessStatusCode)
            {
                _hasDropOrStayDecided = true;
                _dropOrStayDecision = decision;
                Logger.LogDebug("Drop or stay decision recorded: {Decision}", decision);
                // SignalR will update the game state
            }
            else
            {
                Logger.LogError("Failed to record drop or stay decision: {StatusCode}", response.StatusCode);
            }
        }
        catch (Exception ex)
        {
            Logger.LogError(ex, "Error recording drop or stay decision");
        }
        finally
        {
            _isProcessingDropOrStay = false;
            StateHasChanged();
        }
    }

    /// <summary>
    /// Handles keep or trade decision from the Screw Your Neighbor overlay.
    /// </summary>
    private async Task HandleKeepOrTradeDecision(string decision)
    {
        if (!HasKeepOrTrade || _isProcessingKeepOrTrade)
        {
            return;
        }

        _isProcessingKeepOrTrade = true;

        // Set decision BEFORE the API call so SignalR state updates (which fire during
        // the await) see it and don't overwrite with the server's hand evaluation.
        var isTrade = string.Equals(decision, "Trade", StringComparison.OrdinalIgnoreCase);
        _synKeepOrTradeDecision = isTrade ? "Traded" : "Kept";
        if (_currentPlayerSeatIndex >= 0 && _currentPlayerSeatIndex < _seats.Count)
        {
            _seats[_currentPlayerSeatIndex].HandEvaluationDescription = _synKeepOrTradeDecision;
        }

        // Trigger the trade animation immediately for the current player so they see
        // the cards fly before the API response / SignalR update swaps the card values.
        // The dedupe key prevents a second animation when the server notification arrives.
        if (isTrade)
        {
            QueueScrewYourNeighborTradeAnimation(_currentPlayerSeatIndex);
        }

        StateHasChanged();

        try
        {
            var request = new KeepOrTradeRequest(
            decision: decision,
            playerId: _currentPlayerId ?? Guid.Empty
            );

            var response = await GameApiRouter.KeepOrTradeAsync(_gameTypeCode ?? "", GameId, request);

            if (response.IsSuccess)
            {
                Logger.LogDebug("Keep or trade decision recorded: {Decision}", decision);
            }
            else
            {
                Logger.LogWarning("Keep or trade decision failed: {Error}", response.Error);
                _synKeepOrTradeDecision = null;
            }
        }
        catch (Exception ex)
        {
            Logger.LogError(ex, "Error recording keep or trade decision");
        }
        finally
        {
            _isProcessingKeepOrTrade = false;
            StateHasChanged();
        }
    }

    /// <summary>
    /// Handles the "Deal My Cards" action in In-Between — triggers boundary card dealing via PlaceBet.
    /// </summary>
    private async Task HandleInBetweenDealCardsAsync()
    {
        if (_isProcessingInBetween) return;
        _isProcessingInBetween = true;
        _ibShowDealPrompt = false;
        await InvokeStateHasChangedAsync();

        try
        {
            var request = new InBetweenPlaceBetRequest(
                playerId: _currentPlayerId ?? Guid.Empty,
                amount: 0);

            var response = await InBetweenApi.InBetweenPlaceBetAsync(GameId, request);

            if (response.IsSuccessStatusCode && response.Content is not null)
            {
                if (string.Equals(response.Content.TurnResult, "BoundaryCardsDealt", StringComparison.OrdinalIgnoreCase))
                {
                    // Boundary cards dealt — wait for SignalR state update with community cards
                    // before showing bet controls so the cards are visible on the table.
                    _ibWaitingForBoundaryCards = true;
                    _ibAceChoiceRequired = false;
                }
                else
                {
                    // Bet of 0 = pass. Turn resolved, next player.
                    _ibLastTurnDescription = response.Content.Description;
                    _ibShowBetControls = false;
                    _ibAceChoiceRequired = false;
                }
            }
            else
            {
                var errorContent = response.Error?.Content ?? "";
                if (errorContent.Contains("Ace", StringComparison.OrdinalIgnoreCase) &&
                    errorContent.Contains("high or low", StringComparison.OrdinalIgnoreCase))
                {
                    _ibAceChoiceRequired = true;
                    _ibShowBetControls = false;
                }
                else
                {
                    // Unexpected error — show bet controls as fallback
                    _ibShowBetControls = true;
                    _ibAceChoiceRequired = false;
                }
            }
        }
        catch (Exception ex)
        {
            Logger.LogError(ex, "Error dealing In-Between boundary cards");
            _ibShowDealPrompt = true;
        }
        finally
        {
            _isProcessingInBetween = false;
            await InvokeStateHasChangedAsync();
        }
    }

    /// <summary>
    /// Handles ace high/low choice in In-Between.
    /// </summary>
    private async Task HandleInBetweenAceChoiceAsync(bool aceIsHigh)
    {
        await TableActions.ExecuteAsync<InBetweenAceChoiceSuccessful>(
            async () =>
            {
                var request = new InBetweenAceChoiceRequest(
                    playerId: _currentPlayerId ?? Guid.Empty,
                    aceIsHigh: aceIsHigh);

                var response = await InBetweenApi.InBetweenAceChoiceAsync(GameId, request);
                return TableActionResult<InBetweenAceChoiceSuccessful>.From(response);
            },
            onSuccess: _ =>
            {
                _ibAceChoiceRequired = false;
                _ibShowBetControls = true;
                return Task.CompletedTask;
            },
            new TableActionOptions
            {
                ActionName = "In-Between ace choice",
                IsBusy = () => _isProcessingInBetween,
                SetBusy = busy => _isProcessingInBetween = busy,
                RefreshOnStart = true,
                RefreshOnComplete = true,
                FailureMessage = "Failed to submit ace choice.",
            });
    }

    /// <summary>
    /// Handles bet or pass in In-Between.
    /// </summary>
    private async Task HandleInBetweenPlaceBetAsync(int amount)
    {
        await TableActions.ExecuteAsync<InBetweenPlaceBetSuccessful>(
            async () =>
            {
                var request = new InBetweenPlaceBetRequest(
                    playerId: _currentPlayerId ?? Guid.Empty,
                    amount: amount);

                var response = await InBetweenApi.InBetweenPlaceBetAsync(GameId, request);
                var result = TableActionResult<InBetweenPlaceBetSuccessful>.From(response);

                // A success status without content is treated as a failure (no turn data).
                return result.IsSuccess && result.Content is null
                    ? TableActionResult<InBetweenPlaceBetSuccessful>.Fail(result.Error, result.StatusCode)
                    : result;
            },
            onSuccess: result =>
            {
                _ibLastTurnDescription = result.Content!.Description;
                _ibShowBetControls = false;
                _ibAceChoiceRequired = false;
                _ibShowDealPrompt = false;

                // When an actual bet was placed, the server pauses before advancing
                // so all players can see the revealed third card.
                if (amount > 0)
                {
                    _ibWaitingForReveal = true;
                }

                return Task.CompletedTask;
            },
            new TableActionOptions
            {
                ActionName = "In-Between place bet",
                IsBusy = () => _isProcessingInBetween,
                SetBusy = busy => _isProcessingInBetween = busy,
                RefreshOnStart = true,
                RefreshOnComplete = true,
                FailureMessage = "Failed to place bet.",
            });
    }

    /// <summary>
    /// Gets the maximum legal bet for the current In-Between turn (pot amount, clamped to player chips).
    /// </summary>
    private int GetInBetweenMaxBet()
    {
        var chips = GetCurrentPlayerChips();
        return Math.Min(_pot, chips);
    }

    private void ScheduleShowdownOverlayAfterDelay(TimeSpan delay)
    {
        _showdownDelayUntil = DateTimeOffset.UtcNow.Add(delay);
        var scheduledDelayUntil = _showdownDelayUntil.Value;

        _ = Task.Run(async () =>
        {
            await Task.Delay(delay);

            if (_disposed || !_showdownDelayUntil.HasValue || _showdownDelayUntil.Value != scheduledDelayUntil)
            {
                return;
            }

            _showdownDelayUntil = null;

            var hasShowdownInLatestState = _tableState?.Showdown is not null;
            var canRevealDuringResultsLikePhase = IsShowdownPhase || _isResultsPhase || IsCompletePhase || IsEndedPhase || IsGameCompleted;

            if (_showdownResult is null && hasShowdownInLatestState && _tableState?.Showdown is not null)
            {
                UpdateShowdownFromPublicState(_tableState.Showdown);
            }

            if (_showdownResult is null && IsShowdownPhase)
            {
                await TryLoadShowdownAsync();
            }

            if (canRevealDuringResultsLikePhase && (_showdownResult is not null || hasShowdownInLatestState))
            {
                _showShowdownOverlay = ShouldDisplayShowdownOverlay;
            }

            await InvokeStateHasChangedAsync();
        });
    }

    // Kings and Lows - Player vs Deck methods

    /// <summary>
    /// Gets the deck's cards for display in the player vs deck overlay.
    /// </summary>
    private IReadOnlyList<PlayerVsDeckOverlay.CardDto> GetDeckCards()
    {
        var deckCards = _tableState?.PlayerVsDeck?.DeckCards;
        if (deckCards is null || deckCards.Count == 0)
        {
            return [];
        }

        return deckCards
        .Select(c => new PlayerVsDeckOverlay.CardDto(c.Rank ?? "?", c.Suit ?? "Unknown"))
        .ToList();
    }

    /// <summary>
    /// Checks if the current player is the decision maker for the deck draw.
    /// </summary>
    private bool IsCurrentPlayerDeckDecisionMaker
    {
        get
        {
            var playerVsDeck = _tableState?.PlayerVsDeck;
            if (playerVsDeck is null)
            {
                return false;
            }

            // The current player is the decision maker if their seat matches
            return _currentPlayerSeatIndex == playerVsDeck.DecisionMakerSeatIndex;
        }
    }

    /// <summary>
    /// Gets the name of the player who makes the deck draw decision.
    /// </summary>
    private string GetDeckDecisionMakerName()
    {
        var playerVsDeck = _tableState?.PlayerVsDeck;
        if (playerVsDeck is null)
        {
            return "Dealer";
        }

        // Prefer first name for a friendlier display
        return !string.IsNullOrWhiteSpace(playerVsDeck.DecisionMakerFirstName)
        ? playerVsDeck.DecisionMakerFirstName
        : playerVsDeck.DecisionMakerName ?? "Dealer";
    }

    /// <summary>
    /// Gets the staying player's cards for the Player vs Deck overlay.
    /// </summary>
    private IReadOnlyList<CardInfo> GetStayingPlayerCards()
    {
        var playerVsDeck = _tableState?.PlayerVsDeck;
        if (playerVsDeck is null)
        {
            return [];
        }

        var stayingPlayerSeatIndex = playerVsDeck.StayingPlayerSeatIndex;

        // If the staying player is the current player and we have draw cards populated,
        // use those instead (they contain the full hand after the draw phase)
        if (stayingPlayerSeatIndex == _currentPlayerSeatIndex && _currentPlayerDrawCards.Count > 0)
        {
            return _currentPlayerDrawCards;
        }

        // Use the staying player's cards from the PlayerVsDeck state if available (includes newly drawn cards)
        if (playerVsDeck.StayingPlayerCards?.Count > 0)
        {
            return playerVsDeck.StayingPlayerCards
            .Select(c => new CardInfo
            {
                Rank = c.Rank,
                Suit = c.Suit,
                IsFaceUp = c.IsFaceUp,
                IsSelected = false
            })
            .ToList();
        }

        // Fallback to seat cards
        if (stayingPlayerSeatIndex >= 0 && stayingPlayerSeatIndex < _seats.Count)
        {
            return _seats[stayingPlayerSeatIndex].Cards;
        }

        return [];
    }

    /// <summary>
    /// Gets the staying player's name for the Player vs Deck overlay.
    /// </summary>
    private string? GetStayingPlayerName()
    {
        var playerVsDeck = _tableState?.PlayerVsDeck;
        if (playerVsDeck is null)
        {
            return null;
        }

        var stayingPlayerSeatIndex = playerVsDeck.StayingPlayerSeatIndex;
        if (stayingPlayerSeatIndex < 0 || stayingPlayerSeatIndex >= _seats.Count)
        {
            return playerVsDeck.StayingPlayerName;
        }

        // Prefer first name for a friendlier display
        var seat = _seats[stayingPlayerSeatIndex];
        return !string.IsNullOrWhiteSpace(seat.PlayerFirstName)
        ? seat.PlayerFirstName
        : seat.PlayerName ?? playerVsDeck.StayingPlayerName;
    }

    /// <summary>
    /// Gets the staying player's hand description for the Player vs Deck overlay.
    /// </summary>
    private string? GetStayingPlayerHandDescription()
    {
        var playerVsDeck = _tableState?.PlayerVsDeck;
        if (playerVsDeck is null)
        {
            return null;
        }

        var stayingPlayerSeatIndex = playerVsDeck.StayingPlayerSeatIndex;

        // If the staying player is the current player and we have a new hand description from the draw,
        // use that instead (it reflects the hand after drawing new cards)
        if (stayingPlayerSeatIndex == _currentPlayerSeatIndex && !string.IsNullOrWhiteSpace(_newHandDescription))
        {
            return _newHandDescription;
        }

        // Use the hand description from the PlayerVsDeck state if available
        if (!string.IsNullOrWhiteSpace(playerVsDeck.StayingPlayerHandDescription))
        {
            return FormatHandDescriptionForDisplay(playerVsDeck.StayingPlayerHandDescription);
        }

        // Fallback to seat's hand evaluation description
        if (stayingPlayerSeatIndex >= 0 && stayingPlayerSeatIndex < _seats.Count)
        {
            return FormatHandDescriptionForDisplay(_seats[stayingPlayerSeatIndex].HandEvaluationDescription);
        }

        return null;
    }

    private static string? FormatHandDescriptionForDisplay(string? description)
    {
        if (string.IsNullOrWhiteSpace(description))
        {
            return description;
        }

        var trimmed = description.Trim();
        var parts = trimmed.Split(' ', StringSplitOptions.RemoveEmptyEntries);
        if (parts.Length == 0 || !parts[0].Contains('-', StringComparison.Ordinal))
        {
            return trimmed;
        }

        var rankTokens = parts[0].Split('-', StringSplitOptions.RemoveEmptyEntries);
        if (rankTokens.Length == 0)
        {
            return trimmed;
        }

        var mapped = new List<string>(rankTokens.Length);
        foreach (var token in rankTokens)
        {
            if (!int.TryParse(token, out var value) || value < 1 || value > 14)
            {
                return trimmed;
            }

            mapped.Add(value switch
            {
                1 or 14 => "Ace",
                13 => "King",
                12 => "Queen",
                11 => "Jack",
                _ => value.ToString()
            });
        }

        var rebuilt = string.Join('-', mapped);
        return parts.Length == 1 ? rebuilt : $"{rebuilt} {string.Join(' ', parts.Skip(1))}";
    }

    /// <summary>
    /// Handles card selection toggle for deck draw.
    /// </summary>
    private Task HandleDeckCardSelectionToggle(int index)
    {
        if (_deckDrawSelectedIndices.Contains(index))
        {
            _deckDrawSelectedIndices.Remove(index);
        }
        else
        {
            _deckDrawSelectedIndices.Add(index);
        }
        StateHasChanged();
        return Task.CompletedTask;
    }

    /// <summary>
    /// Handles confirmation of deck draw.
    /// </summary>
    private async Task HandleDeckDrawConfirm()
    {
        if (!IsKingsAndLows || _isProcessingDeckDraw)
        {
            return;
        }

        var discardIndices = _deckDrawSelectedIndices.ToList();

        _isProcessingDeckDraw = true;
        _isDeckDrawAnimating = true;
        _deckCardsDrawn = false;
        _delayShowdownOverlay = true; // Delay showdown until animation completes

        // Clear any existing showdown result so we fetch fresh data from API after deck draw
        // This prevents stale SignalR data from being used
        _showdownResult = null;
        _showdownStateSnapshot = null;
        _showdownCommunityCardsSnapshot = [];
        _showShowdownOverlay = false;

        // Initialize deck draw cards from current SignalR state for animation
        var currentDeckCards = GetDeckCards();
        _deckDrawCards = currentDeckCards
        .Select((c, i) => new CardInfo
        {
            Rank = c.Rank,
            Suit = c.Suit,
            IsFaceUp = true,
            IsSelected = discardIndices.Contains(i)
        })
        .ToList();

        StateHasChanged();

        try
        {
            // Start discard animation if there are cards to discard
            if (discardIndices.Count > 0)
            {
                _isDeckDiscardingCards = true;
                await InvokeStateHasChangedAsync();

                // Wait for discard animation
                await Task.Delay(600);
            }

            var request = new DeckDrawRequest(
            discardIndices: discardIndices,
            playerId: _currentPlayerId ?? Guid.Empty
            );

            var response = await KingsAndLowsApi.KingsAndLowsDeckDrawAsync(GameId, request);

            if (response.IsSuccessStatusCode)
            {
                // Transition to drawing new cards animation
                _isDeckDiscardingCards = false;

                if (discardIndices.Count > 0)
                {
                    _isDeckDrawingNewCards = true;
                    _deckNewCardIndices = discardIndices;
                    await InvokeStateHasChangedAsync();

                    // Wait for draw animation
                    await Task.Delay(700);

                    _deckCardsDrawn = true;
                    if (!string.IsNullOrWhiteSpace(response.Content.HandDescription))
                    {
                        _deckHandDescription = FormatHandDescriptionForDisplay(response.Content.HandDescription);
                    }
                    await InvokeStateHasChangedAsync();

                    // Keep the panel visible to show the results
                    // SignalR will update the deck cards with final hand
                    await Task.Delay(TimeSpan.FromSeconds(5));

                    _isDeckDrawingNewCards = false;
                    _deckNewCardIndices = [];
                }
                else
                {
                    // Stand pat - just mark as complete
                    _deckCardsDrawn = true;
                    await InvokeStateHasChangedAsync();

                    // Keep visible briefly
                    await Task.Delay(TimeSpan.FromSeconds(3));
                }

                _hasDeckDrawn = true;
                _deckDrawSelectedIndices.Clear();
                _deckDrawCards = []; // Clear so SignalR state takes over
                Logger.LogDebug("Deck draw completed");

                // Now show the showdown overlay after animation is complete
                _delayShowdownOverlay = false;

                // Update the local game phase from the API response so IsShowdownPhase returns true
                // (SignalR broadcast may not have arrived yet)
                if (_gameResponse is not null && response.Content is not null)
                {
                    Logger.LogDebug("HandleDeckDrawSubmittedAsync: Updating CurrentPhase from {Old} to {New}",
                    _gameResponse.CurrentPhase, response.Content.NextPhase);
                    _gameResponse = _gameResponse with { CurrentPhase = response.Content.NextPhase };
                }
                else
                {
                    Logger.LogWarning("HandleDeckDrawSubmittedAsync: Cannot update phase - _gameResponse={HasResponse}, response.Content={HasContent}",
                    _gameResponse is not null, response.Content is not null);
                }

                await TryLoadShowdownAsync();
                await InvokeStateHasChangedAsync();
            }
            else
            {
                Logger.LogError("Failed to process deck draw: {StatusCode}", response.StatusCode);
                _delayShowdownOverlay = false; // Clear delay on error
            }
        }
        catch (Exception ex)
        {
            Logger.LogError(ex, "Error processing deck draw");
            _delayShowdownOverlay = false; // Clear delay on error
        }
        finally
        {
            _isProcessingDeckDraw = false;
            _isDeckDrawAnimating = false;
            _isDeckDiscardingCards = false;
            _isDeckDrawingNewCards = false;
            _deckCardsDrawn = false;
            _deckDrawCards = [];
            StateHasChanged();
        }
    }

    /// <summary>
    /// Callback invoked when chips are added through the chip management section.
    /// State will be automatically updated via SignalR PrivateStateUpdated event.
    /// </summary>
    private async Task OnChipsAddedAsync()
    {
        // Add chips: TableRefreshPolicy = HubDriven. The cashier update is broadcast over
        // SignalR, so the page waits for that push instead of refetching (see
        // docs/TableRefreshPolicy.md).
        await Task.CompletedTask;
    }
}
