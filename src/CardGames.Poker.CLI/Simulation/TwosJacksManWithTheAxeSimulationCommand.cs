...
using CardGames.Poker.Simulations.TwosJacksManWithTheAxe;
using Spectre.Console;
using Spectre.Console.Cli;

namespace CardGames.Poker.CLI.Simulation;

internal class TwosJacksManWithTheAxeSimulationCommand : Command<SimulationSettings>
{
    private static readonly SpectreLogger Logger = new();

    private static TwosJacksManWithTheAxeSimulation CreateSimulation()
    {
        var simulation = new TwosJacksManWithTheAxeSimulation();
        do
        {
    ...
    private static TwosJacksManWithTheAxePlayer GetPlayer()
    {
        ...
        return new TwosJacksManWithTheAxePlayer(name).WithCards(cards);
    }

    private static void PrintResults(TwosJacksManWithTheAxeSimulationResult result)
        => new[]
            {
                HandsEvaluation
