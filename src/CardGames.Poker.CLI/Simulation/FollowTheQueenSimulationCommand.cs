...
internal class FollowTheQueenSimulationCommand : Command<SimulationSettings>
{
    private static readonly SpectreLogger Logger = new();

    private static FollowTheQueenSimulation CreateSimulation()
    {
    ...
    private static FollowTheQueenPlayer GetPlayer()
    {
        ...
        return new FollowTheQueenPlayer(name)
            .WithHoleCards(holeCards)
            .WithBoardCards(boardCards);
    }

    private static void PrintResults(FollowTheQueenSimulationResult result)
        => new[]
            {
                HandsEvaluation
