...
/// <summary>
/// Dealing engine for Texas Hold'em.
/// Handles dealing 2 hole cards per player and 5 community cards (3 flop, 1 turn, 1 river).
/// Burns one card before each community card round.
/// </summary>
public class HoldEmDealingEngine : IDealingEngine
{
    ...
    /// <inheritdoc/>
    public string VariantId => "holdem";

    /// <inheritdoc/>
    public int Seed => _rng?.Seed ?? 0;

    /// <inheritdoc/>
    public int HoleCardsPerPlayer => 2;
    ...
    public void Initialize(IReadOnlyList<string> playerNames, int dealerPosition, int? seed = null)
    {
        ...
        {
            throw new ArgumentException("Hold'em requires at least 2 players", nameof(playerNames));
        }
