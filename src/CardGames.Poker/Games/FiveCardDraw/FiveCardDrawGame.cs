...
public class FiveCardDrawGame : IPokerGame
{
    ...
    public FiveCardDrawGame(IEnumerable<(string name, int chips)> players, int ante, int minBet)
    {
        ...
        {
            throw new ArgumentException($"Five Card Draw requires at least {MinimumNumberOfPlayers} players");
        }

        if (playerList.Count > MaximumNumberOfPlayers)
        {
            throw new ArgumentException($"Five Card Draw supports at most {MaximumNumberOfPlayers} players");
        }
    ...
    /// <summary>
    /// Gets the game rules metadata for Five Card Draw.
    /// </summary>
    public GameRules GetGameRules()
    {
        return FiveCardDrawRules.CreateGameRules();
    }
    ...
    /// </summary>
    /// <returns>
    /// The <see cref="FiveCardDrawGamePlayer"/> who needs to draw, or <c>null</c> if not in the draw phase
