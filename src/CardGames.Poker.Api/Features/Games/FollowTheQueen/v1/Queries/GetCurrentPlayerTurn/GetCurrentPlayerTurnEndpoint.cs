...
/// <summary>
/// Endpoint for retrieving the current player's turn state in a specific Follow the Queen game.
/// </summary>
public static class GetCurrentPlayerTurnEndpoint
{
	public static RouteGroupBuilder MapGetCurrentPlayerTurn(this RouteGroupBuilder group)
	{
		group.MapGet("{gameId:guid}/current-turn",
			...
			.WithName($"FollowTheQueen{nameof(MapGetCurrentPlayerTurn).TrimPrefix("Map")}")
			.WithSummary(nameof(MapGetCurrentPlayerTurn).TrimPrefix("Map"))
			.WithDescription("Retrieve the current player's turn state for a specific Follow the Queen game, including available actions.")
			.MapToApiVersion(1.0)
