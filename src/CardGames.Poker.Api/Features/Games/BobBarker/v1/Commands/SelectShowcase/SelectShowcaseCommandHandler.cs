    ...
    : IRequestHandler<SelectShowcaseCommand, OneOf<SelectShowcaseSuccessful, SelectShowcaseError>>
{
        ...
        CancellationToken cancellationToken)
    {
        for (var attempt = 0; attempt < MaxConcurrencyAttempts; attempt++)
        {
            ...
            catch (DbUpdateConcurrencyException) when (attempt < MaxConcurrencyAttempts - 1)
            {
                continue;
            }
        ...
        throw new DbUpdateConcurrencyException("Showcase selection could not be completed due to a concurrent update.");
    }
