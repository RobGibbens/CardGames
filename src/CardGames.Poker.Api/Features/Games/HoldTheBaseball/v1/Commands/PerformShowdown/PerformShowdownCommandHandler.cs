	...
	: IRequestHandler<PerformShowdownCommand, OneOf<PerformShowdownSuccessful, PerformShowdownError>>
{
		...
		CancellationToken cancellationToken)
	{
		...
		if (playersInHand.Count == 1)
		{
			...
			if (!isAlreadyAwarded)
			{
				...
				foreach (var pot in currentHandPots)
				{
					...
					pot.WinnerPayouts = System.Text.Json.JsonSerializer.Serialize(
						new[] { new { playerId = winner.PlayerId.ToString(), playerName = winner.Player.Name, amount = totalPot } });
				}
			...
			return new PerformShowdownSuccessful
			{
				...
				[
					new ShowdownPlayerHand
					{
						...
						Cards = winnerCards.Select(c => new ShowdownCard { Suit = c.Suit, Symbol = c.Symbol }).ToList(),
						HandType = null,
						HandStrength = null,
						IsWinner = true,
						AmountWon = totalPot
		...
		// Build community Card objects
		var communityCoreCards = communityCards
			.Select(c => new Card(MapSuit(c.Suit), MapSymbol(c.Symbol)))
			.ToList();

		// Evaluate hands using HoldTheBaseballHand (with wild card support)
		var playerHandEvaluations = new Dictionary<string, (HoldTheBaseballHand hand, List<GameCard> allCards, GamePlayer gamePlayer)>();
		...
		foreach (var gamePlayer in playersInHand)
		{
			...
			var hand = new HoldTheBaseballHand(holeCoreCards, communityCoreCards);

			var allDisplayCards = holeCardEntities.ToList();
			allDisplayCards.AddRange(communityCards);

			playerHandEvaluations[gamePlayer.Player.Name] = (hand, allDisplayCards, gamePlayer);
		}
		...
		var playerHandsList = playerHandEvaluations.Select(kvp =>
		{
			...
			// Use evaluated best cards for wild card display
			var bestHand = kvp.Value.hand.EvaluatedBestCards?.ToList()
				?? FindBestFiveCardHand(kvp.Value.hand);
			...
			return new ShowdownPlayerHand
			{
				...
				Cards = kvp.Value.allCards.Select(c => new ShowdownCard { Suit = c.Suit, Symbol = c.Symbol }).ToList(),
	...
	private static void MoveDealer(Game game)
	{
		...
		game.DealerPosition = seatsAfterCurrent.Count > 0 ? seatsAfterCurrent.First() : occupiedSeats.First();
	}
	...
	private async Task RecordHandHistoryAsync(
		...
		Dictionary<string, (HoldTheBaseballHand hand, List<GameCard> allCards, GamePlayer gamePlayer)>? playerHandEvaluations,
		CancellationToken cancellationToken)
	{
		...
		var playerResults = allPlayers.Select(gp =>
		{
			...
			{
				var bestHand = handEvaluation.hand.EvaluatedBestCards?.ToList()
					?? FindBestFiveCardHand(handEvaluation.hand);
	...
	private static List<Card> FindBestFiveCardHand(HoldTheBaseballHand hand)
	{
