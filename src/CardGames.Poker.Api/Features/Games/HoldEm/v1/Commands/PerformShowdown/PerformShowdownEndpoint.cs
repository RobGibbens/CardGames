...
/// <summary>
/// Endpoint for performing the showdown in a Texas Hold'Em game.
/// </summary>
public static class PerformShowdownEndpoint
{
	...
	public static RouteGroupBuilder MapPerformShowdown(this RouteGroupBuilder group)
	{
		group.MapPost("{gameId:guid}/showdown",
			...
			.WithName($"HoldEm{nameof(MapPerformShowdown).TrimPrefix("Map")}")
			.WithSummary("Perform Showdown")
			.WithDescription(
				"Performs the showdown phase to evaluate all remaining players' hands and award the pot(s) to the winner(s) in a Texas Hold'Em game. " +
				"Each player's best 5-card hand is determined from their 2 hole cards and 5 community cards.\n\n" +
				"**Showdown Scenarios:**\n" +
