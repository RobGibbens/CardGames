...
namespace CardGames.Poker.Api.Features.Games.Tollbooth.v1.Commands.ProcessBettingAction;
...
public static class ProcessBettingActionEndpoint
{
	public static RouteGroupBuilder MapProcessBettingAction(this RouteGroupBuilder group)
	{
		group.MapPost("{gameId:guid}/betting/actions",
			...
			.WithName($"Tollbooth{nameof(MapProcessBettingAction).TrimPrefix("Map")}")
			.WithSummary("Process Betting Action")
			.WithDescription("Processes a Tollbooth betting action and advances the hand when the street is complete.")
