	...
	public static RouteGroupBuilder MapCreateGame(this RouteGroupBuilder group)
	{
		group.MapPost("",
				async (CreateGameCommand command, IMediator mediator, CancellationToken cancellationToken) =>
				{
					...
					return result.Match(
						success => Results.Created($"/api/games/seven-card-stud/{success.GameId}", success.GameId),
			...
			.WithName($"SevenCardStud{nameof(MapCreateGame).TrimPrefix("Map")}")
			.WithSummary(nameof(MapCreateGame).TrimPrefix("Map"))
			.WithDescription("Create a new Seven Card Stud game.")
