	...
	: IRequestHandler<StartHandCommand, OneOf<StartHandSuccessful, StartHandError>>
{
		...
		CancellationToken cancellationToken)
	{
		...
		if (game is null)
		{
			return new StartHandError { Message = $"Game with ID '{command.GameId}' was not found.", Code = StartHandErrorCode.GameNotFound };
		}

		var validPhases = new[] { nameof(Phases.WaitingToStart), nameof(Phases.Complete) };
		if (!validPhases.Contains(game.CurrentPhase))
		{
			return new StartHandError { Message = $"Cannot start a new hand. Game is in '{game.CurrentPhase}' phase.", Code = StartHandErrorCode.InvalidGameState };
		}
		...
		var playersWithInsufficientChips = game.GamePlayers
			.Where(gp => gp.Status == GamePlayerStatus.Active && !gp.IsSittingOut && (gp.ChipStack <= 0 || (ante > 0 && gp.ChipStack < ante)))
			.ToList();
		...
		var eligiblePlayers = game.GamePlayers
			.Where(gp => gp.Status == GamePlayerStatus.Active && !gp.IsSittingOut && (ante == 0 || gp.ChipStack >= ante))
			.ToList();
		...
		{
			logger.LogInformation("StartHand: Removing {Count} existing cards from previous hands for game {GameId}", existingCards.Count, game.Id);
		...
		foreach (var card in dealer.DealCards(52))
		{
