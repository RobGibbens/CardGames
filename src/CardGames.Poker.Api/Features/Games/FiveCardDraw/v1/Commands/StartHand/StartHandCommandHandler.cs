	...
	: IRequestHandler<StartHandCommand, OneOf<StartHandSuccessful, StartHandError>>
{
		...
		CancellationToken cancellationToken)
	{
		...
		// 3. Get active players with chips
		...
		if (activePlayers.Count < 2)
		{
			return new StartHandError
			{
				Message = "Not enough players with chips to start a new hand. At least 2 players are required.",
				Code = StartHandErrorCode.NotEnoughPlayers
			};
		}

		// 4. Reset player states for new hand (mirrors FiveCardDrawGame.StartHand)
		...
		// 5. Remove any existing cards from previous hand
		...
		// 6. Create a new main pot for this hand
		var mainPot = new Pot
		{
			GameId = game.Id,
