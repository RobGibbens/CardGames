...
using CardGames.Poker.Games.Nebraska;

namespace CardGames.Poker.Api.GameFlow;

/// <summary>
/// Game flow handler for Nebraska poker.
/// </summary>
public sealed class NebraskaFlowHandler : BaseGameFlowHandler
{
    public override string GameTypeCode => "NEBRASKA";

    public override GameRules GetGameRules() => NebraskaRules.CreateGameRules();
    ...
    protected override async Task SendBettingActionAsync(AutoActionContext context, BettingActionType action, int amount = 0)
    {
        ...
        {
            context.Logger.LogError(ex, "Error performing auto-betting action for Nebraska game {GameId}", context.GameId);
        }
    }
}
