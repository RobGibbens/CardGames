...
## 2026-02-19: web-design-reviewer skill installed

**Requested by:** Rob Gibbens  
**Status:** ✅ Complete

**Work:**
- Sourced web-design-reviewer skill from `github/awesome-copilot` marketplace at `skills/web-design-reviewer/SKILL.md`
- Installed to `.ai-team/skills/web-design-reviewer/SKILL.md` (no supporting files required)

**Skill capabilities:**
- **Visual Inspection & Validation** — Inspect websites (local, staging, production) for design quality issues
- **Multi-Framework Support** — Works with static HTML/CSS/JS, React, Vue, Angular, Svelte, Next.js, Nuxt, SvelteKit, CMS platforms
- **Issue Detection** — Layout issues (overflow, overlap, alignment, spacing), responsive problems, accessibility issues, visual inconsistencies
- **Automated Workflow** — Information gathering → Visual inspection → Issue fixing → Re-verification → Completion report
- **Fix Principles** — Minimal changes, respect existing patterns, avoid breaking changes, add explanatory comments
- **Browser Automation** — Requires Playwright MCP (reference implementation) or compatible tools (Selenium, Puppeteer, Cypress)
- **Severity Prioritization** — P1 (fix immediately), P2 (fix next), P3 (fix if possible)
- **Responsive Testing** — Tests at Mobile (375px), Tablet (768px), Desktop (1280px), Wide (1920px) viewports

**Issue Categories:**
- Layout Issues: Element overflow, overlap, alignment, spacing, text clipping
- Responsive Issues: Mobile-friendliness, breakpoint transitions, touch targets
- Accessibility Issues: Contrast ratios, focus states, alt text
- Visual Consistency: Font families, color schemes, spacing uniformity

**Why:** The CardGames Poker Web UI (`src/CardGames.Poker.Web/`) is built with Blazor. Squad agents equipped with web-design-reviewer can autonomously inspect the running application, detect design issues visually, locate problematic components, and apply surgical fixes—reducing friction during UI quality gates and enabling structured design review processes accessible to the whole team.

**Prerequisites:**
- Target website must be running (local dev server, staging, or production)
- Browser automation capability (Playwright MCP recommended)
- Access to source code for fix application

**Integration:**
- Skill available to squad agents immediately after routing reload
- Optional: Future MCP server integration with Playwright for browser automation
- No changes to build, runtime, or deployment
