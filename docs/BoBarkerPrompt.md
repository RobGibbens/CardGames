...
Game Name: Bob Barker
Game Type Code : BOBBARKER
Image Name : bobbarker.png
Variant Family: HoldEm-Style
Mode: Production-Ready

Rules:
"Bob Barker" should be based on Omaha, as the rules and game play are very similar.  This is a Hold 'Em style game. When the game starts, there will be a big blind and a small blind. Each player is dealt five cards face down.  Then the dealer deals one card face down (no player can see the value) to the center of the table. Then each player marks one of their hole cards as their "showcase" card. This card is moved to the side of their hole cards and is no longer used in their hand. After that is the first betting round, and proceeds according to the Omaha rules with the remaining four cards in their hole hands (Must use two and exactly two hole cards in their final hand).  During the Showdown, 1/2 of the pot goes to the traditional winner with the best five card hand.  The other 1/2 of the pot goes to the player(s) whose "showcase" cards are the closest to the single dealer card without going over the dealer card amount.
